    ...
    def translate(self, string):
        ...
        if log:
            ...
            try:
                ...
                pid = "0000"
            remember_dict["pid"] = pid
            # <editor-fold desc="过滤一次已经翻译过的演示字符串msg">
            global last_translated_origin_msg
            if len(last_translated_origin_msg) == len(msg):
                if last_translated_origin_msg == msg:
                    return None
            # </editor-fold>
            for translator in self.tag_translators:
                show = translator.translate(tag, msg)
                if show:
                    last_translated_origin_msg = msg
                    # <editor-fold desc="翻译结果过滤一次防止连续显示同一个翻译结论">
                    global last_translated_msg
                    translated = show.translated
                    if len(last_translated_msg) == len(translated):
                        if last_translated_msg == translated:
                            return None
                    last_translated_msg = translated
                    # </editor-fold>
                    show.time = time
                    show.original = msg
                    show.process = pid
