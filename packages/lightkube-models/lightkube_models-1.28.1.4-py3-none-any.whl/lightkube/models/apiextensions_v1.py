...
class ValidationRule(DataclassDictMixIn):
    """ValidationRule describes a validation rule written in the CEL expression
      ...
      * **fieldPath** ``str`` - *(optional)* fieldPath represents the field path returned when the validation fails. It
      ...
      * **messageExpression** ``str`` - *(optional)* MessageExpression declares a CEL expression that evaluates to the validation
        ...
        ("+string(self.max)+")"
      * **reason** ``str`` - *(optional)* reason provides a machine-readable validation failure reason that is returned
