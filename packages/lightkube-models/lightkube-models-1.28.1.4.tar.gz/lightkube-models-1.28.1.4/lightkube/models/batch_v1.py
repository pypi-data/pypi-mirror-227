...
class JobSpec(DataclassDictMixIn):
    """JobSpec describes how the job execution will look like.
      ...
      * **backoffLimitPerIndex** ``int`` - *(optional)* Specifies the limit for the number of retries within an index before marking
      ...
      * **maxFailedIndexes** ``int`` - *(optional)* Specifies the maximal number of failed indexes before marking the Job as
      ...
      * **podReplacementPolicy** ``str`` - *(optional)* podReplacementPolicy specifies when to create replacement Pods. Possible
...
class JobStatus(DataclassDictMixIn):
    """JobStatus represents the current state of a Job.
      ...
      * **failedIndexes** ``str`` - *(optional)* FailedIndexes holds the failed indexes when backoffLimitPerIndex=true. The
      ...
      * **terminating** ``int`` - *(optional)* The number of pods which are terminating (in phase Pending or Running and have
        a deletionTimestamp).
