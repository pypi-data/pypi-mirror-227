...
class SelfSubjectReviewStatus(DataclassDictMixIn):
    """SelfSubjectReviewStatus is filled by the kube-apiserver and sent back to a
      ...
      * **userInfo** ``UserInfo`` - *(optional)* User attributes of the user making this request.
    """
...
class TokenRequest(DataclassDictMixIn):
    """TokenRequest requests a token for a given service account.
      ...
      * **metadata** ``meta_v1.ObjectMeta`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **status** ``TokenRequestStatus`` - *(optional)* Status is filled in by the server and indicates whether the token can be
        authenticated.
    """
