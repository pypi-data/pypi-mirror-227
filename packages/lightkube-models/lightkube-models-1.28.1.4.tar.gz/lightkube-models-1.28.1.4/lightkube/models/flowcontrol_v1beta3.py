...
@dataclass
class ExemptPriorityLevelConfiguration(DataclassDictMixIn):
    """ExemptPriorityLevelConfiguration describes the configurable aspects of the
      ...
      * **lendablePercent** ``int`` - *(optional)* `lendablePercent` prescribes the fraction of the level's NominalCL that can be
      ...
      * **nominalConcurrencyShares** ``int`` - *(optional)* `nominalConcurrencyShares` (NCS) contributes to the computation of the
...
class LimitedPriorityLevelConfiguration(DataclassDictMixIn):
    """LimitedPriorityLevelConfiguration specifies how to handle requests that are
      ...
      * **borrowingLimitPercent** ``int`` - *(optional)* `borrowingLimitPercent`, if present, configures a limit on how many seats this
...
class PriorityLevelConfigurationSpec(DataclassDictMixIn):
    """PriorityLevelConfigurationSpec specifies the configuration of a priority
      ...
      * **exempt** ``ExemptPriorityLevelConfiguration`` - *(optional)* `exempt` specifies how requests are handled for an exempt priority level. This
