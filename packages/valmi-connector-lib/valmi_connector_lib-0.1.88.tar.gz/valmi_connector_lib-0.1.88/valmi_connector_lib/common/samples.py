...
class SampleWriter:
    ...
    def get_writer_by_metric_type(cls, store_config_str=None,
        ...
        if metric_type is not None:
            SampleWriter.metric_type = metric_type

        if SampleWriter.metric_type is not None and \
            (SampleWriter.metric_type not in SampleWriter.writers
             or SampleWriter.writers[SampleWriter.metric_type] is None):
            SampleWriter.writers[SampleWriter.metric_type] = SampleWriter(SampleWriter.store_config_str,
                                                                          ChunkEndFlushPolicy(store_config_str=SampleWriter.store_config_str),
                                                                          SampleWriter.sync_id,
                                                                          SampleWriter.run_id,
                                                                          SampleWriter.connector,
                                                                          SampleWriter.metric_type)
        if SampleWriter.metric_type is not None:
            return SampleWriter.writers[SampleWriter.metric_type]
        return None
