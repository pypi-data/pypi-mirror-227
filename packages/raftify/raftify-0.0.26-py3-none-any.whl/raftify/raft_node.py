...
class RaftNode:
    ...
    async def handle_config_change_entry(
        ...
        # Previous configuration change are not applied yet, just return and caller should try later.
        while self.raw_node.get_raft().has_pending_conf():
            self.logger.info(
                "Ignore the confchage because there are other pending confchanges."
            )
            if sender := senders.pop(seq, None):
                sender.put_nowait(JoinFailedRespMessage())
            return
