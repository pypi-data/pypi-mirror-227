            ...
            if isinstance(message, ConfigChangeReqMessage):
                ...
                if not self.is_leader():
                    ...
                    if self.raw_node.get_raft().has_pending_conf():
                        sender = client_senders[self.seq.value]

                        self.logger.info(
                            "Ignore the confchange because there are other pending confchanges."
                        )
                        sender.put_nowait(ConfChangeFailedRespMessage())
                    else:
                        self.seq.increase()
