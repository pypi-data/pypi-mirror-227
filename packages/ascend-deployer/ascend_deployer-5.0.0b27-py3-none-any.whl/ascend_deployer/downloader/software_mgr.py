...
G_SOFTWARE_LIST = set()
sys_software_list, framework_whl_list, other_software_list = ([] for _ in range(3))
...
class Software(object):
    ...
    def get_framework_whl(self, sys_name):
        """
        get framework whl
        """
        if sys_name not in self.framework_whl:
            return []
        return self.framework_whl.get(sys_name, '')

    def set_framework_whl(self, sys_name, pkg_list):
        """
        set framework_whl
        """
        self.framework_whl[sys_name] = pkg_list
...
def load_software(json_file):
    ...
    with open(json_file) as file_obj:
        ...
        if 'framework_whl' in json_obj:
            for sys_obj in json_obj['framework_whl']:
                soft.set_framework_whl(sys_obj['system'], sys_obj['whl'])
            framework_whl_list.append(soft)
...
def software_init():
    ...
    G_SOFTWARE_LIST = set(sys_software_list + framework_whl_list + other_software_list)
...
def get_software_framework(name, sys_name, version=None):
    """
    获取软件依赖的操作系统依赖
    :param in:  name      软件名
    :param in:  sys_name  操作系统
    ...
    if len(G_SOFTWARE_LIST) == 0:
        software_init()
    for soft in framework_whl_list:
        if version is None:
            if soft.get_name().lower() == name.lower():
                return soft.get_framework_whl(sys_name)
        else:
            if soft.get_name().lower() == name.lower() and soft.get_version() == version:
                return soft.get_framework_whl(sys_name)
    return []
