...
class BaseResource(View):
    ...
    async def update_obj(self, id, body):
        ...
        if self.update_fields:
            diff = set(keys) - set(self.create_fields)
            diff = (', ').join(list(diff))

        if not allowed or diff:
    ...
    async def create_obj(self, request, body):
        ...
        for field in self.model._meta.local_fields:
            ...
            if default is not True and not blank and field_value == '':
                blank_errors.append(field.verbose_name)

            if default is not True and not null and not field_value:
        ...
        if blank_errors or null_errors:
            errors = ', '.join(blank_errors)
            errors += ', '.join(null_errors)
            raise HTTPException(403, f'Field(s): {errors} can\'t be blank or null')
