...
class JunoConfig(UserDict):
    def __init__(self):
        ...
        if os.path.exists('project-ext.yml'):
            with open('project-ext.yml', 'r') as f:
        ...
        try:
            ...
            self.data["instance_id"] = instance_id = dcfg(
                "instance_id", "") if dcfg("instance_id", "") else self.data['plant'] + '-' + self.data['module'] + '.' + uuid.uuid4().hex + '.' + datetime.now().strftime("%Y%m%d%H%M%S")
        except:
            logging.error(
                "can't find plant and module. make sure you are under an project folder")
        self.data["input_cfg"] = {}
        try:
