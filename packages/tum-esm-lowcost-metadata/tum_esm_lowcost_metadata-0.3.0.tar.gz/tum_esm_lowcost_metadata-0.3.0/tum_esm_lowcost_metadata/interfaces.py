...
def load_from_github(
    ...
    return Interface(
        samples=[types.Sample(**item) for item in _req("SAMPLING")],
        sensors={key: types.Sensor(sensor_id=key, **dict(value)) for key, value in _req("SENSORS").items()}, 
        sites={key: types.Site(site_id=key, **dict(value)) for key, value in _req("SITES").items()},
        )
