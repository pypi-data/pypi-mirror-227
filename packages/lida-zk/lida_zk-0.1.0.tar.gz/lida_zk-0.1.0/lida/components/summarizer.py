...
class Summarizer():
    ...
    def get_column_properties(self, df: pd.DataFrame, n_samples: int = 3) -> list[dict]:
        """Get properties of each column in a pandas DataFrame 获取pandas DataFrame中每列的属性"""
        properties_list = []
    ...
    def encrich(self, base_summary: dict, text_gen: TextGenerator,
        ...
        try:
            ...
            enriched_summary = json.loads(json_string)          # TypeError: can only concatenate str (not "dict") to str
    ...
    def summarize(
        ...
        data_properties = self.get_column_properties(data, n_samples)           # 获取数据的属性
