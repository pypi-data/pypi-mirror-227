...
def preprocess_code(code: str) -> str:
    """Preprocess code to remove any preamble and explanation text 预处理代码以删除任何序言和解释文本"""
    # print("process!!!")
...
def get_globals_dict(code_string, data):
    # Parse the code string into an AST 将代码字符串解析为AST
...
class ChartExecutor:
    ...
    def execute(
        ...
        """Validate and convert code 验证并且转换成可执行的code"""
        ...
        if library == "altair":
            ...
            for code in code_specs:
                try:
                    # print(code)
                    ex_locals = get_globals_dict(code, data)             # AST语法树解析
                    print(ex_locals.keys())         # dict_keys(['plt', 'pd', 'np', 'data'])
                    exec(code, ex_locals)           # 校验代码有没有bug  exec:执行动态创建的程序，该程序是字符串或代码对象
                    chart = ex_locals["chart"]
                    if plt:
                        buf = io.BytesIO()
                        plt.box(False)
