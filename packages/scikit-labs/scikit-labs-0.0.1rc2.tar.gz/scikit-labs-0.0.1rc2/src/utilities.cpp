...
List pack_result(Eigen::VectorXd &beta, double &coef0, Eigen::MatrixXd &path) {
    Eigen::MatrixXd beta_temp = beta;
    Eigen::VectorXd coef0_temp = Eigen::VectorXd::Ones(1) * coef0;
    return std::make_tuple(beta_temp, coef0_temp, path);
};
List pack_result(Eigen::MatrixXd &beta, Eigen::VectorXd &coef0, Eigen::MatrixXd &path) {
    return std::make_tuple(beta, coef0, path);
};

Eigen::MatrixXd Seg_by_row(Eigen::MatrixXd &x, Eigen::VectorXi &ind) {
    Eigen::MatrixXd x1(ind.size(), x.cols());
    for (int i = 0; i < ind.size(); i++) {
        x1.row(i) = x.row(ind(i));
    }
    return x1;
};

Eigen::SparseMatrix<double> Seg_by_row(Eigen::SparseMatrix<double> &x, Eigen::VectorXi &ind) {
    Eigen::SparseMatrix<double> temp(ind.size(), x.rows());
    temp.reserve(ind.size());
    for (int i = 0; i < ind.size(); i++) {
        temp.insert(i, ind(i)) = 1;
    }
    return (temp * x);
};
