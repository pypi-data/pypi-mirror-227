...
class SDK:
    ...
    async def dial(self,
        ...
        if response.get("context", False) and response['context'] == Joker.INVALIDAUTH:
            self.api_key = None
            raise INVALIDAPIKEY(ExceptionMsg.INVALID_KEY)
        
        elif response.get("success", False) and response['status'] == Joker.SUCCESS:
            return 1
        
        elif response.get("context", False) and response['context'] == Joker.INVALIDCOUNTRY:
            raise INVALIDCOUNTRY(ExceptionMsg.INVALID_COUNTRY)
        
        elif response.get("context", False) and response['context'] == Joker.INVALIDPARAM:
            raise INVALIDPARAM(ExceptionMsg.INVALID_PARAM)
