...
class Transits():
    ...
    def is_fake_aspect(self, aspect: dict) -> bool:
        ...
        if aspect['aspect'] in {'trine', 'conjunction'} and element_1 != element_2 and orb > 1.5:
            return True
            ...
            while current_time <= self.end:
                ...
                while current_time <= self.end:
                    ...
                    if old_sign != current_sign:
                        sign_transitions[self.planet_names[planet]['name']] = {'when': self.__get_local_time(current_time), 'old': old_sign, 'new': current_sign, 'ascendant': self.find_ascendant(jd)}
                    if old_direction != current_direction:
                        direction_status = "retrograde" if current_direction else "direct"
                        direction_transitions[self.planet_names[planet]['name']] = {'when': self.__get_local_time(current_time), 'status': direction_status, 'ascendant': self.find_ascendant(jd)}
                    if old_house != current_house:
                        house_transitions[self.planet_names[planet]['name']] = {'when': self.__get_local_time(current_time), 'old': old_house, 'new': current_house, 'ascendant': self.find_ascendant(jd)}
