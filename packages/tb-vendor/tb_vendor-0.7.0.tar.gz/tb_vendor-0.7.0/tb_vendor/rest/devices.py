...
def get_devices_by_type(
    rest_client: rest_models.RestClient,
    data_type: str,
    retry_for_timeout: int,
    page_size: int = 100,
) -> List[rest_models.TbDevice]:
...
def add_credentials(
    rest_client: rest_models.RestClient, devices: List[rest_models.Device]
) -> List[TbDevice]:
...
def main_device_inventory(
    RestClientKlass: rest_models.RestClientClass,
    base_url: str,
    username: str,
    password: str,
    data_type: str,
