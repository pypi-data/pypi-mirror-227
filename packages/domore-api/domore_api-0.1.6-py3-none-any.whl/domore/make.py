...
# original: all test steps in one class
...
{% for index in range(class_name|length) %}
class {{ class_name[index] }}(DoMore):
    ...
    teststeps = [
        {{ teststeps_chain_style[index] }}
    ]
{% endfor %}

if __name__ == "__main__":
    {% for index in range(class_name|length) %}
    {{ class_name[index] }}().test_start()
    {% endfor %}
"""
)
...
def make_classname_chain_style(teststeps: List[dict]) -> List[str]:
    if len(teststeps) < 1:
        raise exceptions.TestCaseFormatError(f"Invalid test step: {teststeps}")

    # Extract api name within HTTP request as class name
    classnames = [f'TestCase{teststep["name"].title().replace("/", "").replace("-", "").replace(".", "").replace(":", "")}' for teststep in teststeps]
    # If there are duplicate class names, append a number to the end to make them unique
    unique_classnames = [v + str(classnames[:i].count(v) + 1) if classnames.count(v) > 1 else v for i, v in enumerate(classnames)]
...
def make_teststep_chain_style(teststep: Dict) -> Text:
    if teststep.get("request"):
        ...
        raise exceptions.TestCaseFormatError(f"Invalid test step: {teststep}")
...
def make_testcase(testcase: Dict, dir_path: Text = None) -> Text:
    ...
    # modification: all test steps in different class
    data = {
        "version": __version__,
        "testcase_path": testcase_path,
        "diff_levels": diff_levels,
        "class_name": make_classname_chain_style(teststeps),
        "imports_list": imports_list,
        "config_chain_style": make_config_chain_style(config),
        "parameters": config.get("parameters"),
        "teststeps_chain_style": [
