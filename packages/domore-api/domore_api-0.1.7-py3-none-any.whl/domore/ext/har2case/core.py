...
class HarParser(object):
    def __init__(self, har_file_path, filter_str=None, exclude_str=None, profile=None):
        ...
        self.cookie = profile and utils.extract_session_token_from_profile(self.profile.get("login_info"))
    ...
    def __make_request_cookies(self, teststep_dict, entry_json):
        ...
        elif self.profile and self.profile.get("login_info"):
            for cookie in entry_json["request"].get("cookies", []):
                if cookie["name"] == self.cookie.split("=")[0]:
                    cookies[cookie["name"]] = self.cookie.split("=")[-1]
                    logger.info(f"the property name of 'Cookie': {self.cookie.split('=')[0]}")
                    logger.info(f"the property value of 'Cookie': {self.cookie.split('=')[-1]}")
                    continue
    ...
    def __make_request_headers(self, teststep_dict, entry_json):
        ...
        """
        teststep_headers = {}
        entry_json_headers: List = entry_json["request"].get("headers", [])
        # use user defined parameters of HTTP Header to substitute original
        if self.profile and self.profile.get("headers"):
            ...
            for k, v in headers.items():
                for header in entry_json_headers:
                    teststep_headers[header["name"]] = header["value"]
                    if header["name"] == k:
                        entry_json_headers.remove(header)
                        teststep_headers.pop(k)
                        teststep_headers[k] = v

            if teststep_headers:
                teststep_dict["request"]["headers"] = teststep_headers
        elif self.profile and self.profile.get("login_info"):
            for header in entry_json_headers:
                # use new token to substitute original cookies and all HTTP request headers which contain the same token
                if header["name"] == "cookie" or header["name"] == "Cookie":
                    # replace cookies with the latest token
                    new_cookie = self.cookie
                    teststep_headers[header["name"]] = new_cookie
                    # remove cookie in the headers in case it's reused in the following for loop
                    entry_json_headers.remove(header)
                    for v in entry_json_headers:
                        # replace original token in HTTP request header with the latest one
                        if v["value"] == header["value"].split("=")[-1]:
                            teststep_headers[v["name"]] = new_cookie.split("=")[-1]
                            entry_json_headers.remove(v)
                        # remove original other token in HTTP request header if it's not the session token
                        if v["value"] != header["value"].split("=")[-1] and "token" in v["name"].lower():
                            if v["name"] in teststep_headers.keys():
                                teststep_headers.pop(v["name"])
                            entry_json_headers.remove(v)
                    continue
            ...
            for header in entry_json["request"].get("headers", []):
                if header["name"] == "cookie" or header["name"] == "Cookie" or header["name"].startswith(":"):
                    continue
    ...
    def _prepare_teststeps(self):
        ...
        def is_exclude(url, exclude_str):
            ...
            for exclude_str in exclude_str_list:
                if exclude_str and urlparse.unquote(exclude_str) in url:   # decode url in case of it being encoded
                    return True

            return False

        def is_filter(url, filter_str: str):
            filter_str_list = filter_str.split("|")
            for filter_str in filter_str_list:
                if filter_str and urlparse.unquote(filter_str) in url:  # decode url in case of it being encoded
                    return False
        ...
        for entry_json in log_entries:
            url = entry_json["request"].get("url")
            if self.filter_str and is_filter(url, self.filter_str):
                continue
