...
def is_domore_testcase(item):
    """ check if a variable is a DoMore testcase class
    """
    from domore import DoMore

    # TODO: skip referenced testcase
    return bool(
        inspect.isclass(item)
        and issubclass(item, DoMore)
        and item.__name__ != "DoMore"
    )
...
def prepare_locust_tests() -> List:
    ...
    for pytest_file in pytest_files:
        ...
        for name, item in vars(module).items():
            ...
            if not is_domore_testcase(item):
                continue
...
def main_locusts():
    ...
    from domore.make import main_make
