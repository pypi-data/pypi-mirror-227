...
# NEEDS TO BE REMOVED WHEN CONFIDENT NEW CODE IS WORKING
# def _define_start_end(cadence, normal_cadence, interp_limit):
#     """
...
#     start = cadence.index(normal_cadence)  # Look for first good frame
#     nfiles = len(cadence)
#     end = nfiles-1
#     criteria = interp_limit*normal_cadence

#     # Find locations where the jump is larger than the Limit x normal cadence
#     (large_gap_loc,) = np.where(np.asarray(cadence[start:end]) > criteria)

#     # Determine final image - either last frame or frame before large gap
#     end_frame = end if len(large_gap_loc) == 0 else large_gap_loc[0]+start

#     # Cuts down file list to those values being loaded in
#     print('Potential files to read in: Start of sequence', start,
#           ' || End of sequence', end_frame)
...
def _find_gaps(date, cadence, normal_cadence):
    ...
    if number_gaps > 0:
        gap_size = cadence[location_of_gaps]//normal_cadence
...
def _interpolate_data(cubes, gap_info, nx, ny):
    ...
    for start, end in zip(gap_start, gap_size):
        ...
        for add, zvals in enumerate(z):
            ...
            # NEEDS TO BE REMOVED WHEN CONFIDENT NEW CODE IS WORKING
            # dumi = np.r_['0,3', cube_i[ii-1], cube_i[loc]]
            # dumv = np.r_['0,3', cube_v[ii-1], cube_v[loc]]
            # dumw = np.r_['0,3', cube_w[ii-1], cube_w[loc]]

            # # Order=1 set interpolation to linear
            # loc2 = ii+add
            # cube_i[loc2] = ndimage.map_coordinates(dumi, coords, order=1).T
            # cube_v[loc2] = ndimage.map_coordinates(dumv, coords, order=1).T
            # cube_w[loc2] = ndimage.map_coordinates(dumw, coords, order=1).T
    ...
    for ii in range(nt):
        if ii != frames_to_skip[k]:
            with fits.open(file_list[ii-add]) as hdul:
                ...
                cube_i[ii] = hdul["Intensity"].data
                cube_i_head.append(hdul["Intensity"].header)

                cube_v[ii] = hdul[dop_string].data
                cube_v_head.append(hdul[dop_string].header)

                cube_w[ii] = hdul["Line Width"].data
                cube_w_head.append(hdul["Line Width"].header)
