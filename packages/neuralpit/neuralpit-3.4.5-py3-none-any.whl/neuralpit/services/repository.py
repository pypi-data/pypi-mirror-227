...
import time
import boto3
from neuralpit.utils.settings import appSettings
...
conversationTable = dynamodb.Table(appSettings.get('CONVERSATION_TABLE'))
conversationDocumentTable = dynamodb.Table(appSettings.get('CONVERSATION_DOCUMENT_TABLE'))
conversationDatasourceTable = dynamodb.Table(appSettings.get('CONVERSATION_DATASOURCE_TABLE'))
conversationHistoryTable = dynamodb.Table(appSettings.get('CONVERSATION_HISTORY_TABLE'))
openAiTable = dynamodb.Table(appSettings.get('OPENAI_KEY_TABLE'))

openAiRepo = OpenAIKeyRepository(openAiTable)
conversationRepo = ConversationRepository(conversationTable,openAiRepo)
conversationDocumentRepo = ConversationDocumentRepository(conversationDocumentTable)
conversationDatasourceRepo = ConversationDatasourceRepository(conversationDatasourceTable)
conversationHistoryRepo = ConversationHistoryRepository(conversationHistoryTable)
