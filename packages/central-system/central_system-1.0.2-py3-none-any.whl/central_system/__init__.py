...
def set_file_logger(file_path, name="central_system", level=logging.INFO, format_string=None):
    global logger
...
def set_stream_logger(name='central_system', level=logging.DEBUG, format_string=None):
    global logger
    if not format_string:
        format_string = "%(asctime)s %(name)s [%(levelname)s] %(thread)d : %(message)s"
    logger = logging.getLogger(name)
