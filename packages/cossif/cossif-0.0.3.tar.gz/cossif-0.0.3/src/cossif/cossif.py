...
class CosSIF:
    ...
    def _calculate_similarities(self, t_path, s_path, record_save_path, file_name, record_range=0, image_size=64):
        ...
        if os.path.exists(record_save_path) != True:
            raise FileNotFoundError(
                f"Error! Path '{record_save_path}' does not exist!")
        ...
        t_table.add_row([self._get_folder_name(
            t_path), len(os.listdir(t_path))])
        print(t_table)
        ...
        for path in s_path:
            ...
            s_table.add_row(
                [self._get_folder_name(path), len(os.listdir(path))])
        print(s_table)
        ...
        temp_dir, t_dir, s_dir = self._create_temp_directories(
            t_path, s_path, record_save_path, image_size)
        ...
        def _update_range_cell(range_cell, temp_record):
            for idx, i in enumerate(temp_record):
                range_cell['SECONDARY_CLASS_NAME_%s' %
                           str(idx + 1)] = i['NAME']
                range_cell['SECONDARY_CLASS_IMAGE_%s' %
                           str(idx + 1)] = i['IMAGE']
                range_cell['COSINE_DISTANCE_SCORE_%s' %
                           str(idx + 1)] = i['SCORE']
        ...
        for t_img in tqdm(os.listdir(t_dir), colour='BLUE'):
            ...
            for class_name in os.listdir(s_dir):
                ...
                for s_img in os.listdir(class_path):
                    ...
                    result = self._calculate_cosine_distance(
                        imgA_from_array, imgB_from_array)
            ...
            # reverse sort
            temp_sort = sorted(
                temp_sort, key=lambda d: d['SCORE'], reverse=True)
        ...
        df.to_csv(os.path.join(record_save_path,
            ...
            if remove == 'similar':
                for cell in set_R:
                    if cell_count == filtered_images:
                        ...
                        threshold_table.field_names = [
                            "Total Images", "Filtered Images", "Removed Images", "Threshold Score", "Order"]
                        threshold_table.add_row(
                            [total_images, filtered_images, removed_images, threshold_score, "Similar"])
                ...
                for cell in reversed(set_R):
                    if cell_count == filtered_images:
                        ...
                        threshold_table.field_names = [
                            "Total Images", "Filtered Images", "Removed Images", "Threshold Score", "Order"]
                        threshold_table.add_row(
                            [total_images, filtered_images, removed_images, threshold_score, "Dissimilar"])
    ...
    def calculate_and_filter(self, target_path, secondary_path, record_save_path, file_name, filter_type, filter_range, filtered_path,
        ...
        # load saved records
        set_R_of_records = np.load(os.path.join(
            record_save_path, f'{file_name}.npy'), allow_pickle=True)
    ...
    def filter_from_records(self, target_path, record_save_path, file_name, filter_type, filter_range, filtered_path, removed_path):
        # load saved records
        set_R_of_records = np.load(os.path.join(
            record_save_path, f'{file_name}'), allow_pickle=True)
