...
def gemm_quantize_initializers(qgemm,
    ...
    if output_32_bit:
        ...
        scale, s_out, output_scale = downscale(output_range, i_scale)
    ...
    qnode = qgemm.make_node(inputs, [output_name], name=gemm_name)
    onnx_weights = array_to_tp(**weights_dict)

    return qnode, output_scale, onnx_weights


def gemm_convert(nodes, tensor_range, graph, i_scale, last_block):
