...
from .animation import EauAnimation
import pygame
...
class AnimatedSprite(EauSprite):
  ...
  def __init__(self, *paths: str, size: int, animations: dict[str, EauAnimation]) -> None:
    super().__init__()
    self.images: list[tuple[pygame.Surface, pygame.Rect]] = []
    for i in range(size):
      image, rect = load_image(*paths[:-1], f"{paths[-1]}_{i}")
      self.images.append((image, rect))
    self.animations = animations
    self.action = self.animations.keys()[0]

  @property
  def animation(self) -> EauAnimation:
