...
def run():
    ...
    for sheet in r.sheets:
        ...
        for violation in sheet.violations:
            tc = test_cases.get(violation.type, TestCase(violation.type, allow_multiple_subelements=True))
            ...
            if violation.severity == "ignore":
                ...
                tc.add_failure_info(violation.description, output, failure_type=violation.severity)
            test_cases[violation.type] = tc

        ts = TestSuite(sheet.path, list(test_cases.values()), timestamp=r.date, file=r.source, properties={'uuid_path': sheet.uuid_path})
