...
def any1D(pos_optimized, latt_system, elastic_constants_dict, stress_set_dict, conversion_factor):
    stresses = []
    strains = []
    ...
    for key in stress_set_dict.keys():
        ...
        for s in strain_matrices:
            current_strains = array(s) 
            strains.append(current_strains)

        current_stresses = array([[current_stress_list[i]] for i in range(3)])
        stresses.extend(current_stresses)  # Using extend instead of append to match dimensions
...
# Printing out c_ij components
    for i in range(3):
        #print(f'c{i+1}3 = {cij[i][0]}')  # Get values from the first column of each row
        key = f'c{i+1}3'
        elastic_constants_dict[key] = cij[i][0]
...
def Hydrostatic(pos_optimized, latt_system, elastic_constants_dict, stress_set_dict, conversion_factor):
    stresses = []
    strains = []
...
#    for i in range(3):
#        for j in range(1):
#            #print(f'c{i+1}{j+1} = {cij[i][j]}')
#            key = f'c{i+1}{j+1}'
#            elastic_constants_dict[key] = cij[i][j]
    ...
    # Iterate through the c_matrix to store finite elastic constants for a column matrix
#    for i in range(3):
#        cij = c_matrix[i, 0]  # Accessing the value from the only column in the column matrix
#        if isfinite(cij):
#            key = f'c{i+1}{i+1}'  # Naming convention for diagonal elastic constants cii
#            elastic_constants_dict[key] = cij
...
def Biaxial(pos_optimized, latt_system, elastic_constants_dict, stress_set_dict, conversion_factor):
    stresses = []
    strains = []
...
def Radial(pos_optimized, latt_system, elastic_constants_dict, stress_set_dict, conversion_factor):
    stresses = []
    strains = []
...
def Nanotube(pos_optimized, latt_system, elastic_constants_dict, stress_set_dict, conversion_factor):
    ...
    elastic_constants_dict['c33'] = c33  # Longitudinal elastic constant
    elastic_constants_dict['c22'] = c22  # Circumferential elastic constant
    elastic_constants_dict['c23'] = c23  # Torsional elastic constant
...
def calc_elastic_constants(pos_optimized, latt_system, elastic_constants_dict, stress_set_dict):
    ...
    if strains_matrix != 'asess':
        ...
        elif latt_system == 'Hydrostatic':
            elastic_constants_dict = Hydrostatic(pos_optimized, latt_system, elastic_constants_dict, stress_set_dict, conversion_factor)   
        elif latt_system == 'Biaxial':
            elastic_constants_dict = Biaxial(pos_optimized, latt_system, elastic_constants_dict, stress_set_dict, conversion_factor)   

        elif latt_system == 'Radial':
            elastic_constants_dict = Radial(pos_optimized, latt_system, elastic_constants_dict, stress_set_dict, conversion_factor) 
