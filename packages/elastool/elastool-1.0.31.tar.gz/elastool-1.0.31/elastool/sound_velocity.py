...
#        print(Y_2D)
       ...
       # Na = 6.02214076E+23  # Avogadro's number
        ...
        if latt_system == 'any1D': #Longitudinal strain along z-direction
        ...
        elif latt_system == 'Hydrostatic': #Material experiences strain equally in all three directions: x, y, and z.
            c13 = elastic_constants_dict['c13']
            c23 = elastic_constants_dict['c23']
            c33 = elastic_constants_dict['c33']
            v = -c23/c33
            K = (c13+c23+c33)/3. # c13/3
            G = c13/(1.+v)/2.
            V_l = 1E-3 * sqrt((abs(c33) * 1E+9) / mass_density)
            V_t = 1E-3 * sqrt(abs(G*1E+9) / mass_density)
            V_a = sqrt(abs(V_l * V_t))
            T_D = (h / k) * (3. * n / 4. / pi * Na * mass_density / M / 1E-3)**(1. / 3.) * V_a * 1E+3 #hbar / k * ( n * Na * mass_density / M / 1E-3)**(1. / 3.) * V_a * 1E+3
            ...
            R_f = 0.56 * sqrt(abs(c13) * 1E+9 / mass_density) / (2 * pi * length* 1E-10 )
            
            elastic_constants_dict['Y'] = c33
            elastic_constants_dict['B'] = K # Bulk modulus
            elastic_constants_dict['G_L'] = G #Shear modulus
            elastic_constants_dict['v'] = v
            elastic_constants_dict['V_t'] = V_t
            elastic_constants_dict['V_l'] = V_l
            elastic_constants_dict['T_D'] = T_D
        ...
        elif latt_system == 'Biaxial': #material experiences shearing forces in two different planes (x-z and y-z) simultaneously
            c13 = elastic_constants_dict['c13']
            c23 = elastic_constants_dict['c23']
            c33 = elastic_constants_dict['c33']
            vx = -c33/c13
            ...
            Yx = c13/(1.-vx**2)
            Yy = c23/(1.-vy**2)
            K = (c13+c23+2.*c33)/3. # c13/3
            G = (c13+c23-2.*c33)/3.
            V_l = 1E-3 * sqrt((abs(K +3/4.*G) * 1E+9) / mass_density)
            V_t = 1E-3 * sqrt(abs(G*1E+9) / mass_density)
            V_a = sqrt(abs(V_l * V_t))
            T_D = (h / k) * (3. * n / 4. / pi * Na * mass_density / M / 1E-3)**(1. / 3.) * V_a * 1E+3 #hbar / k * ( n * Na * mass_density / M / 1E-3)**(1. / 3.) * V_a * 1E+3
            ...
            elastic_constants_dict['Y_x'] = Yx
            elastic_constants_dict['Y_y'] = Yy
            elastic_constants_dict['B'] = K # Bulk modulus
            elastic_constants_dict['G_L'] = G #Shear modulus
            elastic_constants_dict['v_x'] = vx
            elastic_constants_dict['v_y'] = vy
            elastic_constants_dict['V_t'] = V_t
            elastic_constants_dict['V_l'] = V_l
            elastic_constants_dict['T_D'] = T_D
            elastic_constants_dict['T_D_l'] = T_D_l
        ...
        elif latt_system == 'Radial': #material experiences shearing forces in two different planes (x-z and y-z) simultaneously
            c13 = elastic_constants_dict['c13']
            c23 = elastic_constants_dict['c23']
            c33 = elastic_constants_dict['c33']
            v = -c23/c33
