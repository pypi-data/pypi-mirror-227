...
class LoggedInUsersSerializer(Serializer):
...
class LoggedInUsersViewSet(XLSXFileMixin, GenericViewSet):
    serializer_class = LoggedInUsersSerializer
    since_minutes = None
    ...
    def list(self, request, *args, **kwargs):
        ...
        if self.since_minutes is None:
            return Response(
                ...
                data={
                    "last": "This field is required. Please specify the last number of minutes to check logged in users for"
                },
            )
        try:
    ...
    def get_queryset(self):
        ...
        logged_in_users = get_logged_in_users(since)
        logged_in_users.sort(key=lambda x: x.login_time, reverse=True)
        return logged_in_users

    def get_filename(self, *args, **kwargs):
        return f"logged_in_users-{export_format_datetime()}.xlsx"


def get_logged_in_users(since: datetime) -> List:
    logged_in_users = []
