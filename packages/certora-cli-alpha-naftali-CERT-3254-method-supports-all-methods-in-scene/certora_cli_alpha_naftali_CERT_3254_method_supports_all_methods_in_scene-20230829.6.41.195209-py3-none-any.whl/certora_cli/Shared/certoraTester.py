...
UNDEFINED = "\033[33mundefined\033[0m"
                ...
                if isinstance(expected_rule_result, str):
                    ...
                    for result, func_list in rule_result.items():
                        ...
                        if func_list != expected_res:
                            ...
                            # functions that appear in the expected results but don't exist in the current results.
                            missing_funcs = filter(lambda f: f not in func_list, expected_res)
                            for func_name in missing_funcs:
                                violations.add((test_name, UNDEFINED, result, rule, func_name))
        else:
            result = (rule_result if isinstance(rule_result, str)
                      else "Object{" + ", ".join(rule_result.keys()) + "}")
            violations.add((test_name, result, UNDEFINED,
                            rule, ""))
