...
#### <a name="md-login"></a> login(userid, password, twoFA, vendor_code, mt_secret, imei)
...
from myntapi import app as mt

ret = mt.login(userid=uid, password=pwd, twoFA=factor2, vendor_code=vc, mt_secret=app_key, imei=imei)
```
Request Details :
...
ret = mt.login(userid=user, password=pwd, twoFA=factor2, vendor_code=vc, mt_secret=app_key, imei=imei)
print(ret)
```
