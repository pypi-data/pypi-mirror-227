        ...
        for i0, epsilon, i1, data, weight, original_angle in zip(
            ...
            if i0 == 0 or i1 == 0:
                ...
                for i_diag in range(2):
                    ...
                    if original_angle < self.diagnostic_angles[i_diag] + 3.14 / 10:
                        self.diagnostic_radios[i_diag][:] = 0
                        self.diagnostic_weights[i_diag][:] = 0
                        self.diagnostic_angles[i_diag] = original_angle

                    if abs(original_angle - self.diagnostic_angles[i_diag]) < 3.15 / 10:
...
def _fill_in_chunk_by_shift_crop_data(
    ...
    data_read_precisely_shifted = nd.interpolation.shift(data_read, (-fract_shit, x_shift), order=1, mode="nearest")[:-1]
