...
class LibraryExperiment(object):
    ...
    def write_count_matrix(self, barcode_only=False):
        ...
        # normalized count matrix to counts per million
        normalized_count_mat = self.count_mat
        counts = normalized_count_mat[self.pools].values.astype(float)
        counts = (counts/np.sum(counts, axis=0))*1e6
        normalized_count_mat[self.pools] = counts
        
        self.normalized_count_mat = normalized_count_mat
        ...
        filtered_count_mat = normalized_count_mat
