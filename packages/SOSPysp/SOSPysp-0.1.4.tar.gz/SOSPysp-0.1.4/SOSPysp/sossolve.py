...
def sossolve(sos,options=None,verbose=1):
    ...
    SOLVER_OPT.solver (name of the solver). This can be 'mosek'.
    SOLVER_OPT.params (a structure containing solver-specific parameters)

    The default values for solvers is 'mosek'.
    '''
    # Default options
    if options is None:
        options = {}
    ...
    barvardims = K['s']     # Dimension of each SDP variable
    ...
    # So far, sol is the MOSEK solution of the problem
    x = []
    for item in sol:
        x.extend(item.flatten().tolist())
    sos.solinfo['x'] = x
    sos.solinfo['y'] = y.flatten().tolist()

    # 
    if not sol and info['pinf']:
