...
class Z3solver:
    ...
    def label_circuit(self):
        ...
        for key in self.labeling_graph.gate_dict:
            ...
            self.create_pruned_z3pyscript_approximate(removed_gate)
    ...
    def create_pruned_z3pyscript(self, gates: list):
        self.create_pruned_graph_approximate(gates)
    ...
    def create_pruned_z3pyscript_approximate(self, gates: list):
        self.create_pruned_graph_approximate(gates)
        if self.experiment == SINGLE:
            gate = gates[0]
        # TODO
        ...
        if self.metric == WRE:
            if self.optimization == OPTIMIZE or self.optimization == MAXIMIZE and (self.strategy != BISECTION):
                folder = f'{folder}/{self.approximate_benchmark}_{self.experiment}_{self.metric}_d{self.precision}_{self.strategy}_{self.optimization}'
                os.makedirs(folder, exist_ok=True)
                self.set_z3_report(
                ...
                folder = f'{folder}/{self.approximate_benchmark}_{self.experiment}_{self.metric}_d{self.precision}_{self.strategy}'
                os.makedirs(folder, exist_ok=True)
                self.set_z3_report(
            ...
            if self.optimization == OPTIMIZE or self.optimization == MAXIMIZE and (self.strategy != BISECTION):
                folder = f'{folder}/{self.approximate_benchmark}_{self.experiment}_{self.metric}_{self.strategy}_{self.optimization}'
                os.makedirs(folder, exist_ok=True)
                self.set_z3_report(
                    f'{folder}/{self.approximate_benchmark}_{self.experiment}_{self.metric}_{self.strategy}_{self.optimization}_{gate}.{extension}')
            else:
                folder = f'{folder}/{self.approximate_benchmark}_{self.experiment}_{self.metric}_{self.strategy}'
                os.makedirs(folder, exist_ok=True)
                self.set_z3_report(
    ...
    def create_pruned_graph_approximate(self, gates: list):
