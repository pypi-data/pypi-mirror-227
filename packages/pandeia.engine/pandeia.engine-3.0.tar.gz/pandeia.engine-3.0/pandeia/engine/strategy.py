    ...
    def is_pixel_units(self):
        ...
        if hasattr(self, "units") and 'pixel' == self.units:
            if not self.instrument.telescope.tel_name == "hst":
                raise NotImplementedError(f"Pixel Extraction units are not supported for {self.instrument.telescope.tel_name}")
            is_pixels = True
...
class ImagingScanApPhot(ImagingApPhot):
    """
    Strategy for aperture scan photometry.  This strategy applies to WFC3 Scan Imaging modes.
...
class TASpec(SpecSegmentedPhot):
    ...
    **kwargs: list of keyword/value pairs
    ...
    def __init__(self, instrument, webapp=False, config={}, **kwargs):
        ...
        if instrument.mode == "nuv_specacq": 
            if self.acq_mode == "acq_peakxd":
                ...
                self.combine = True
                self.num_segments = 1
        if instrument.mode == "fuv_specacq":
    ...
    def _make_segment(self, grid, signal, segment, raise_except=False, name="Region"):
        ...
        # in FUV spec acq mode, add acq windows to each segment
        if self.mode == "fuv_specacq" and "acq_windows" in list(segment.keys()):
            ...
            for window in acq_windows:
                ...
                # get the indices of those bounds
                wmin_window_ind = np.searchsorted(signal.wave_pix, wmin_window, side="right")
                wmax_window_ind = np.searchsorted(signal.wave_pix, wmax_window, side="left") - 1
