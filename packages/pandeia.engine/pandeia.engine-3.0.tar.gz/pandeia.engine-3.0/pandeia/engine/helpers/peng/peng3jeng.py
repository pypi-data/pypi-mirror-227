...
IGNORE_KEYS = '[ \"scalar.saturation_time\", \"warnings\", \"scalar.reference_wavelength\"]'
...
def sourceval_parse(adict, calculation, pweb_key):
    ...
    try:
        ...
        if any(key in sky_expr for key in pweb_src_keys):
            ...
            if 'rn(' in sky_expr:
                if pweb_dict['fsorigin'] in ['SpectrumFlat', 'SpectrumNonStellar', 'SpectrumKurucz']:
...
def write_sorted_json(data, fname):
    ...
    # Now check to see if this dir has an ignore_keys.dat file.  If not, write it.
    ...
    if os.path.isfile(dirname + '/ignore_keys.dat') == False:
