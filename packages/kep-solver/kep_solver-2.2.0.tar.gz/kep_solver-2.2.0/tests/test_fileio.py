...
# We read raw JSON and XML entities to compare the outputs from our file IO
# Note: As we need to write XML, we cannot used defusedxml. Do not
...
def test_one_write_json(test_one) -> None:
    # We only test against JSON as only JSON has recipient blood groups
    if test_one[0] != "JSON":
        return
    instance = test_one[1]
