...
class vxUUIDField(vxFieldBase):
    ...
    def __init__(
        ...
        def default_factory():
            return uuid.uuid4().hex if auto else ""

        self._prefix = prefix
        super().__init__(doc, default_factory)

    def convert(self, obj: Any, value: Any) -> Any:
        return f"{self._prefix}_{value}" if self._prefix and value else str(value)
...
class vxDatetimeField(vxFieldBase):
    ...
    def __init__(
        ...
        def formatter(value):
            return datetime.datetime.fromtimestamp(value).strftime(fmt_string)
    ...
    def convert(self, obj: Any, value: Any) -> Any:
        ...
        value = value or self.set_default()
