...
def visualize_3D(points_original, transformed_points_4D, transformed_points_5D):
...
def main():
    ...
    # Generate the 12 edges of the cube
    cube_edges = generate_cube_edges(lower, upper, points_per_edge)
    
    # Transform these points using u1, u2, u3, u4
    transformed_points = [u_to_v(p[0], p[1], p[2], 0.1, 0.8)[:3] for p in cube_edges]
    
    # Add u5 and perform the second transformation
    transformed_points_5D = [u_to_v(p[0], p[1], p[2], 0.1, 0.2)[:3] for p in transformed_points]

    # Visualize the results
    visualize_3D(cube_edges, transformed_points_5D)
