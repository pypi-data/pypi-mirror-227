    ...
    def get_nearest_gas_station(self, state_name: str, lat: float, lon: float) :
        ...
        try:
            query: str = f"""
                            ...
                            FROM GasStation gs 
                            JOIN StoreLocation sl ON sl.gasstation_id = gs.gasstation_id 
                            JOIN Brands b ON b.gasstation_id = gs.gasstation_id 
                            WHERE sl.region = '{state_name}'
                        """
            
            current_state_loc: 'DataFrame' = pd.read_sql(query,con=self.con)
