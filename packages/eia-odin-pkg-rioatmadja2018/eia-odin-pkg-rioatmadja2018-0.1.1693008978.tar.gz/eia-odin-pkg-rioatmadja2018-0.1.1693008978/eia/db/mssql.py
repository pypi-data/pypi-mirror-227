...
class OdinDBMSSQL(object): 
    ...
    def get_historical_gas_station_data(self, gas_station_name: str ):
        ...
        if gasoline_df.shape[0] > 0: 
            ...
            cash_df: 'DataFrame' = pd.read_sql(f"SELECT * FROM CreditPrices WHERE store_id IN ( {store_id})", con=self.con).drop_duplicates() 
            credit_df: 'DataFrame' = pd.read_sql(f"SELECT * FROM CashPrices WHERE store_id IN ({store_id})", con=self.con).drop_duplicates() 
