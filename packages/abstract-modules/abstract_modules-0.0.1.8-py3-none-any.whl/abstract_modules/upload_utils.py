...
def edit_installs(event):
    if event == 'Yes':
        close_window(window=windows_mgr.get_last_window_method())
    if event == "Default":
    ...
    if event == "Choose Edited":
        windows_mgr.update_values(window=windows_mgr.get_last_window_method(),key="-DEFAULT_INSTALLS-",value=windows_mgr.get_values()["-EDIT_INSTALLS-"])
        close_window(window=windows_mgr.get_last_window_method())
def get_install_requires(setup_js,project_dir):
    install_requires_new = get_installed_versions(scan_folder_for_required_modules(folder_path=project_dir,exclude=["setuptools"]))
...
def run_setup_loop(parent_directory:str=os.getcwd()):
    ...
    get_install_requires(setup_js,project_dir)
    get_url(setup_js)
...
upload_main(directory="/home/john-putkey/Documents/modules")
