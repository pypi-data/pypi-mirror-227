...
from emzed_gui.configs import config_for_eic, config_for_fitted_peakshape_model
...
class EicPlottingWidget(CurveWidget):
    ...
    def plot_eics_iter(self, peak_data, models, titles=None, background=True):
        self.clear()
        self.plot.add_item(self.marker)
        if self.range_ is not None:
            self.plot.add_item(self.range_)

        for _ in self.plot.plot_eics(peak_data, models, titles, background):
            yield

        if peak_data and self.range_ is not None:
            _, rtmins, rtmaxs, *_ = zip(*peak_data)
            rtmin = min(rtmins)
            rtmax = max(rtmaxs)
            self.range_.set_range(rtmin, rtmax, block_signals=True)
...
class EicPlot(PositiveValuedCurvePlot, ExtendedCurvePlot):
    ...
    def _init_patched_object(self):
        ...
        self._current_rtmin = self._current_rtmax = None
    ...
    def plot_eics(self, peak_data, models, titles, background):
        ...
        for i, (pm, rtmin, rtmax, mzmin, mzmax) in enumerate(peak_data):
            ...
            rtmaxs.append(rtmax)
            if len(iis):
                iimaxs.append(max(iis))
            else:
                iimaxs.append(1)
                continue

            config = config_for_eic(i)
            if titles:
                title = titles[i]
            else:
                title = ""
            ...
            config = config_for_fitted_peakshape_model(i)
        ...
        self._current_rtmin = min(rtmins)
        self._current_rtmax = max(rtmaxs)
        self._current_iimax = max(iimaxs)

        # TODO: make attribute access vs method call consistent:
