...
def yd_help():
    yd_one_print("共21个功能                        twenty_two(a,b,c,d)为求24点abcd为四个数值中间要加英文逗号,                  yd_zs()求质数括号内求范围,                                      yd_one_print(text,time)文字一个一个输出括号里先填文字（字符串）后面逗号写间隔                            yd_zhengchu(a,b)a能否整除b(没啥用)          yd_pf(a)求a的平方                                                            yd_cf(a,b)求a的b次方                                                yd_abs(a)求a的绝对值                                     yd_d_qz(a)a向下取整                   yd_u_qz(a)a向上取整                yd_sswr(a)四舍五入                                              功能为返回值，加上_p为print出来",0.05)
def yd_zs_p(aaa):
    qqq = 0
