...
class Union(Base):
    ...
    game_type = Column(SmallInteger(), default=0, comment="状态 0.房卡 1.比赛")
    ...
    notice = Column(String(255), comment="公告")
...
def union_list(club_id):
    ...
    try:
        ...
        _select = sqlalchemy.select(Union).options(
            load_only(Union.union_id, Union.game_type, Union.union_status, Union.name, Union.owner_id)) \
            .join(UnionMember, Union.id == UnionMember.union_id).filter(UnionMember.club_id == club_id)
