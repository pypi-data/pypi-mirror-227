...
class Seat(object):
    ...
    def add_ready_timeout(self, room):
        if 0 != room.operation_time:
            if None is not self.ready_timeout:
                ...
                self.ready_timeout = None
            data = pickle.dumps(
                {"timeout_type": TimeoutType.READY, "room_no": room.room_no, "user_id": self.user_id,
                 "current_game_times": room.current_game_times, "into_time": self.into_time}, -1)
    ...
    def add_leave_seat_timeout(self, room):
        if None is not self.leave_seat_timeout:
            ...
            self.leave_seat_timeout = None
        data = pickle.dumps(
            {"timeout_type": TimeoutType.LEAVE_SEAT, "room_no": room.room_no, "user_id": self.user_id}, -1)
