...
from game_base.base.protocol.base import ROOM_REVIEW, ROOM_REVIEW_NOT_EXIST
from game_base.base.protocol.base import ReqRoomReview, RecRoomReview
    ...
    try:
        ...
        if review.gameTimes < _room.current_game_times:
            ...
            if None is not record:
                ...
                users = base_mode.query_values_by(Account, load_only(Account.account_name, Account.nickname), 0,
                                                  tuple([Account.id.in_(users)]))
                rec_view = RecRoomReview()
                for u in users:
                    user = rec_view.users.add()
                    user.id = u.id
                    user.account_name = u.account_name
                ...
                return
        send_to_subscribe(REDIS_SUB_GATEWAY, sid, None, ROOM_REVIEW, None, ROOM_REVIEW_NOT_EXIST)
        return
    except:
