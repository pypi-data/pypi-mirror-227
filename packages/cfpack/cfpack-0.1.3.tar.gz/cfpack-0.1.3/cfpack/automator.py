...
class automator:
    ...
    # drag/move mouse to position of image
    def move_mouse(self, image_file, click=False):
        pos = self.get_mouse_position(image_file) # get position of image
        if pos is not None:
            pag.moveTo(pos[0], pos[1]) # move to position
            if click: pag.click() # click
            return 0
        else:
            return None

    # do a mouse click on image
    def click(self, image_file):
        err = self.move_mouse(image_file, click=True)
        if err is not None: return 0
        else: return None
