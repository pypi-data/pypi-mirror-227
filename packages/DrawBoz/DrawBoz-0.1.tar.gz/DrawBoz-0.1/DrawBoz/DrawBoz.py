...
class DrawBoz:
    ...
    def MigrateInputArrayToCompleteArray(self):
        for text, LineNumber in self.InputArray:
            if 0 <= LineNumber < len(self.CompleteArray):
                self.CompleteArray[LineNumber] = text
    def RenderString(self) -> str:  
        ...
        for i in range(len(self.CompleteArray)):
            ...
            elif isinstance(self.CompleteArray[i], str) and i == self.CompleteArray.index(self.CompleteArray[i]):
                OutputString += f"{BoxClass.UC}{(self.CompleteArray[i]).center(56)}{BoxClass.UC}\n"
