...
def execute_query(query: str, config_file_path: str, database_config_name: str,
    ...
    logging.info('Step 2: Connecting to database using oracledb...')
    dsn = oracledb.makedsn(
        db_config['host'], db_config['port'], service_name=db_config['service_name']
    )
    conn = oracledb.connect(
        user=db_config['username'],
        password=password,  # Use the decrypted password
        dsn=dsn,
        encoding='UTF-8'
    )

    # Execute SQL query using pd.read_sql_query
    logging.info('Step 3: Executing SQL query using pd.read_sql_query...')
    with conn.cursor() as cursor:
        if query_params:
            query_with_params = cursor.mogrify(query, query_params).decode('utf-8')
            result_df = pd.read_sql_query(query_with_params, conn)
        else:
