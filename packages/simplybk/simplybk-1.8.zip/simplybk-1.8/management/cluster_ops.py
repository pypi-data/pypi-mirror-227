...
def deploy_spdk(node_docker):
    ...
    for node in nodes:
        if node.attrs["Name"] == "/spdk":
            logger.info("spdk container found, removing...")
            node.stop()
    ...
    while retries > 0:
        ...
        if not is_running:
            logger.info("SPDK is not running, waiting...")
            time.sleep(3)
            retries -= 1
        else:
            logger.info(f"Spdk status: {status}, Is Running: {is_running}")
            break
...
def join_cluster(cluster_ip, cluster_id, role, ifname, data_nics):  # role: ["management", "storage"]
    ...
    # docker_ip = cluster_ip
    # try:
    #     cluster_docker = docker.DockerClient(base_url=f"tcp://{docker_ip}:2375", version="auto")
    #     if role == 'management':
