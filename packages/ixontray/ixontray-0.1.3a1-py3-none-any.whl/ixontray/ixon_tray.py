...
qsettings = QSettings("martrobotics", "ixontray")
CONFIG_DIR = pathlib.Path(qsettings.fileName()).parent
...
class IxonTray:
    ...
    def run_command(self, command: Command, ixon_id: str | None = None) -> None:
    ...
    def add_menu_items_for_favourites(self, menu: QMenu) -> None:
        for ixon_id, agent in self.favourite_ixon_ids.items():
            ...
            # self.menu.addSeparator()
            # WIP: -)
            # for server in agent.servers or []:

            #     menu_action = sub_menu.addAction(server.name)
            #     xdg_open_command = Command(
            #         name="XDG_open",
            #         icon="web-browser-symbolic",
            #         cmd=f"xdg-open {agent.base_url}{server.link}",
            #         force_connection=False
            #     )
            #     menu_action.triggered.connect(functools.partial(self.run_command,
            #     command=xdg_open_command, ixon_id=ixon_id))
...
def main() -> None:
    ...
    try:
        ...
        print("Running the application failed, maybe yoo are missing the following dependencies")
        print(
            (
                "sudo apt-get -qq install libegl1 libxcb-cursor0 libxcb-icccm4 libxcb-image0 libxcb-keysyms1"
