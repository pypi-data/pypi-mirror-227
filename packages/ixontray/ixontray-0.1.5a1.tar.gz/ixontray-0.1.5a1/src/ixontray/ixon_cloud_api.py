...
class IxonCloudAPI:
    ...
    def get_agents(self, company_id: str) -> AgentsResponse:
        result = self.send_request(
            endpoint="agents",
            params={"fields": "servers.*,name,publicId,description,activeVpnSession.vpnAddress,networkReportedOn"},
            headers={"Api-Company": company_id},
        )
        if result is not None:
            return AgentsResponse.parse_obj(result)
        return None
    ...
    def get_companies(self) -> CompaniesResponse:
        ...
        if result is not None:
            return CompaniesResponse.parse_obj(result)
        return None
...
class IxonCloudAPIv1(IxonCloudAPI):
    ...
    def get_webaccess_url(self, agent: Agent, server: Server) -> str:
        full_url = "webaccess"
...
class IxonCloudAPIv2(IxonCloudAPI):
    ...
    def get_webaccess_url(self, agent: Agent, server: Server) -> str:
        ...
        data = {"method": "http", "server": {"publicId": server.publicId}}
        results = self.send_request(method="POST", data=data, url=full_url)
        print(results)
        return ""
