...
def convert_data_types(df: pd.DataFrame, conversions: List[Union[str, dict]]) -> pd.DataFrame:
    """
        Converts the data types of specified columns in the DataFrame. Possible conversion types:
        datetime, date, int, float, str, boolean
    ...
    Examples:
        Possible conversion entries:
        conversions = ['column1', 'column2']
        conversions = ['column1', {'column2': 'float'}, {'column3': 'bool'}]
        conversions = [{'column1': 'int'}, {'column2': 'float'}, {'column3': 'bool'}, {'column4': 'date'}]

        convert_data_types(df, conversions = conversions)

    Notes:
        - For conversions to 'datetime' and 'date', the function expects the date format to be in '%Y-%m-%d'.
        ...
        for conversion in conversions:
            if isinstance(conversion, str):
                ...
                if target_type == 'datetime':
                    df[column] = pd.to_datetime(df[column], errors='coerce', format='%Y-%m-%d')  # Convert to datetime type
                elif target_type == 'date':
                    df[column] = pd.to_datetime(df[column], errors='coerce', format='%Y-%m-%d').dt.normalize()  # Convert to datetime type set to midnight
                elif target_type == 'int':   
                    df[column] = df[column].astype(float).astype(int)  # Convert to integer type
                elif target_type == 'float':
                    df[column] = df[column].astype(float)  # Convert to float type
                elif target_type == 'str':
                    df[column] = df[column].astype(str)  # Convert to string type
                elif target_type == 'bool':
                    df[column] = df[column].astype(bool)  # Convert to boolean type
                else:
