...
from atscale.errors.atscale_errors import EDAException
...
def pca(dbconn:SQLConnection, data_model:DataModel, pc_num:int, numeric_features:List[str], categorical_features:List[str]) -> Tuple[DataFrame, DataFrame]:
    ...
    base_table_query = 'CREATE TABLE atscale_pca_base_table AS ' + \
                       f'({generate_db_query(data_model=data_model, atscale_query=_generate_atscale_query(data_model=data_model, feature_list=numeric_features + categorical_features))}); '
...
def _get_pca_sql(table_name: str, table_cols: List[str], pc_num: int, platform_type: PlatformType) -> Tuple:
