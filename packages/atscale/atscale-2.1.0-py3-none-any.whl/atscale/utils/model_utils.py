...
from atscale.utils import dimension_utils, measure_utils, project_utils, time_utils
...
def add_data_set_ref(model_dict, dataset_id):
...
def create_dataset_relationship(atconn, project_dict, cube_id, database, schema, table_name, join_features,
                                join_columns=None,
    ...
    return create_dataset_relationship_from_dataset(project_dict=project_dict,
                                                    cube_id=cube_id,
...
def create_dataset_relationship_from_dataset(project_dict: dict,
                                             cube_id: str,
                                             dataset_name: str,
...
def _get_column_type_category(column_type: str) -> str:
    ...
    return type_dict[column_type.lower().split('(')[0]]
    ...
    for column in columns:
        ...
        if column_type == 'date':
            ...
            try:#determine_time_levels depends on count(distinct(column_name)) sql working. If the db errors out, then we just skip
                ...
                logger.warning(f"Unable to determine TimeLevels in create_semantic_layer for column {column} and db type {dbconn.platform_type}. The error was{e}")
...
def _dataset_exists(data_model:DataModel,project_dict:Dict, dataset_name: str) -> bool:
