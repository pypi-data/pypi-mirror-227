...
from atscale.utils import model_utils, metadata_utils,  project_utils, input_utils, db_utils, request_utils, measure_utils, dimension_utils, feature_utils, query_utils
...
class DataModel:
    ...
    def get_features(self, feature_list: List[str] = None, folder_list: List[str] = None,
        ...
        """Gets the feature names and metadata for each feature in the DataModel.
        ...
        Returns:
            dict: A dictionary of dictionaries where the feature names are the keys in the outer dictionary 
                  while the inner keys are the following: 'data type'(value is a level-type, 'Aggregate', or 'Calculated'), 
                  'description', 'expression', caption, 'folder', and 'feature type'(value is Numeric or Categorical).
        """
        level_filter_by = {}
        measure_filter_by = {}
        hier_filter_by = {}
        ...
        if feature_type is FeatureType.ALL or feature_type is FeatureType.CATEGORICAL:
            ...
            for name, info in dimension_dict.items():
                folder = hier_dict[info[Level.hierarchy.name]][Hierarchy.folder.name]
                feature_dict[name] = {'caption':info[Level.caption.name],'data type':info[Level.type.name], 'description':info[Level.description.name],
                                      'expression':'', 'folder':folder, 'feature type':'Categorical'}
        if feature_type is FeatureType.ALL or feature_type is FeatureType.NUMERIC:
            catalog_licensed = self.project.atconn._validate_license('data_catalog_api')
            ...
            for name, info in measure_dict.items():
                feature_dict[name] = {'caption':info[Measure.caption.name],'data type':info[Measure.type.name], 'description':info[Measure.description.name],
                                      'folder':info[Measure.folder.name], 'feature type':'Numeric'}
                if catalog_licensed:
    ...
    def dataset_exists(self, dataset_name: str) -> bool:
        ...
        """
        project_dict = self.__project._get_dict()
        return model_utils._dataset_exists(self, project_dict, dataset_name)
    ...
    def get_column_names(self, dataset_name: str) -> List[str]:
        ...
        if not model_utils._dataset_exists(self, project_dict, dataset_name=dataset_name):
    ...
    def write_feature_importance(self, dbconn: SQLConnection, table_name: str, dataframe: pd.DataFrame, feature_name_prefix: str,
        ...
        project_dataset, dataset_id = project_utils.create_dataset(
            table_name, warehouse_id, column_dict.values, database, schema)

        project_utils.add_dataset(project_dict, project_dataset)
        model_dict = model_utils._get_model_dict(self, project_dict)
        ...
        for i in range(0,1):
            ...
            if dataframe[column].dtype.kind in 'iufc':
                measure_utils.create_measure(
                    project_dict=project_dict, data_model_id=self.id, dataset_id=dataset_id, column_name=column_dict[column],
                    name=feature_name_prefix+'_feature_importance')
            else:
                dimension_utils.create_categorical_dimension_for_column(
                    project_dict=project_dict, data_model_id=self.id, dataset_id=dataset_id, column_name=column_dict[column],
                    name=feature_name_prefix+'_feature_name')
    ...
    def writeback(self, dbconn: SQLConnection, table_name: str, dataframe: pd.DataFrame, join_features: list, join_columns: list = None, roleplay_features: list = None,
        ...
        project_dict = model_utils.create_dataset_relationship(atconn=self.project.atconn, project_dict=project_dict, cube_id=self.cube_id,
    ...
    def writeback_spark(self, dbconn: SQLConnection, pyspark_dataframe, jdbc_format: str, jdbc_options: Dict[str,str],
        ...
        dbconn.write_pysparkdf_to_external_db(pyspark_dataframe= pyspark_dataframe, jdbc_format= jdbc_format, jdbc_options= jdbc_options, 
    ...
    def writeback_spark_to_spark(self, dbconn: SQLConnection, pyspark_dataframe, join_features: list, 
                  ...
                  publish: bool = True, if_exists: PysparkTableExistsActionType = PysparkTableExistsActionType.ERROR):
        """Writes the pyspark dataframe to a table in the database accessed via jdbc with the given table_name. Joins that table to this
        DataModel by joining on the given join_features or join_columns. Only supports hive metastore for now. 
        ...
        dbconn.write_pysparkdf_to_spark_db(pyspark_dataframe= pyspark_dataframe, alt_database_path= alt_database_path, 
                                    table_name=table_name, if_exists=if_exists)
        database, schema = db_utils.get_database_and_schema(dbconn=dbconn)
        atscale_table_name = db_utils.get_atscale_tablename(atconn=self.project.atconn, warehouse_id=warehouse_id, database=database, schema=schema, table_name=table_name)
    ...
    def create_secondary_attribute(self, dataset_name: str, column_name: str, new_attribute_name: str, hierarchy_name: str, level_name: str,
        ...
        if not model_utils._dataset_exists(self, project_json, dataset_name):
            raise atscale_errors.UserError(
        ...
        feature_utils._check_hierarchy(data_model=self, hierarchy_name=hierarchy_name,
    ...
    def create_filter_attribute(self, new_feature_name: str, level_name: str, hierarchy_name: str, filter_values: List[str],
        ...
        if calculated_column_name in list(self.get_features().keys()):
            raise atscale_errors.UserError(
                f'Invalid name: \'{calculated_column_name}\'. A feature already exists with that name')

        feature_utils._check_hierarchy(data_model=self, hierarchy_name=hierarchy_name,
                        level_name=level_name)

        feature_utils._create_filter_attribute(self, project_dict=project_dict, new_feature_name= 
                                        new_feature_name, level_name= level_name, hierarchy_name= hierarchy_name, 
    ...
    def create_mapped_columns(self, dataset_name: str, column_name: str, mapped_names: List[str], data_types: List[MappedColumnDataTypes],
        ...
        model_utils.check_features(features=[column_name],
    ...
    def add_column_mapping(self, dataset_name: str, column_name: str, mapped_name: str, data_type: MappedColumnDataTypes, publish: bool = True):
        ...
        """
        project_dict = self.project._get_dict()
        cube_dict = model_utils._get_model_dict(self, project_dict)[0]
        
        if not model_utils._dataset_exists(self, project_dict, dataset_name):
            raise atscale_errors.UserError(
                f'Dataset \'{dataset_name}\' not associated with given model')
    ...
    def create_calculated_feature(self, new_feature_name: str, expression: str, description: str = None,
        ...
        project_json = self.project._get_dict()
        feature_utils._create_calculated_feature_local(project_json=project_json,
                                        cube_id=self.cube_id,
    ...
    def create_denormalized_categorical_feature(self, dataset_name: str, column_name: str, name: str, description: str = None,
        ...
        dimension_utils.create_categorical_dimension_for_column(project_dict=project_dict, data_model_id=self.id, dataset_id=dataset_id,
    ...
    def create_aggregate_feature(self, dataset_name: str, column_name: str, name: str, aggregation_type: Aggs,
        ...
        feature_utils._create_aggregate_feature_local(project_json= project_json, 
                                        cube_id= self.cube_id, 
                                        dataset_name=dataset_name,
    ...
    def create_rolling_features(self, new_feature_name: str, numeric_feature_name: str, time_length: int, hierarchy_name: str,
        ...
        model_utils.check_features([numeric_feature_name], list(self.get_features().keys()),
    ...
    def create_lag_feature(self, new_feature_name: str, numeric_feature_name: str, time_length: int, hierarchy_name: str, level_name: str,
        ...
        """
        model_utils.check_features(features=[numeric_feature_name],
    ...
    def get_hierarchies(self) -> Dict:
        ...
        """
        return metadata_utils._get_hierarchies(self)
