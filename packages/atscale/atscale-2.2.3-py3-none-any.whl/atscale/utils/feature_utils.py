    ...
    for dataset in project_dsets + cube_dsets:
        for attribute in dataset.get('logical', {}).get('attribute-ref', []):
            if attribute['id'] == column_id:
                ...
                counter = 1
                existing_columns = model_utils._get_column_names(project_dict=project_dict, dataset_name=project_dataset['name'])
...
def _delete_measures_local(data_model, measure_list: List[str],
    ...
    other_cubes = []
    for c in project_parser.get_cubes(project_dict=data_model.project._get_dict()):
...
def _set_dependants(calculated_measure: Dict, dependants_of: Dict[str, List[str]]):
    """ A helper for delete_measures_local to identify dependents of the provided passed parents
...
def _check_unpublished_hierarchy(project_dict: dict, data_model_name: str, hierarchy_name: str, level_name: str, expect_base_input: bool = False, 
    ...
    # check if the hierachy is in the base hierarchies
    ...
    # check if the hierachy is in the non base hierarchies
