...
class SQLAlchemyConnection(SQLConnection):
    ...
    def engine(self, value):
        ...
        logger.warn('Using setters on the database connection object will dispose the engine and will require it to be reset.')
        if not isinstance(value, Engine):
            raise UserError('Value passed is not a sqlalchemy engine')
        self._engine = value
