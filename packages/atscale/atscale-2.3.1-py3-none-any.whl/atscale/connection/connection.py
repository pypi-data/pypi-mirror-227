...
class Connection:
    ...
    def _auth(self):
        ...
        response = self.session.get(url, headers=header, auth=requests.auth.HTTPBasicAuth(self.username, password),
                                    stream=False)
        if response.ok:
    ...
    def get_connected_tables(self, warehouse_id: str, database: str = None, schema: str = None) -> List[str]:
        ...
        Args:
            warehouse_id (str): The atscale warehouse connection to use.
            database (str, optional): The database to use. Defaults to None. When None attempts to use default database
            schema (str, optional): The schema to use. Defaults to None. When None show all tables across schemas
        ...
        if database:
            ...
            logger.warning(
                'No database provided, attempting to return tables of default database. This will not be possible with certain database types')
        ...
        if schema:
            ...
            logger.warning(
                'No schema provided, attempting to return tables of default database. This will not be possible with certain database types')
    ...
    def get_table_columns(self, warehouse_id: str, table_name: str, database: str = None, schema: str = None, 
                          expected_columns: List[str] = None) -> List[Tuple]:
    ...
    def _check_connected(self, err_msg = None):
