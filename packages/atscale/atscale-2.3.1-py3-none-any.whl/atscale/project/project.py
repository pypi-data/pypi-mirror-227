...
class Project:
    ...
    def select_data_model(self, data_model_id: str = None, name_contains: str = None):
        ...
        if data_model_id is None:
            ...
            if name_contains is not None:
                data_model_dict_list = [x for x in data_model_dict_list if name_contains.lower() in x['name'].lower()]
    ...
    def publish(self, new_query_name: str = None, version_tag: str = None) -> str:
        ...
        if new_query_name is None:
            if self.published_project_name != self.project_name:
                ...
                data['modifications'] = {'projectId': self.published_project_id,
                ...
                for table in project_tables:
                    if tables is None or table['name'] in tables:
                        ...
                        if set(server_columns) != set(project_columns):
                            added_columns = project_utils.create_dataset_columns_from_atscale_table_columns(server_columns)
                            columns = project_sql_columns + added_columns
    ...
    def clone(self, clone_name: str, publish: bool = True):
        ...
        """
        self.atconn._check_connected(err_msg= 'Establishing a connection in the atconn field is required')
    ...
    def get_snapshots(self, snapshot_name: str = None):
        ...
        Returns:
            dict: a dict of all snapshot IDs with corresponding names
        """
    ...
    def delete_dataset(self,
        ...
        for index, cube in enumerate(project_parser.get_cubes(project_dict=temp_json)):
            cube_datasets = data_model_parser._get_dataset_refs(
                cube_dict=cube)
            new_cube_datasets = []
