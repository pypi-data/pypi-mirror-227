...
from atscale.base.enums import TimeLevels, PlatformType

logger = logging.getLogger(__name__)

def _generate_calculated_columns_for_date_column(atconn: Connection, data_set_project: dict, column_name: str, time_levels: list) -> dict:
    """
    ...
    """
    platform = atconn._get_warehouse_platform(warehouse_id=data_set_project['physical']['connection']['id'])
    ...
    for level in time_levels:
        ...
        project_utils.add_calculated_column_to_project_dataset(
            atconn=atconn, data_set=data_set_project, column_name=calc_col_name, expression=level.get_sql_expression(column_name, platform_type))
    ...
    if not TimeLevels.Year.name in calculated_columns.keys():
        ...
        project_utils.add_calculated_column_to_project_dataset(
            atconn=atconn, data_set=data_set_project, column_name=year_calc_col_name, expression=level.get_sql_expression(column_name, platform_type))
    ...
    if(any(TimeLevels.Day.index < l.index for l in time_levels)) and not TimeLevels.Day.name in calculated_columns.keys(): # if we have any sub day levels and day is not already in calculated_columns
        ...
        project_utils.add_calculated_column_to_project_dataset(
            atconn=atconn, data_set=data_set_project, column_name=day_calc_col_name, expression=level.get_sql_expression(column_name, platform_type))
    
    return calculated_columns

def create_time_dimension_for_column(atconn: Connection, project_dict: dict, cube_id: str, dataset_id: str, column_name: str, time_levels: List[TimeLevels], base_name: str = None, description: str = None,
    ...
    calculated_columns = _generate_calculated_columns_for_date_column(atconn=atconn, data_set_project=data_set_project, column_name=column_name, time_levels=time_levels)
    ...
    for level in time_levels:  # create a hierarchy level for each of the time_levels
        ...
        new_level = templates.create_hierarchy_level_dict(
            visible=visible, level_id=level_id, keyed_attribute_id=keyed_attribute_id, level_type=level)
    ...
    new_hierarchy = templates.create_hierarchy_dict(hierarchy_id=hierarchy_id, hierarchy_name=f'{base_name}_Hierarchy',
    ...
    new_dimension = templates.create_dimension_dict(
        hierarchy_dict=new_hierarchy, dim_id=dimension_id, name=f'{base_name}_Dimension', visible=visible, time_dimension=True)
...
def create_categorical_dimension_for_column(project_dict: dict, cube_id: str, dataset_id: str, column_name: str, base_name: str = None, description: str = None,
                                            caption: str = None, folder: str = None, visible: bool = True):
    ...
    dimension_id = str(uuid.uuid4())
    new_dimension = templates.create_dimension_dict(
        hierarchy_dict=new_hierarchy, dim_id=dimension_id, name=f'{base_name}_Dimension', visible=visible)
...
def create_time_dimension_from_table(project_dict: dict, cube_id: str, dataset_id: str):
    ...
    # Let's get the dataset from the project so we can add calculated columns to it.
    # Remember there are datasets in project, and data-set-ref in the data model. We will have to modify both.
    dataset = project_parser.get_dataset(
    ...
    for (level, level_type) in time_levels:  # create a hierarchy level for each of the time_levels
        ...
        # Set attribute-key element values. This seems mostly superfluous (but the data_model will break without it) since the visible field also occures in keyed-attribute
        # below which references this. The main thing that changes is number of columns, which I believe is associated with key columns.
        attribute_key_dict = templates.create_attribute_key_dict(key_id=ref_id, columns=len(keys), visible=True)
        project_dict['attributes'].setdefault('attribute-key', [])
        project_dict['attributes']['attribute-key'].append(
            attribute_key_dict)
        ...
        # create the hierarchy level, to be used in creating the hierarchy and dimension below this loop
        level_id = str(uuid.uuid4())
        new_level = templates.create_hierarchy_level_dict(level_id=level_id, keyed_attribute_id=keyed_attribute_id, level_type=level_type, visible=True)
    ...
    hierarchy_id = str(uuid.uuid4())
    new_hierarchy = templates.create_hierarchy_dict(hierarchy_id=hierarchy_id, hierarchy_name=f'date_hierarchy',
                                                    levels=list(levels.values()), caption='date_hierarchy', description='',
                                                    folder='',visible=True)

    dimension_id = str(uuid.uuid4())
    new_dimension = templates.create_dimension_dict(
        hierarchy_dict=new_hierarchy, dim_id=dimension_id, name='date_dimension', time_dimension=True,
        visible=True, participating_datasets=[dataset_id])
    ...
    feature_utils._create_secondary_attribute(cube_id, project_dict, dataset, column_name= month_name, 
            new_attribute_name= 'month_name', hierarchy_name='date_hierarchy', level_name= 'month')
    feature_utils._create_secondary_attribute(cube_id, project_dict, dataset, column_name= day_name, 
            new_attribute_name= 'day_name', hierarchy_name='date_hierarchy', level_name= 'date')
