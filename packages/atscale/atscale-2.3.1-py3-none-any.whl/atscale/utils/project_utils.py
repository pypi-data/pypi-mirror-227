...
def clone_project_dict(atconn: Connection, original_project_id: str, new_project_name: str) -> dict:
    ...
    response = atconn._submit_request(
        request_type=RequestType.GET, url=f'{url}')
...
def add_dataset(project_dict: dict, dataset: dict):
...
def create_dataset(table_name: str, warehouse_id: str, table_columns: list, database: str = None, schema: str = None, dataset_name: str = None):
    ...
    dataset = templates.create_dataset_dict(dataset_id=dataset_id, dataset_name=dataset_name,
                                            table_name= table_name, warehouse_id=warehouse_id, 
                                            columns=columns, schema=schema, database=database)
    return dataset, dataset_id

def create_query_dataset(name: str,
                         query: str,
    ...
    return templates.create_query_dataset_dict(dataset_id= str(uuid.uuid4()),
                            dataset_name= name, warehouse_id= warehouse_id,
                            columns= column_dict_list, allow_aggregates= allow_aggregates,
                            query= query)
...
def add_calculated_column_to_project_dataset(atconn: Connection, data_set: dict, column_name: str, expression: str):
    ...
    url = endpoints._endpoint_expression_eval(atconn, 
...
def _check_if_qds(data_set: dict) -> bool:
    ...
    """
    return data_set['physical'].get('queries', None) is not None
