...
from atscale.utils.metadata_utils import _get_all_numeric_feature_names, _get_all_categorical_feature_names
...
def linear_regression(dbconn:SQLConnection, 
                      data_model:DataModel, 
                      predictors:List[str], 
                      prediction_target:str, 
                      granularity_levels:List[str],
                      if_exists:PandasTableExistsActionType=PandasTableExistsActionType.FAIL) -> Dict:
    ...
    Args:
        ...
        predictors (List[str]): The numeric features corresponding to the regression inputs
        prediction_target (str): The numeric feature that will be predicted via linear_regression
    ...
    if if_exists == PandasTableExistsActionType.APPEND:        
        raise UserError(f'The ActionType of APPEND is not valid for this function, only REPLACE AND FAIL are valid.')
    ...
    _check_features(predictors + [prediction_target] + granularity_levels, all_numeric_features + all_categorical_features)

    ### Check that numeric/categorical features are in correct categories
    _check_features(predictors + [prediction_target], all_numeric_features, errmsg='Make sure predictors and/or prediction_target consist(s) only of numeric features')
    _check_features(granularity_levels, all_categorical_features, errmsg='Make sure granularity_levels consists only of categorical features')
    ...
    base_table_atscale_query = generate_db_query(data_model=data_model, 
                atscale_query=_generate_atscale_query(data_model=data_model,
    ...
    try:
        ...
        if 'already exists.' in err_msg:
            ...
            if if_exists == PandasTableExistsActionType.FAIL:
                raise UserError(f'A table already exists with name: {table_name}. Name collisions between runs are rare '\
    ...
    query_statements_a, query_statements_b, drop_statements_a, drop_statements_b, display_statement = _get_linear_regression_sql(
                                                                                                            base_table_name, 
                                                                                                            predictors, 
                                                                                                            prediction_target, 
                                                                                                            dbconn.platform_type)
    ...
    has_linearly_dependent_columns = dbconn.submit_query(f'SELECT lin_dep_count FROM {base_table_name}_dependency_check; ')['lin_dep_count'][0] > 0
...
def _get_linear_regression_sql(table_name:str, 
                               predictors:List[str], 
                               prediction_target:str, 
                               platform_type:PlatformType) -> Tuple:
    ...
    Args:
        ...
        predictors (List[str]): The numeric features corresponding to the regression inputs
        prediction_target (str): The numeric feature that will be predicted via linear_regression
        platform_type (PlatformType): The type of warehouse connection passed to the orchestrator function
    ...
    # Make Snowflake columns readable, capitalize drop statements
    ...
    # the constant term
    predictor_observation_matrix_string = f'CREATE TABLE {table_name}_predictor_observation_matrix AS SELECT '
    for col in predictors:
        predictor_observation_matrix_string += f'{table_name}.{col} AS {col}, '
    predictor_observation_matrix_string += '1.0 AS constant, ' + \
    ...
    predictor_transpose_times_predictor_string = f'CREATE TABLE {table_name}_predictor_transpose_times_predictor AS '
    ...
    for i in range(1, (predictor_plus_constant_dim ** 2) + 1):
        ...
        predictor_transpose_times_predictor_string += f'SELECT {i} AS id, {r} AS r, {c} AS c, ' + \
                                                      f'SUM({col_list[r - 1]} * {col_list[c - 1]}) AS vals FROM {table_name}_predictor_observation_matrix ' + \
                                                      f'UNION ALL '
    predictor_transpose_times_predictor_string = predictor_transpose_times_predictor_string[:-11] + '; '
    query_statements_a.append(predictor_transpose_times_predictor_string)

    # Check for linearly dependent tables
    dependency_check_string = f'CREATE TABLE {table_name}_dependency_check AS (SELECT COUNT(sd) AS lin_dep_count FROM ('
    for i in range(1, predictor_plus_constant_dim + 1):
        for j in range(i + 1, predictor_plus_constant_dim + 1):
            dependency_check_string += f'SELECT STDDEV(vals_{i} / vals_{j}) AS sd FROM (' + \
    ...
    query_statements_b.append(f'CREATE TABLE {table_name}_q AS (SELECT {table_name}_predictor_transpose_times_predictor.id, ' + \
    ...
    query_statements_b.append(f'CREATE TABLE {table_name}_r AS (SELECT {table_name}_predictor_transpose_times_predictor.id, ' + \
                            f'{table_name}_predictor_transpose_times_predictor.r, ' + \
                            f'{table_name}_predictor_transpose_times_predictor.c, ' + \
                            f'({table_name}_predictor_transpose_times_predictor.vals * 0.0) AS vals ' + \
                            f'FROM {table_name}_predictor_transpose_times_predictor); ')
    query_statements_b.append(f'CREATE TABLE {table_name}_v AS (SELECT * FROM {table_name}_predictor_transpose_times_predictor); ')
    ...
    for i in range(1, predictor_plus_constant_dim + 1):
        query_statements_b.append(f'UPDATE {table_name}_q SET vals = q{i}.vals FROM ' + \
        ...
        query_statements_b.append(f'UPDATE {table_name}_r SET vals = (SELECT SQRT(SUM(POWER(vals, 2))) FROM {table_name}_v ' + \
        ...
        for j in range(i + 1, predictor_plus_constant_dim + 1):
            query_statements_b.append(f'UPDATE {table_name}_r SET vals = (SELECT SUM(q{i}.vals * v{j}.vals) FROM ' + \
            ...
            query_statements_b.append(f'UPDATE {table_name}_v SET vals = v_minus.vals FROM ' + \
    ...
    predictor_transpose_times_observation_string = f'CREATE TABLE {table_name}_predictor_transpose_times_observation AS '
    for i in range(1, predictor_plus_constant_dim):
        predictor_transpose_times_observation_string += f'SELECT {i} AS id, SUM({predictors[i - 1]} * {prediction_target}) ' + \
                                                        f'AS vals FROM {table_name}_predictor_observation_matrix UNION ALL '
    predictor_transpose_times_observation_string += f'SELECT {predictor_plus_constant_dim} AS id, SUM(constant * {prediction_target}) ' + \
    ...
    for i in range(1, (predictor_plus_constant_dim ** 2) + 1):
        ...
        c = i - (r - 1) * predictor_plus_constant_dim
        if r == c:
            # Differentiating Snowflake here since constructing the table this way defaults the column to
            # wrong data type (same below)
            if platform_type == PlatformType.SNOWFLAKE:
                r_inverse_init_string += f'SELECT {i} AS id, {r} AS r, {c} AS c, 1.0::FLOAT AS vals UNION ALL '
            else:
                r_inverse_init_string += f'SELECT {i} AS id, {r} AS r, {c} AS c, 1.0 AS vals UNION ALL '
        else:
            if platform_type == PlatformType.SNOWFLAKE:
                r_inverse_init_string += f'SELECT {i} AS id, {r} AS r, {c} AS c, 0.0::FLOAT AS vals UNION ALL '
            else:
                r_inverse_init_string += f'SELECT {i} AS id, {r} AS r, {c} AS c, 0.0 AS vals UNION ALL '
    ...
    for i in range(predictor_plus_constant_dim, 0, -1):
        ...
        if i > 1:
            r_inverse_string = f'UPDATE {table_name}_r_inverse SET vals = row_update.vals FROM ('
            for j in range(1, i):
                r_inverse_string += 'SELECT id, (vals - sub_vals) AS vals FROM (' + \
            ...
            r_inverse_string = r_inverse_string[:-11] + f') AS row_update WHERE {table_name}_r_inverse.id = row_update.id; '
            query_statements_b.append(r_inverse_string)
    ...
    for i in range(1, (predictor_plus_constant_dim ** 2) + 1):
        r = (i - 1) // predictor_plus_constant_dim + 1
        c = i - (r - 1) * predictor_plus_constant_dim
        q_transpose_string += f'SELECT {i} AS id, {r} AS r, {c} AS c, ' + \
    ...
    for i in range(1, predictor_plus_constant_dim + 1):
        intermed_string += f'SELECT {i} AS id, SUM(col_prods.vals) AS vals FROM ' + \
    ...
    for i in range(1, predictor_plus_constant_dim + 1):
        coeff_string += f'SELECT {i} AS id, SUM(col_prods.vals) AS vals FROM ' + \
                        f'(SELECT id, r, c, (vector_vals * matrix_vals) AS vals FROM ' + \
    ...
    for i in range(predictor_plus_constant_dim - 1):
        display_statement += f'SELECT \'{predictors[i]}\' AS coefficient, ' + \
                             f'(SELECT vals FROM {table_name}_coeff WHERE id = {i + 1}) AS value UNION ALL '
    display_statement += f'SELECT \'<constant>\' AS coefficient, ' + \
                         f'(SELECT vals FROM {table_name}_coeff WHERE id = {predictor_plus_constant_dim}) AS value; '

    return query_statements_a, query_statements_b, drop_statements_a, drop_statements_b, display_statement
