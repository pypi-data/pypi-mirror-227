...
from atscale.base.enums import Hierarchy, Level, Measure, TimeSteps, FeatureType
...
from atscale.utils.feature_utils import _check_time_hierarchy, _create_calculated_feature_local
from atscale.utils.model_utils import _check_features, _check_conflicts, _perspective_check


def create_one_hot_encoded_features(data_model, categorical_feature: str, hierarchy_name: str = None,
                                    description: str = None, folder: str = None, format_string: str = None,
    ...
    Args:
        ...
        hierarchy_name (str, optional): The name of the hierarchy to use for the feature. Only necessary if the feature is duplicated in multiple hierarchies.
    ...
    original_proj_dict = copy.deepcopy(project_json)  # need to check that the new names were free BEFORE adding them
    created_names = []
    for value in df_values[categorical_feature].values:
        ...
        _create_calculated_feature_local(project_json, data_model.cube_id, name, expression, description=description,
                                         caption=None,
                                         folder=folder,
                                         format_string=format_string)

    _check_conflicts(to_add=created_names,
                     data_model=data_model,
                     project_dict=original_proj_dict)
    data_model.project._update_project(
        project_json=project_json, publish=publish)
    return created_names

def create_percent_change(data_model, new_feature_name: str, numeric_feature_name: str, time_length: int, hierarchy_name: str,
    ...
    numerics = data_model.get_features(use_published=False,
                                       feature_type=FeatureType.NUMERIC)
    _check_features(features=[numeric_feature_name],
                    check_list=numerics,
    ...
    data_model.create_calculated_feature(new_feature_name, expression, description=description, caption=caption, folder=folder,
                              format_string=format_string, visible=visible, publish=publish)

def create_period_to_date(data_model, new_feature_name: str, numeric_feature_name: str, hierarchy_name: str, level_name: str,
    ...
    existing_measures = dparse.filter_dict(to_filter=existing_features,
    ...
    _check_conflicts(to_add=new_feature_name,
...
def create_pct_error_calculation(data_model, new_feature_name: str, predicted_feature_name: str, actual_feature_name: str,
                                 description: str = None, caption: str = None, folder: str = None, format_string: str = None,
                                 visible: bool = True, publish: bool = True):
    ...
    _check_features([predicted_feature_name], numerics,
                    f"Make sure '{predicted_feature_name}' is a numeric feature")
    _check_features([actual_feature_name], numerics,
...
def create_scaled_feature_minmax(data_model, new_feature_name: str, numeric_feature_name: str, min: float, max: float, feature_min: float = 0,
...
def create_scaled_feature_z_score(data_model, new_feature_name: str, numeric_feature_name: str, mean: float = 0, standard_deviation: float = 1,
                                  description: str = None, caption: str = None, folder: str = None, format_string: str = None,
                                  visible: bool = True, publish: bool = True):
...
def create_scaled_feature_maxabs(data_model, new_feature_name: str, numeric_feature_name: str, maxabs: float, description: str = None, caption: str = None,
...
def create_scaled_feature_robust(data_model, new_feature_name: str, numeric_feature_name: str, median: float = 0, interquartile_range: float = 1,
                                 description: str = None, caption: str = None, folder: str = None, format_string: str = None,
                                 visible: bool = True, publish: bool = True):
...
def create_scaled_feature_log_transformed(data_model, new_feature_name: str, numeric_feature_name: str, description: str = None, caption: str = None,
                                          folder: str = None, format_string: str = None, visible: bool = True, publish: bool = True):
...
def create_scaled_feature_unit_vector_norm(data_model, new_feature_name: str, numeric_feature_name: str, magnitude: float, description: str = None,
...
def create_scaled_feature_power_transformed(data_model, new_feature_name: str, numeric_feature_name: str, power: float, method: str = 'yeo-johnson',
                                            description: str = None, caption: str = None, folder: str = None, format_string: str = None,
                                            visible: bool = True, publish: bool = True):
...
def create_net_error_calculation(data_model, new_feature_name: str, predicted_feature_name: str, actual_feature_name: str,
                                 description: str = None, caption: str = None, folder: str = None, format_string: str = None,
                                 visible: bool = True, publish: bool = True):
    ...
    measure_list = dmh._get_unpublished_features(project_dict=project_dict,
                                                 data_model_name=data_model.name,
                                                 feature_type=FeatureType.NUMERIC)
    _check_features([predicted_feature_name], measure_list,
                    f'Invalid parameter value \'{predicted_feature_name}\' '
                    f'is not a numeric feature in the data model')
    _check_features([actual_feature_name], measure_list,
                    f'Invalid parameter value \'{actual_feature_name}\' '
                    f'is not a numeric feature in the data model')
    level_list = dmh._get_unpublished_features(project_dict=project_dict,
                                               data_model_name=data_model.name,
    ...
    _create_calculated_feature_local(project_dict=project_dict,
                                     cube_id=data_model.cube_id,
...
def create_binned_feature(data_model, new_feature_name: str, numeric_feature_name: str, bin_edges: List[float], description: str = None,
                          caption: str = None, folder: str = None, format_string: str = None, visible: bool = True,
                          publish: bool = True):
...
def generate_time_series_features(data_model, dataframe, numeric_features: List[str], time_hierarchy: str, level: str, group_features: List[str] = None,
                                  intervals: List[int] = None, shift_amount: int = 0):
    ...
    """
    hierarchy_dict, level_dict = _check_time_hierarchy(data_model=data_model,
                                                       hierarchy_name=time_hierarchy, level_name=level)

    measure_list = data_model.get_features(use_published=True,
                                           feature_type=FeatureType.NUMERIC)
    categorical_list = data_model.get_features(use_published=True,
                                               feature_type=FeatureType.CATEGORICAL)
        ...
        for heir_level in reversed(levels):
            if found and heir_level in dataframe.columns:
                ...
                dataframe[name] = dataframe.groupby(
                    group_features + [heir_level])[feature].cumsum().shift(shift_amount).reset_index(drop=True)
            if heir_level == level:
                found = True
