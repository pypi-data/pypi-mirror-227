...
class AtScale:
    ...
    def _parse_measures(self):
        ...
        for measure in measure_rows:
            ...
            if folder:
                ...
                measure_dict['folder'] = ''
            #todo: currently expression doesn't get returned in DMV query
        ...
        # add calculated measure expression from project
        if 'calculated-members' in self.project_json and 'calculated-member' in self.project_json['calculated-members']:
            for calculated_measure in self.project_json['calculated-members']['calculated-member']:
                if calculated_measure['name'] in self._measure_dict:
                    self._measure_dict[calculated_measure['name']]['expression'] = calculated_measure['expression']
    ...
    def create_one_hot_encoded_feature(self, categorical_feature, hierarchy, description=None, folder=None,
        ...
        for value in df_values[categorical_feature].values:
            expression = f'IIF([{dimension}].[{hierarchy}].[{categorical_feature}].CurrentMember.Name="{value}",1,0)'
    ...
    def create_db_connection(self, db: Database):
        ...
        atscale_connection_group = self._check_single_connection(db.atscale_connection_id)

        db.validate(atscale_connection_group)
    ...
    def add_table(self, table_name, dataframe, join_features, join_columns=None, roleplay_features=None,
        ...
        #check_multiple_features(join_columns, dataframe.columns,
        #                              errmsg='Make sure all items in join_features are in the dataframe')

        self.database.add_table(table_name, dataframe, chunksize, if_exists=if_exists)

        table_name = self.database.fix_table_name(table_name)
        #todo: tests case: try appending a df with an extra column
        #todo: make sure db implementations fail when theres an extra column
        #todo: look into if databases allow appending columns and rows

        if if_exists == 'replace':
            self.update_project_tables([table_name], False)#...

        self.join_table(table_name, join_features, join_columns, roleplay_features,
        ...
        for join_feature, join_column, roleplay_feature in joins:
            ...
            for col in join_column:
                if col in column_names:
                    cols.append(col)
                elif col.upper() in column_names:
                    cols.append(col.upper())
                elif col.lower() in column_names:
    ...
    def generate_db_query(self, atscale_query):
        ...
        self.custom_query(inbound_query, dryRun=True)
