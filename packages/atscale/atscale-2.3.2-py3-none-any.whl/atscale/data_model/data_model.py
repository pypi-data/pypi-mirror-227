...
from atscale.utils import model_utils, metadata_utils, project_utils, input_utils, db_utils, request_utils, \
    dimension_utils, feature_utils, query_utils, validation_utils, bulk_operator_utils
from atscale.utils.dmv_utils import get_dmv_data
from atscale.base.enums import PandasTableExistsActionType, PysparkTableExistsActionType, Measure, Level, Hierarchy, \
    FeatureType, MappedColumnDataTypes
...
class DataModel:
    ...
    def get_features(self, feature_list: List[str] = None, folder_list: List[str] = None,
        ...
        Args:
            feature_list (List[str], optional): A list of features to return. Defaults to None to return all. All
        ...
        if use_published:
            ret_dict = data_model_helpers._get_published_features(self, feature_list=feature_list,
                                                                  folder_list=folder_list,
                                                                  feature_type=feature_type)
        else:
            model_utils._perspective_check(
                self, "Getting unpublished features is not supported for perspectives."
            )
            project_dict = self.project._get_dict()
            ret_dict = data_model_helpers._get_unpublished_features(project_dict, data_model_name=self.name,
        ...
        if isinstance(feature_list, list):
            model_utils._check_features(feature_list, ret_dict, errmsg='The requested features were not found: {}')
        return ret_dict
    ...
    def _get_referenced_project_datasets(self) -> List[dict]:
        ...
        return model_utils._get_fact_datasets(data_model_dict= data_model_dict,
    ...
    def get_fact_dataset_names(self) -> List[str]:
        ...
        project_dict = self.__project._get_dict()
        data_model_dict = model_utils._get_model_dict(self, project_dict= project_dict)[0]
        return [ds['name'] for ds in model_utils._get_fact_datasets(
    ...
    def get_dataset_names(self) -> List[str]:
        ...
        names = {dset['name'] for dset in fact_dsets + dimension_dsets}
    ...
    def delete_measures(self, measure_list: List[str], publish: bool = True, delete_children: bool = None):
        ...
        existing = data_model_helpers._get_unpublished_features(project_dict=json_dict,
                                                                data_model_name=self.name,
                                                                feature_type=FeatureType.NUMERIC)
        model_utils._check_features(features=measure_list,
                                    check_list=existing)
    ...
    def add_query_dataset(self, dataset_name: str, query: str,
        ...
        warehouse_id = validation_utils._validate_warehouse_id_parameter(project=self.project,
                                                                         project_dict=project_dict,
                                                                         warehouse_id=warehouse_id)

        # check if there is an existing dataset with the given name
        existing_dset = project_parser.get_dataset(project_dict=project_dict, dataset_name=dataset_name)
        ...
        qds_dict = project_utils.create_query_dataset(name=dataset_name,
                                                        query=query,
                                                        columns=columns,
                                                        warehouse_id=warehouse_id,
        ...
        if create_hinted_aggregate:
            model_utils._create_hinted_agg_for_qds(project_dict=project_dict,
                                                   cube_id=self.cube_id,
                                                   dataset_name=dataset_name)
    ...
    def write_feature_importance(self, dbconn: SQLConnection, table_name: str, dataframe: pd.DataFrame,
        ...
        atscale_table_name = db_utils.get_atscale_tablename(atconn=self.project.atconn, warehouse_id=warehouse_id,
        ...
        column_dict = db_utils.get_column_dict(atconn=self.project.atconn, dbconn=dbconn,
                                               warehouse_id=warehouse_id, atscale_table_name=atscale_table_name,
                                               dataframe_columns=dataframe.columns)
        # create a dataset for the new table
        columns = self.project.atconn.get_table_columns(
            warehouse_id=warehouse_id, table_name=atscale_table_name, database=database, schema=schema, expected_columns=dataframe.columns)
    ...
    def writeback(self, dbconn: SQLConnection, table_name: str, dataframe: pd.DataFrame, join_features: list,
                  join_columns: list = None, roleplay_features: list = None,
        ...
        df = dataframe.copy()  # mapping value column to key column may mutate df

        join_features, join_columns, roleplay_features, df = data_model_helpers._check_joins(
            project_dict=project_dict,
            cube_id=self.cube_id,
            join_features=join_features,
            join_columns=join_columns,
        ...
        atscale_columns = [c[0] for c in self.project.atconn.get_table_columns(warehouse_id=warehouse_id,
                                                              table_name=atscale_table_name,
                                                              database=database,
                                                              schema=schema, 
                                                              expected_columns=df.columns)]
        join_columns: List[List[str]] = data_model_helpers._prep_join_columns_for_join(join_columns=join_columns,
                                                                                       atscale_columns=atscale_columns,)

        # create_dataset_relationship now mutates the project_json and returns, then we're responsible for posting
        project_dict = model_utils._create_dataset_relationship(atconn=self.project.atconn, project_dict=project_dict,
                                                                cube_id=self.cube_id,
    ...
    def add_table(self,
        ...
        Args:
            ...
            join_features (List[str]): The features in the data model to join on. Defaults to None to create no joins.
            join_columns (list, optional): The columns in the dataframe to join to the join_features. List must be either
                None or the same length and order as join_features. Defaults to None to use identical names to the
                join_features. If multiple columns are needed for a single join they should be in a nested list
        ...
        """
        model_utils._perspective_check(self)
        self.project._update_project_tables(publish=False)
        project_dict = self.project._get_dict()
        warehouse_id = validation_utils._validate_warehouse_id_parameter(project=self.project,
                                                                         project_dict=project_dict,
                                                                         warehouse_id=warehouse_id)
        atscale_table_name = db_utils.get_atscale_tablename(
            atconn=self.project.atconn,
            warehouse_id=warehouse_id,
            database=database,
            schema=schema,
            table_name=table_name)
        # tuples of (column_name, column_dtype)
        table_columns = self.project.atconn.get_table_columns(warehouse_id=warehouse_id,
                                                              database=database,
                                                              schema=schema,
                                                              table_name=atscale_table_name)
        column_names = {column[0] for column in table_columns}
        join_features, join_columns, roleplay_features, _ = data_model_helpers._check_joins(project_dict=project_dict,
                                                                                            cube_id=self.cube_id,
                                                                                            join_features=join_features,
                                                                                            join_columns=join_columns,
                                                                                            roleplay_features=roleplay_features,
    ...
    def create_dataset_relationship(self,
                                    dataset_name: str,
        ...
        self.project._update_project_tables(publish=False)  # update the project tables to get the latest columns
        ...
        dataset_columns = model_utils._get_column_names(project_dict=original_project_dict,
                                                        dataset_name=dataset_name)
        column_set = set(dataset_columns)
        join_columns: List[List[str]] = data_model_helpers._prep_join_columns_for_join(join_columns=join_columns,
                                                                                       atscale_columns=column_set)
        ...
        if not project_parser.get_dataset(project_dict=original_project_dict, dataset_name=dataset_name):
            raise atscale_errors.UserError(
                f'No dataset with the name {dataset_name} found in project')
        project_dict = model_utils._create_dataset_relationship_from_dataset(project_dict=original_project_dict,
                                                                             cube_id=self.cube_id,
                                                                             dataset_name=dataset_name,
                                                                             join_features=join_features,
                                                                             join_columns=join_columns,
    ...
    def get_data(self, feature_list: List[str], filter_equals: Dict[str, str] = None,
                 ...
                 limit: int = None, comment: str = None,
                 use_aggs: bool = True, gen_aggs: bool = True,
        ...
        Args:
            ...
            order_by (List[Tuple[str, str]]): The sort order for the returned dataframe. Accepts a list of tuples of the
                feature name and ordering respectively: [('feature_name_1', 'DESC'), ('feature_2', 'ASC') ...].
    ...
    def get_data_direct(self, dbconn: SQLConnection, feature_list, filter_equals=None, filter_greater=None,
                        filter_less=None,
                        ...
                        order_by: List[Tuple[str, str]] = None,
                        limit=None, comment=None, use_aggs=True, gen_aggs=True) -> pd.DataFrame:
    ...
    def get_data_spark(self, feature_list: List[str], sparkSession, jdbc_format: str, jdbc_options: Dict[str, str],
                       ...
                       filter_null: List[str] = None, filter_not_null: List[str] = None,
                       order_by: List[Tuple[str, str]] = None,
                       limit: int = None,
        ...
        query = query_utils.generate_db_query(self, query_utils._generate_atscale_query(data_model=self,
                                                                                        ...
                                                                                        limit=limit,
                                                                                        comment=comment),
                                              use_aggs=use_aggs, gen_aggs=gen_aggs)
        ...
        our_warehouse = \
            [warehouse for warehouse in connected_warehouses if warehouse['warehouse_id'] == warehouse_in_use][0]

        if our_warehouse['platform'] in ['databrickssql', 'databricks']:
            raise atscale_errors.UserError(
                'This function is not currently supported for Databricks Warehouses due to ' /
    ...
    def get_database_query(self, feature_list: List[str],
                           filter_equals: Dict[str, Any] = None, filter_greater: Dict[str, Any] = None,
                           ...
                           order_by: List[Tuple[str, str]] = None,
                           limit: int = None,
                           comment: str = None, use_aggs: bool = True, gen_aggs: bool = True) -> str:
        ...
        return query_utils.generate_db_query(data_model=self,
                                             atscale_query=query_utils._generate_atscale_query(
                                                 data_model=self,
                                                 feature_list=feature_list, filter_equals=filter_equals,
                                                 filter_greater=filter_greater,
                                                 filter_less=filter_less,
                                                 filter_greater_or_equal=filter_greater_or_equal,
                                                 filter_less_or_equal=filter_less_or_equal,
                                                 filter_not_equal=filter_not_equal,
                                                 filter_in=filter_in, filter_between=filter_between,
                                                 filter_like=filter_like,
                                                 filter_rlike=filter_rlike, filter_null=filter_null,
                                                 filter_not_null=filter_not_null,
                                                 order_by=order_by,
                                                 limit=limit, comment=comment), use_aggs=use_aggs, gen_aggs=gen_aggs)
    ...
    def writeback_spark(self, dbconn: SQLConnection, pyspark_dataframe, jdbc_format: str, jdbc_options: Dict[str, str],
        ...
        join_features, join_columns, roleplay_features, pyspark_dataframe = data_model_helpers._check_joins(
            project_dict=project_dict,
            cube_id=self.cube_id,
            join_features=join_features,
            join_columns=join_columns,
        ...
        dbconn._write_pysparkdf_to_external_db(pyspark_dataframe= pyspark_dataframe, jdbc_format=jdbc_format,
                                               jdbc_options=jdbc_options, table_name=table_name, if_exists=if_exists)
        database, schema = db_utils.get_database_and_schema(dbconn=dbconn)
        atscale_table_name = db_utils.get_atscale_tablename(atconn=self.project.atconn, warehouse_id=warehouse_id,
                                                            database=database, schema=schema, table_name=table_name)
    ...
    def writeback_spark_to_spark(self,
        ...
        column_dict = db_utils.get_column_dict(atconn=self.project.atconn, dbconn=dbconn,
                                               warehouse_id=warehouse_id, atscale_table_name=atscale_table_name,
                                               dataframe_columns=pyspark_dataframe.columns)
    ...
    def create_secondary_attribute(self, dataset_name: str, column_name: str, new_attribute_name: str, hierarchy_name: str, level_name: str,
        ...
        model_utils._check_conflicts(data_model=self,
                                     to_add=new_attribute_name,
        ...
        feature_utils._check_unpublished_hierarchy(project_json, self.name, hierarchy_name, level_name,
                                                   expect_base_input=True)

        data_set = project_parser.get_dataset(project_dict=project_json, dataset_name=dataset_name)

        # call the helper
        feature_utils._create_secondary_attribute(self.cube_id, project_json, data_set, column_name=column_name,
                                                  new_attribute_name=new_attribute_name, hierarchy_name=hierarchy_name,
    ...
    def create_filter_attribute(self, new_feature_name: str, level_name: str, hierarchy_name: str, filter_values: List[str],
        ...
        model_utils._check_conflicts(to_add=new_feature_name,
                                     data_model=self,
                                     project_dict=project_dict)

        feature_utils._check_unpublished_hierarchy(project_dict, self.name, hierarchy_name, level_name,
                                                   expect_base_input=True)
    ...
    def create_mapped_columns(self, dataset_name: str, column_name: str, mapped_names: List[str],
                              ...
                              map_key_type: MappedColumnDataTypes, map_value_type: MappedColumnDataTypes,
    ...
    def create_calculated_feature(self, new_feature_name: str, expression: str, description: str = None,
        ...
        model_utils._validate_mdx_syntax(self.project.atconn, expression)
    ...
    def update_calculated_feature(self, feature_name: str, expression: str = None, description: str = None, caption: str = None,
        ...
        project_dict = self.project._get_dict()
        calculated_measures = data_model_helpers._parse_calculated_features(data_model_name=self.name,
                                                                            project_dict=project_dict)
        ...
        feature_utils._update_calculated_feature(project_dict=project_dict, feature_name=feature_name,
                                                 expression=expression,
    ...
    def create_denormalized_categorical_feature(self, fact_dataset_name: str, column_name: str, name: str, description: str = None,
        ...
        project_dict = self.project._get_dict()
        model_utils._check_conflicts(to_add=name,
                                     data_model=self,
                                     project_dict=project_dict)
        data_model_dict = model_utils._get_model_dict(self, project_dict = project_dict)[0]
        if not model_utils._get_fact_dataset(data_model_dict, project_dict, dataset_name=fact_dataset_name):
            raise atscale_errors.UserError(
        ...
        data_set_project = project_parser.get_dataset(project_dict=project_dict, dataset_name=fact_dataset_name)
    ...
    def create_aggregate_feature(self, fact_dataset_name: str, column_name: str, name: str, aggregation_type: Aggs,
        ...
        data_model_dict = model_utils._get_model_dict(self, project_dict = project_dict)[0]
        dset = model_utils._get_fact_dataset(data_model_dict, project_dict, dataset_name=fact_dataset_name)
        if not dset:
            raise atscale_errors.UserError(
                f'Fact Dataset \'{fact_dataset_name}\' not associated with given model')
    ...
    def update_aggregate_feature(self, feature_name: str, description: str = None, caption: str = None,
        ...
        project_json = self.project._get_dict()
        agg_features = data_model_helpers._parse_aggregate_features(data_model_name=self.name,
                                                                    project_dict=project_json)
    ...
    def create_rolling_features(self, new_feature_name: str, numeric_feature_name: str, time_length: int, hierarchy_name: str,
        ...
        project_json = self.project._get_dict()
        measure_list = data_model_helpers._get_unpublished_features(project_dict=project_json,
                                                                    data_model_name=self.name)
        ...
        model_utils._check_conflicts(to_add=new_feature_names,
    ...
    def create_lag_feature(self, new_feature_name: str, numeric_feature_name: str, time_length: int, hierarchy_name: str, level_name: str,
        ...
        project_json = self.project._get_dict()
        existing_features = data_model_helpers._get_unpublished_features(project_dict=project_json,
                                                                         data_model_name=self.name)
    ...
    def create_percentage_features(self, numeric_feature_name: str, hierarchy_name: str,
        ...
        project_dict = self.project._get_dict()
        measure_list = data_model_helpers._get_unpublished_features(project_dict=project_dict,
                                                                    data_model_name=self.name,
                                                                    feature_type=FeatureType.NUMERIC)
        ...
        for lev_index, level in enumerate(level_names):
            ...
            feature_utils._create_percentage_feature_local(project_dict=project_dict, cube_id=self.cube_id,
                                                           new_feature_name=name,
                                                           numeric_feature_name=numeric_feature_name,
                                                           dimension_name=dimension_name,
                                                           hierarchy_name=hierarchy_name, level_name=level,
                                                           description=description,
                                                           caption=caption, folder=folder, format_string=format_string,
                                                           visible=visible)
    ...
    def create_period_to_date_features(self, numeric_feature_name: str, hierarchy_name: str,
        ...
        for lev_index, level in enumerate(level_names):
            name = new_feature_names[lev_index]
            feature_utils._create_period_to_date_feature_local(project_dict=project_json, cube_id=self.cube_id,
                                                               new_feature_name=name,
                                                               numeric_feature_name=numeric_feature_name,
                                                               hierarchy_name=hierarchy_name, level_name=level,
                                                               base_name=base_level, time_dimension=time_dimension,
    ...
    def get_hierarchies(self, secondary_attribute: bool = False, folder_list: List[str] = None) -> Dict:
    ...
    def list_related_hierarchies(self, dataset_name: str) -> List[str]:
        ...
        key_ref_ids = []
        dataset_ref = dictionary_parser.parse_dict_list(data_model_parser._get_dataset_refs(cube_dict), 'id',
                                                        dataset.get('id'))
        ...
        hierarchy_names = set()
        for dimension in project_dict.get('dimensions', {}).get('dimension', []) + cube_dict.get('dimensions', {}).get(
            'dimension', []):
    ...
    def create_perspective(self, name: str, dimensions: List[str] = None, hierarchies: List[str] = None,
                           categorical_features: List[str] = None, numeric_features: List[str] = None,
                           publish: bool = True):
        ...
        """
        model_utils._perspective_check(self, "Perspectives cannot be created from other perspectives")
        ...
        if name in perspective_names:
            raise atscale_errors.UserError(f'A perspective named: {name} already exists')

        numeric_features_info = data_model_helpers._get_unpublished_features(project_dict, data_model_name=self.name,
                                                                             feature_type=FeatureType.NUMERIC)
        categorical_feature_info = data_model_helpers._get_unpublished_features(project_dict, data_model_name=self.name,
                                                                                feature_type=FeatureType.CATEGORICAL)
        hierarchy_info = self.get_hierarchies(secondary_attribute=False)
        hierarchy_roleplays = {}
        dimension_roleplays = {}
        for info in categorical_feature_info.values():
            for hier, base_hier in zip(info.get('hierarchy'), info.get('base_hierarchy', info.get('hierarchy'))):
            ...
            if info.get('dimension') not in dimension_roleplays:
                dimension_roleplays[info.get('dimension')] = {
                    ...
                    'ref_id': info.get('roleplay_ref_id', '')}
        if numeric_features:
            model_utils._check_features(numeric_features, numeric_features_info,
                                        errmsg='The following are not numeric features: {}',
                                        diff_in_msg=True)
        if categorical_features:
            model_utils._check_features(categorical_features, categorical_feature_info,
                                        errmsg='The following are not categorical features: {}',
                                        diff_in_msg=True)
        if hierarchies:
            model_utils._check_features(features=hierarchies,
                                        check_list=hierarchy_info,
                                        errmsg='The following hierarchies do not exist: {}',
                                        diff_in_msg=True)
        ...
        if numeric_features:
            ...
            calculated_measures_dict = {
                'calculated-member-ref': []
                }
            for feature in numeric_features:
                if numeric_features_info[feature]['atscale_type'] == 'Aggregate':
                    measure_id = [x['id'] for x in project_dict.get('attributes', {}).get('attribute', []) + cube.get('attributes', {}).get('attribute', []) if x['name'] == feature][0]
                    measure_dict = {
                        'id': measure_id,
                        'properties': {
                            'visible': False
                    ...
                    calculated_measure_id = [x['id'] for x in project_dict['calculated-members']['calculated-member'] if x['name'] == feature][0]
        ...
        if dimensions:
            for dimension in dimensions:
                for dim in project_dict.get('dimensions', {}).get('dimension', []) + cube.get('dimensions', {}).get(
                    'dimension', []):
                    if dim.get('name') == dimension_roleplays.get(dimension, {}).get('base_dimension'):
                        ...
                        if dimension != dimension_roleplays[dimension]['base_dimension']:
                            dimension_dict['ref-path'] = {
                                'ref': [{
                                    'id': dimension_roleplays[dimension]['ref_id']}]}
        ...
        if hierarchies:
            for hierarchy in hierarchies:
                for dimension in  project_dict.get('dimensions', {}).get('dimension', []) + cube.get('dimensions', {}).get('dimension', []):
                                ...
                                for dim in dimensions_dict['flat-dimension-ref']:
                                    if dim.get('id') == dimension.get('id'):
                                        ...
                                        if dim.get('ref-path', {}).get('ref',[]):
                                            if dim.get('ref-path', {}).get('ref',[])[0].get('id','') == hierarchy_roleplays[hierarchy]['ref_id']:
                                                match = True
                                    ...
                                    if hierarchy != hierarchy_roleplays[hierarchy]['base_hierarchy']:
                                        dimension_dict['ref-path'] = {'ref': [{'id':hierarchy_roleplays[hierarchy]['ref_id']}]}
                                    dimensions_dict['flat-dimension-ref'].append(dimension_dict)
        ...
        if categorical_features:
            for categorical_feature in categorical_features:
                ...
                for dimension in project_dict.get('dimensions', {}).get('dimension', []) + cube.get('dimensions', {}).get('dimension', []):
                    for hier in dimension.get('hierarchy', []):
                        if secondary_attribute or hier.get('name') == hierarchy_roleplays[hierarchy]['base_hierarchy']:
                            for attribute in project_dict.get('attributes', {}).get('keyed-attribute', []) + cube.get('attributes', {}).get('keyed-attribute', []):
                                if attribute.get('name') == categorical_feature_info[categorical_feature].get('base_name', categorical_feature):
                                    ...
                                    if secondary_attribute:
                                        ...
                                        if categorical_feature != categorical_feature_info[categorical_feature].get('base_name', categorical_feature):
                                            attribute_dict['ref-path']['ref'] = [{'id':categorical_feature_info[categorical_feature]['roleplay_ref_id']}]
                                        perspective_dict['flat-attributes'].setdefault('flat-attribute-ref', [])
                                        perspective_dict['flat-attributes']['flat-attribute-ref'].append(attribute_dict)
                                    ...
                                    for dim in dimensions_dict['flat-dimension-ref']:
                                        if dim.get('id') == dimension.get('id'):
                                            if dim.get('ref-path', {}).get('ref',[]):
                                                if dim.get('ref-path', {}).get('ref',[])[0].get('id','') == categorical_feature_info[categorical_feature].get('roleplay_ref_id', ''):
                                                    match = True
                                            # todo: test these nests with a degenerate and cube dimension
                                        ...
                                        if categorical_feature != categorical_feature_info[categorical_feature].get('base_name', categorical_feature):
                                            dimension_dict['ref-path'] = {'ref': [{'id':categorical_feature_info[categorical_feature]['roleplay_ref_id']}]}
                                        dimensions_dict['flat-dimension-ref'].append(dimension_dict)
                                    break
                            break
                    else:
    ...
    def delete(self, publish: bool=True):
        ...
        if self.is_perspective():
            updated_perspectives = [x for x in project_dict['perspectives']['perspective'] if x['id'] != self.id]
            ...
            project_dict.setdefault('perspectives', {})
            project_dict['perspectives'].setdefault('perspective', [])
            updated_perspectives = [x for x in project_dict['perspectives']['perspective'] if x['cube_ref'] != self.id]
            project_dict['perspectives']['perspective'] = updated_perspectives
        self.project._update_project(project_dict, publish)

    def bulk_operator(self, function: Callable, parameter_list: List[Dict], publish: bool = True):
        ...
        if bulk_operation is None:
            raise atscale_errors.UserError(f'The function {function.__name__} is not supported for bulk operations'
                                           f' at this time.')

        project_dict = self.project._get_dict()
        feature_dict = self.get_features(use_published= False)

        #run our bulk operator
        bulk_operation(self.project.atconn, project_dict, feature_dict, self.cube_id, parameter_list)
