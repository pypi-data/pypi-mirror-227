...
def _prep_join_columns_for_join(
    join_columns: List[List[str]], atscale_columns: List[str]
) -> List[List[str]]:
...
def _parse_denormalized_categorical_features(data_model_name, project_dict):
            ...
            for level in hierarchy["level"]:
                if level["primary-attribute"] in folder_info:
                    ...
                    folder_info[level["primary-attribute"]] = {
                        ...
                        "atscale_type": level.get("properties", {}).get("level-type", "Standard"),
                    }
    ...
    for i in feature_info:
        return_dict[i["name"]]["caption"] = i.get("properties", {}).get("caption", "")
        return_dict[i["name"]]["atscale_type"] = folder_info.get(i["id"], {}).get(
            "atscale_type", "Standard"
        )
        return_dict[i["name"]]["description"] = i.get("properties", {}).get("description", "")
...
def _get_published_features(
    ...
    if feature_type is FeatureType.ALL or feature_type is FeatureType.CATEGORICAL:
        ...
        dimension_dict = get_dmv_data(
            model=data_model,
            fields=[Level.type, Level.description, Level.hierarchy, Level.caption, Level.data_type],
