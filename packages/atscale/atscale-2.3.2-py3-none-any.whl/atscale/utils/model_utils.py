...
def _check_conflicts(
    ...
    if preexisting is not None:
        ...
        preexisting = dmh._get_unpublished_features(
...
def _create_dataset_relationship(
    ...
    if not dataset_id:  # then we have to create a project_dataset
        ...
        if table_columns is None:
            table_columns = atconn.get_table_columns(
        ...
        project_dataset, dataset_id = project_utils.create_dataset(
            table_name, warehouse_id, table_columns, database, schema, dataset_name=dataset_name
        )
...
def _create_dataset_relationship_from_dataset(
    ...
    for join_feature, join_column, roleplay_feature in zip(
        ...
        if dimension_index is not None:  # don't do bool because if index is 0 it will return false
            ...
            key_ref = {"id": ref, "unique": False, "complete": "false", "column": join_column}
            if roleplay_feature != "":
        ...
        # if not dimension_index:
            ...
            if dimension_index is not None:
                ...
                ref = dimension["key-ref"]
                key_ref = {"id": ref, "unique": False, "complete": "partial", "column": join_column}
                if roleplay_feature != "":
                    if "{0}" not in roleplay_feature:
                        roleplay_feature = roleplay_feature + " {0}"
                    ref_id = str(uuid.uuid4())
                ...
                key_refs.append(key_ref)
                uid = dimension["id"]
                attr = {"id": uid, "complete": "partial", "column": join_column}
    ...
    if not found:
        data_set_ref = templates.create_dataset_ref_dict(dataset_id, key_refs, attribute_refs)
        cube_dict["data-sets"]["data-set-ref"].append(data_set_ref)
    return project_dict
    ...
    for (column_name, column_type) in date_cols:
        ...
        if not generate_date_table:
            ...
            if not date_key_feature:
                ...
                columns = atconn.get_table_columns(
                    warehouse_id=warehouse_id,
...
def _add_related_hierarchies(
    data_model: DataModel, hierarchy_list: List[str], hierarchies_to_check: List[str] = None
) -> None:
...
def _add_related_hierarchies_helper(
    hierarchy_list: List[str], hierarchies_to_check: List[str], hierarchy_relationship_dict: Dict
) -> None:
    """Recursively adds hierarchies to hierarchy_list if they are related to a hierarchy in hierarchies_to_check.
