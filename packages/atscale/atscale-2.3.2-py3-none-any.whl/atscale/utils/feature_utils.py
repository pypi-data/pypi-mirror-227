...
from atscale.base.enums import FeatureFormattingType, Hierarchy, Level, Aggs, MappedColumnFieldTerminator, \
    MappedColumnKeyTerminator, MappedColumnDataTypes, FeatureType
from atscale.base.templates import (create_attribute_ref_dict, create_calculated_member_dict,
                                    create_calculated_member_ref_dict, create_keyed_attribute_dict,
...
def _create_secondary_attribute(cube_id, project_dict: Dict, data_set: dict,column_name: str, new_attribute_name: str, hierarchy_name: str, level_name: str,
...
def _create_filter_attribute(data_model, project_dict, new_feature_name: str,
                             level_name: str,
                             hierarchy_name: str,
    ...
    for dataset in project_dsets + cube_dsets:
        for attribute in dataset.get('logical', {}).get('attribute-ref', []):
            if attribute['id'] == column_id:
                ...
                project_dataset = project_parser.get_dataset(project_dict=project_dict,
                ...
                counter = 1
                existing_columns = model_utils._get_column_names(project_dict=project_dict,
                                                                 dataset_name=project_dataset['name'])
                ...
                project_utils.add_calculated_column_to_project_dataset(atconn=data_model.project.atconn,
                                                                       data_set=project_dataset,
                                                                       column_name=calculated_column_name,
                                                                       expression=expression)

                data_set = project_parser.get_dataset(project_dict=project_dict, dataset_name=dset_name)

                _create_secondary_attribute(data_model.cube_id, project_dict, 
                                           data_set=data_set,
                                           column_name=calculated_column_name,
...
def _create_mapped_columns(dataset: Dict, column_name: str, mapped_names: List[str],
    ...
    cols = project_utils.create_dataset_columns_from_atscale_table_columns(list(zip(mapped_names, data_types_names)))
...
def _delete_measures_local(data_model, measure_list: List[str],
    ...
    for attribute in calculated_members:
        ...
        name_to_id[name] = attribute['id']
        keep_id[attribute['id']] = True
        _set_dependencies(calculated_measure=attribute,
    ...
    for name in measure_list:
        ...
        if new_dependants:
            ...
            if not should_delete:
                raise atscale_errors.DependentMeasureException(f'Aborted deletions due to dependent measures')
            else:
    ...
    for measure in calculated_members:
        if keep_id[measure['id']] or calc_member_in_other_cubes.get(measure['id']):
...
def _set_dependencies(calculated_measure: Dict, dependants_of: Dict[str, List[str]]):
...
def _check_unpublished_hierarchy(project_dict: dict, data_model_name: str, hierarchy_name: str, level_name: str, expect_base_input: bool = False,
                                 raise_error: bool = True, additional_error_message: str = None):
    ...
    """
    all_features = data_model_helpers._get_unpublished_features(project_dict = project_dict, data_model_name = data_model_name, feature_type=FeatureType.CATEGORICAL)
    ...
    if not found_hierarchy_in_published and not found_hierarchy_in_base and not found_hierarchy_outside:
...
def _check_hierarchy(data_model, hierarchy_name: str, level_name: str):
    """Queries the provided DataModel for the provided hierarchy name and level name to ensure both exist.  
...
def _create_calculated_feature_local(project_dict: dict, cube_id: str, name, expression, description=None, caption=None,
...
def _create_aggregate_feature_local(project_dict: dict, cube_id: str, dataset_id: str, column_name: str, name: str, 
    ...
    if aggregation_type.dict_expression:
        key_ref_id = str(uuid.uuid4())
        # create attribute key in cube
        attribute_key_dict = create_attribute_key_dict(key_id=key_ref_id,
                                                       columns=1,
                                                       visible=visible)
        ...
        key_ref_for_dataset = create_attribute_key_ref_dict(key_id=key_ref_id,
                                                            columns=[column_name],
                                                            complete=True,
                                                            unique=False)
    ...
    new_measure = create_measure_dict(measure_id=uid, measure_name=name,
                                      agg_type=aggregation_type, caption=caption, visible=visible,
...
def _create_rolling_agg(project_dict: Dict, cube_id: str, time_dimension: str, agg_type , new_feature_name: str, 
                           numeric_feature_name: str, time_length: int, hierarchy_name: str, level_name: str,
                           description: str = None, caption: str = None, folder: str = None, 
                           format_string: Optional[Union[FeatureFormattingType, str]] = None,
                           visible: bool = None):
    ...
    _create_calculated_feature_local(project_dict, cube_id=cube_id, name = new_feature_name, 
...
def _create_lag_feature_local(project_dict: Dict, cube_id: str, time_dimension: str, 
               ...
               hierarchy_name: str, level_name: str, description: str = None, 
...
def _create_percentage_feature_local(project_dict: Dict, cube_id: str, new_feature_name: str, numeric_feature_name: str, 
                            dimension_name: str, hierarchy_name: str, level_name: str,
    ...
    _create_calculated_feature_local(project_dict=project_dict, cube_id=cube_id, name=new_feature_name, expression=expression,
                                     description=description, caption=caption, folder=folder,
                                     format_string=format_string, visible=visible)

def _create_period_to_date_feature_local(project_dict: Dict, cube_id: str, new_feature_name: str, 
                        numeric_feature_name: str, hierarchy_name: str, level_name: str, 
                        base_name: str, time_dimension: str, description: str = None,
    ...
    expression = f'CASE WHEN IsEmpty([Measures].[{numeric_feature_name}]) THEN NULL ELSE ' \
                    ...
                    f'[{time_dimension}].[{hierarchy_name}].CurrentMember), [Measures].[{numeric_feature_name}]) END'
    _create_calculated_feature_local(project_dict=project_dict, cube_id=cube_id, name= new_feature_name,
