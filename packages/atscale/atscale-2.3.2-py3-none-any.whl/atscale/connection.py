...
class Connection:
    ...
    def _set_jdbc_string(self):
        if self.__token:
            ...
            response = self._submit_request(request_type=RequestType.GET, url=f'{self.server}:{self.engine_port}/organizations/orgId/{self._organization}')
    ...
    def connect(self, organization: Optional[str] = None):
        ...
        Args:
            organization (Optional[str], optional): _description_. Defaults to None.
        """
    ...
    def _post_query(self, query, project_name, use_aggs=True, gen_aggs=False, fake_results=False,
    ...
    def get_connected_databases(self, warehouse_id: str) -> List[str]:
        ...
        u = f'{self.server}:{self.engine_port}/data-sources/orgId/{self.organization}/conn/{warehouse_id}/databases'
    ...
    def get_connected_schemas(self, warehouse_id: str, database: str = None) -> List[str]:
        ...
        u = f'{self.server}:{self.engine_port}/data-sources/orgId/{self.organization}/conn/{warehouse_id}/schemas{info}'
        response = self._submit_request(request_type=RequestType.GET, url=u)
        return json.loads(response.content)['response']
    ...
    def get_query_columns(self, warehouse_id: str, query: str):
        ...
        # cache refresh
        u = f'{self._warehouse_endpoint()}/conn/{warehouse_id}/tables/cacheRefresh'
        self._submit_request(request_type=RequestType.POST,
                             url=u)

        # preview query
        url = f'{self._warehouse_endpoint()}/conn/{warehouse_id}/query/sample'
        payload = {"query": query}
