...
from atscale.errors.atscale_errors import (
    UserError,
    AtScaleExtrasDependencyImportError,
    AuthenticationError,
    AtScaleServerError,
)
...
class Connection:
    ...
    def design_center_url(
        ...
        """
        raise UserError("The value of design_center_url is FINAL")
    ...
    def engine_url(
        ...
        """
        raise UserError("The value of engine_url is FINAL")
    ...
    def _post_atscale_query(
        ...
        if re.search("<succeeded>(.*?)</succeeded>", content).group(
            1
        ) == "false" and "Error during query planning: no such vertex in graph" in re.search(
        ...
        for org in org_list:
            if org.get("id") == self.organization:
                if org.get("OAuth", "") == "azure":
                    ...
                    return False
        raise UserError(f"Unable to find organization: {self.organization}")
