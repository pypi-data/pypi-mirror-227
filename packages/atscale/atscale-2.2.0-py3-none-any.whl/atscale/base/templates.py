...
from atscale.base.enums import Aggs, MappedColumnKeyTerminator, MappedColumnFieldTerminator, MappedColumnDataTypes, TimeLevels
...
def create_dimension_dict(hierarchy_dict: dict, dim_id: str, name: str,
                          visible: bool, time_dimension: bool = False):
    properties = {
            'visible': visible
        }
    if time_dimension:
...
def create_query_for_post_request(query: str, project_name: str, organization: str, use_aggs=True, gen_aggs=False,
                                  fake_results=False, dry_run=False,
                                  use_local_cache=True, use_aggregate_cache=True, timeout=10) -> dict:
    return {
        'language': 'SQL',
        'query': query,
        'context': {
