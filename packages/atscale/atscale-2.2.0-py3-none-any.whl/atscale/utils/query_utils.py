...
def _generate_atscale_query(data_model, feature_list: List[str], filter_equals: Dict[str,str] = None, 
                           ...
                           use_aggs=True, gen_aggs=True, raise_multikey_warning = True) -> str:
    ...
    list_all = list(all_features.keys())
    ...
    if raise_multikey_warning:
        ...
        levelTypeNames = [ x.name for x in LevelType]
        
        #get the subset of all columns that are non-measures and then see if any of the features overlap
        features_to_validate = [x for x in feature_list if all_features[x]['data_type'] in levelTypeNames]

        if features_to_validate != []: 
            features_needed = project_parser._get_multi_key_query_names_from_features(cube_id=  data_model.cube_id, feature_list = features_to_validate, 
                project_dict = project_dict)

            for feature_name in features_needed.keys():
                #find missing keys from the provided feature list
                missing_keys = [i for i in features_needed[feature_name] if i not in feature_list ]
                if len(missing_keys) > 0:
                    missing_key_string = ', '.join(missing_keys)
                    logger.warning(f"Feature: {feature_name} has a compound key, additional features of the key [{missing_key_string}] should be included to avoid ambiguous results")
    ...
    all_categorical_features = [name for name, metadata in all_features.items() if
