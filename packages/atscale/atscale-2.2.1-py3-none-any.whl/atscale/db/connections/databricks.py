...
class Databricks(SQLAlchemyConnection):
    ...
    def __init__(self, host: str, catalog: str, schema: str, http_path: str, token: str = None, port: int = 443):
        ...
        try:
            ...
            logger.error(
                'Unable to create database connection, please verify the inputs')
            raise
    ...
    def _get_connection_url(self):
        ...
        if not self._token:
            self._token = cryptocode.encrypt(getpass.getpass(
                prompt='Please enter your Databricks token: '), self.platform_type.value)
    ...
    def write_df_to_db(
            self, table_name: str, dataframe: DataFrame,
            if_exists: PandasTableExistsActionType = PandasTableExistsActionType.FAIL, chunksize: int = 1000):
        from databricks import sql
