...
from atscale.parsers import data_model_parser, project_parser, response_parser
from atscale.project.project import Project
from atscale.utils import model_utils, project_utils, input_utils, db_utils
...
class DataModel:
    ...
    def add_queried_dataset(self, warehouse_id: str, dataset_name: str, query: str,
        ...
        """ Creates a new Queried Dataset in this data model which returns results of querying the given query against
    ...
    def get_data(self, feature_list: List[str], filter_equals: Dict[str, str] = None,
        ...
        from atscale.utils.query_utils import _generate_atscale_query
        ...
        query = _generate_atscale_query(data_model=self, feature_list=feature_list,
        ...
        df: pd.DataFrame = response_parser.parse_rest_query_response(
            queryResponse)
    ...
    def get_data_direct(self, dbconn: SQLConnection, feature_list, filter_equals=None, filter_greater=None, filter_less=None,
        ...
        """
        from atscale.utils.query_utils import generate_db_query, _generate_atscale_query  # avoid circular import
    ...
    def get_data_spark(self, feature_list: List[str], sparkSession, jdbc_format: str, jdbc_options: Dict[str,str],
        ...
        """Submits a query using the supplied information and returns the results in a spark DataFrame.
        Args:
            feature_list (List[str]): The list of features to query.
        ...
        query = generate_db_query(self, _generate_atscale_query(data_model=self, feature_list=feature_list,
                                                filter_equals=filter_equals,
                                                filter_greater=filter_greater,
                                                filter_less=filter_less,
                                                filter_greater_or_equal=filter_greater_or_equal,
