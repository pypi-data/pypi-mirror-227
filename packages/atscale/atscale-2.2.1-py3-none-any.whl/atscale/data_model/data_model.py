...
class DataModel:
    ...
    def writeback(self, dbconn: SQLConnection, table_name: str, dataframe: pd.DataFrame, join_features: list, join_columns: list = None, roleplay_features: list = None,
        ...
        for i, (join_feature, join_column) in enumerate(joins):
            ...
            if len(join_column) != len(key_dict[join_feature]['key_cols']):
                required_feature_names = project_parser._get_query_names_from_value_columns(project_dict, self.cube_id, key_dict[join_feature]["key_cols"])
                raise atscale_errors.UserError(f'Relationship for feature: "{join_feature}" requires keys: "{", ".join(required_feature_names.values())}" but received: "{", ".join(join_column)}"')
            if len(key_dict[join_feature]['key_cols']) == 1 and key_dict[join_feature]['key_cols'][0] != key_dict[join_feature]['value_col']:
                ...
                if df_key is not None:
                    if join_column[0] != key_dict[join_feature]['value_col']:
                        df_key.rename(columns={key_dict[join_feature]['value_col']:join_column[0]},inplace=True)
    ...
    def create_calculated_feature(self, new_feature_name: str, expression: str, description: str = None,
        ...
        model_utils._validate_mdx_syntax(self, expression)
