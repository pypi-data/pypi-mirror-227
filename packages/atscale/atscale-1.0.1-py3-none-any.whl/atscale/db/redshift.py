...
class Redshift(Database):
    ...
    def _add_table_internal(self, table_name: str, dataframe: pandas.DataFrame, chunksize: int=1000,
        ...
        df.to_sql(name=fixed_table_name, con=engine, schema=self.schema, method='multi', index=False,
                  chunksize=chunksize, if_exists=if_exists)
        logging.info(f'Table \"{fixed_table_name}\" created in Redshift with {df.size} rows and {len(df.columns)} columns')
