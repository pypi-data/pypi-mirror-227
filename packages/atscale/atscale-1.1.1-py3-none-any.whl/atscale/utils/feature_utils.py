...
from atscale.utils.enums import FeatureFormattingType, Hierarchy, Level, Measure, MDXAggs, Aggs, TimeSteps
...
def create_secondary_attribute(data_model: DataModel, dataset_name: str, column_name: str, new_attribute_name: str, hierarchy_name: str, level_name: str, 
        ...
        data_set = project_parser.get_dataset_from_datasets_by_name(project_parser.get_datasets(project_json),
                                                                        dataset_name)
        if not data_set:
            raise atscale_errors.UserError(f'Dataset \'{dataset_name}\' not associated with given project')
        ...
        new_keyed_attribute = {   #in project
            'id': attribute_id,
            'key-ref': ref_id,
...
def create_filter_attribute(data_model: DataModel, new_feature_name: str, level_name: str, hierarchy_name: str, filter_values: List[str],
    ...
    cube_ka_list = data_model._get_model_dict()[0].get('attributes', {}).get('keyed-attribute', [])
    ...
    cube_dsets = data_model_parser._get_cube_datasets(cube_dict=data_model._get_model_dict()[0])
    for dataset in project_dsets + cube_dsets:
        for attribute in dataset.get('logical', {}).get('attribute-ref', []):
            if attribute['id'] == column_id:
                ...
                create_calculated_column(data_model=data_model,
...
def generate_time_series_features(data_model: DataModel, dataframe: DataFrame, numeric_features:List[str], time_hierarchy:str, level:str, group_features:List[str]=None,
    ...
    """
    _check_time_hierarchy(data_model=data_model, hierarchy_name=time_hierarchy, level_name=level)

    measure_list = list(get_dmv_data(model=data_model, id_field=Measure.name).keys())
    ...
    for feature in numeric_features:
        for interval in intervals:
            ...
            if group_features:
                grouper = lambda x: x.groupby(group_features)
            else:
                grouper = lambda x: x
                group_features = []  # set this to an empty list so we can add it to hier_level later no matter what
            if interval > 1:
...
def create_calculated_column(data_model: DataModel, dataset_name: str, column_name: str, expression: str, publish: bool = True):
    ...
    cube_dict = project_parser.get_cube(project_dict=project_dict, id=data_model.cube_id)
    ...
    conn = dset['physical']['connection']['id']
    table = dset['physical']['tables'][0]
    ...
    data = {
        ...
        'expression': expression,
        'database': database}
    headers = atconn._generate_headers('x-www-form-urlencoded')
    response = request_utils.post_request(url, data=data, headers=headers)
    ...
    dset['physical']['columns'].append(new_column)
...
def create_mapped_columns(data_model: DataModel, dataset_name: str, column_name: str, mapped_names: List[str],
                          ...
                          publish: bool = True):
    """Creates a mapped columns.  Maps a column that is a key value structure into one or more new columns with the
...
def add_column_mapping(data_model: DataModel, dataset_name: str, column_name: str, mapped_name: str, data_type: str, publish: bool=True):
    ...
    if len(mapping_cols) < 1:
        raise atscale_errors.UserError(f'No mapped column exists for column: {mapped_name}. Use create_mapped_columns '
                                       f'to create one')
    already_mapped_w_name = [col for col in mapping_cols[0]['columns']['columns'] if col['name'] == mapped_name]
    if already_mapped_w_name:
        raise atscale_errors.UserError(f'There is already a mapping on column \'{column_name}\' for the key \'{mapped_name}\'')
...
def _delete_measures_local(data_model, measure_list: List[str],
    ...
    # top level calculated
    try:
        calculated_members: List[Dict] = json_dict['calculated-members']['calculated-member']
    except KeyError:
        calculated_members = []
    ...
    try:
        ...
        cube_attributes = []
    try:
        # cube level, id's of calculated measures in cube
        cube_calculated_member_ref = cube['calculated-members']['calculated-member-ref']
    except KeyError:
        cube_calculated_member_ref = []
    ...
    for attribute in calculated_members:
        ...
        name_to_id[name] = attribute['id']
        keep_id[attribute['id']] = True
        _set_dependants(calculated_measure=attribute, dependants_of=dependants_of)
        if name in measure_found:
            if measure_found[name]:
                raise Exception(
                    f'There is more than one measure with the given query name {name}')
...
def _check_hierarchy(data_model, hierarchy_name, level_name):
    ...
    if level_name:
        ...
        if level.get(Level.hierarchy.name) != hierarchy_name:
            raise atscale_errors.UserError(f'Level: {level_name} does not exist in Hierarchy: {hierarchy_name}')
...
def _create_calculated_feature_local(project_json: dict, cube_id: str, name, expression, description=None, caption=None,
    ...
    project_json.setdefault('calculated-members', {})
    project_json['calculated-members'].setdefault('calculated-member', [])
    project_json['calculated-members']['calculated-member'].append(
        new_calculated_measure)
    ...
    cube.setdefault('calculated-members', {})
    cube['calculated-members'].setdefault('calculated-member-ref', [])
...
def create_denormalized_categorical_feature(data_model: DataModel, dataset_name: str, column_name: str, name: str, description: str=None,
    ...
    if not data_model.dataset_exists(dataset_name):
        raise atscale_errors.UserError(f'Dataset \'{dataset_name}\' not associated with given model')

    if not data_model.column_exists(dataset_name, column_name):
        raise atscale_errors.UserError(f'Column \'{column_name}\' not found in the \'{dataset_name}\' dataset')
    ...
    # we do it this way so we can use pass by reference to edit the base dict
    cube = project_parser.get_cube(project_dict=project_json, id=data_model.cube_id)
    ...
    cube.setdefault('dimensions', {})
    cube['dimensions'].setdefault('dimension', [])
...
def create_aggregate_feature(data_model: DataModel, dataset_name: str, column_name: str, name: str, aggregation_type: Aggs,
                             ...
                             format_string: Optional[Union[FeatureFormattingType, str]] = None, publish: bool = True):
    ...
    new_measure = {
        ...
        'properties': {
            'type': {
                'measure': {
                    'default-aggregation': aggregation_str}
                },
            'caption': caption,
            'visible': True}
        }
    ...
    data_set_project = project_parser.get_dataset_from_datasets_by_name(project_parser.get_datasets(project_json),
                                                                        dataset_name)
    data_set_id = data_set_project.get('id')
    dataset: dict = [x for x in cube['data-sets']['data-set-ref'] if x['id'] == data_set_id][0]
            ...
            if length > 1:
                for agg in MDXAggs:
                    ...
                    expression = agg.value + f'(' \
                                             ...
                                             f'[Measures].[{feature}])'
                    features_to_create.append((specific_name, expression, feature_folder))
...
def create_one_hot_encoded_features(data_model: DataModel, categorical_feature: str, description: str=None, folder: str=None,
    ...
    for name in created_names:
        if name in existing_measures:
            raise atscale_errors.UserError(f'Invalid name: \'{name}\'. A feature already exists with that name')
    data_model.project._update_project(project_json=project_json, publish=publish)
...
def create_time_differencing(data_model: DataModel, new_feature_name: str, numeric_feature_name: str, time_length: int,
    ...
    check_features(features=[numeric_feature_name], check_list=list(existing_measures.keys()))
...
def create_percentage(data_model: DataModel, new_feature_name: str, numeric_feature_name: str, hierarchy_name: str, level_name: str, 
    ...
    project_dict = data_model.project._get_dict()
    if new_feature_name in measure_list:
        raise atscale_errors.UserError(
            f'Invalid name: \'{new_feature_name}\'. A feature already exists with that name')
    _create_percentage_local(project_dict=project_dict, cube_id=data_model.cube_id, name=new_feature_name,
                             numeric_feature_name=numeric_feature_name, dimension_name=dimension_name,
                             hierarchy_name=hierarchy_name, level_name=level_name, description=description,
                             caption=caption, folder=folder, format_string=format_string, visible=visible)
    data_model.project._update_project(project_json=project_dict, publish=publish)


def create_percentages(data_model: DataModel, numeric_feature_name: str, hierarchy_name: str, description: str = None,
    ...
    for level in level_list:
        if level != level_list[-1]:
            ...
            _create_percentage_local(project_dict=project_dict, cube_id=data_model.cube_id, name=name,
                                     ...
                                     caption=None, folder=folder, format_string=format_string, visible=visible)
...
def create_periods_to_date(data_model: DataModel, numeric_feature_name: str, hierarchy_name: str, description: str = None,
    ...
    hier_levels = sorted(hier_levels, key=(lambda x: x[Level.level_number.name]), reverse=False)  # sort so output or error is in order
    base = hier_levels[-1]

    existing_measures = list(get_dmv_data(model=data_model, id_field=Measure.name).keys())
    check_features(features=[numeric_feature_name], check_list=existing_measures)

    project_json = data_model.project._get_dict()
    cube_id = data_model.cube_id
    names = []
    for level in hier_levels[:-1]:
        names.append(f'{numeric_feature_name}_{level[Level.name.name]}_To_{base[Level.name.name]}')
