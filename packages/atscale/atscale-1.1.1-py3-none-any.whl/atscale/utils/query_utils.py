...
from atscale.utils import request_utils, feature_utils, config
...
def generate_atscale_query(data_model: DataModel, feature_list: List[str], filter_equals: Dict[str,str] = None, 
                           ...
                           filter_not_null: Dict[str,str] = None, limit: int = None, comment:str = None) -> str:
    ...
    if numeric_features:
        numeric_columns_string = ' ' + \
                                 ', '.join(f'`{x}`' for x in numeric_features)
    else:
        numeric_columns_string = ''
    ...
    query = f'SELECT {categorical_columns_string}{numeric_columns_string} FROM `{data_model.project.project_name}`' \
            f'.`{data_model.name}`{filter_string}{order_string}{limit_string}{comment_string}' \
            f'{version_comment}'
    return query


def generate_db_query(data_model: DataModel, atscale_query: str) -> str:
    ...
    atconn._post_query(inbound_query, published_project_name)

    url = f'{atconn._query_view_endpoint()}' \
          f'&querySource=user&queryStarted=5m&queryDateTimeStart={date_time}'

    response = request_utils.get_request(url=url, headers=atconn._generate_headers())
            ...
            for event in query_info['timeline_events']:
                if event['type'] == 'SubqueriesWall':
                    ...
                    if event['children'][0]['query_text_truncated']:
                        url = f'{atconn._design_org_endpoint()}/' \
