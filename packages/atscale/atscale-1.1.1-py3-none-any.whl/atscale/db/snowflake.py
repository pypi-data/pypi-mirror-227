...
class Snowflake(SQLConnection):
    ...
    def __init__(self, username: str, account: str, warehouse: str,
        ...
        if None in [username, account, warehouse, database, schema]:
            raise ValueError(
    ...
    #        self._password = getpass.getpass(prompt='Please enter your password for snowflake: ')
            self._password = cryptocode.encrypt(getpass.getpass(prompt='Please enter your password for Snowflake: '), self.platform_type.value)
        password = cryptocode.decrypt(self._password, self.platform_type.value)
    #    connection_url = f'snowflake://{self._username}:{self._password}@{self._account}/{self._database}/{self._schema}?warehouse={self._warehouse}'
    ...
    def write_df_to_db(self, table_name, dataframe: pd.DataFrame, if_exists: TableExistsActionType = TableExistsActionType.FAIL, chunksize=10000):
        #fixed_table_name = self.fix_table_name(table_name)
        #dataframe = dataframe.rename(columns=lambda c: self._fix_column_name(c))
        from snowflake.connector.pandas_tools import pd_writer
        dataframe.to_sql(name=table_name, con=self.engine, schema=self._schema, if_exists=if_exists.value, index=False,
                         chunksize=chunksize, method=pd_writer)

    def _submit_queries_pca_optimized(self, queries: list) -> list:
