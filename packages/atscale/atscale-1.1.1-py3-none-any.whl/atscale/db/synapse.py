...
class Synapse(SQLConnection):
    ...
    def __init__(self, username:str, host:str, database:str, driver:str, schema:str, port:int=1433, password:str=None):
        ...
        if None in [username, host, database, driver, schema, port]:
            raise ValueError(
    ...
    def _format_types(self, dataframe: DataFrame) -> dict:
        ...
        for i in dataframe.columns:
            if str(type(dataframe[i].loc[~dataframe[i].isnull()].iloc[0])) in Synapse.conversion_dict_synapse:
                types[i] = Synapse.conversion_dict_synapse[str(
                    type(dataframe[i].loc[~dataframe[i].isnull()].iloc[0]))]
            else:
                types[i] = Synapse.conversion_dict_synapse['<class \'str\'>']
        return types
    ...
    def write_df_to_db(self, table_name: str, dataframe: DataFrame, if_exists: TableExistsActionType = TableExistsActionType.FAIL, chunksize: int = 10000):
        ...
        if exists and if_exists == TableExistsActionType.FAIL:
            raise Exception(
                f'A table named: {table_name} already exists in schema: {self.schema}')

        types = self.format_types(dataframe)

        if exists and if_exists == TableExistsActionType.REPLACE:  # if replace, then drop and recreate the table
            operation = f"DROP TABLE \"{self.schema}\".\"{table_name}\""
            cursor.execute(operation)
            self.create_table(table_name, types, cursor)
