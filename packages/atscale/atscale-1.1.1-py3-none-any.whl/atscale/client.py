...
from atscale.utils import input_utils, request_utils, config
...
class Client:
    ...
    def __init__(self, config_path: str = None, server: str = None, username: str = None, organization: str = None,
        ...
        if password is not None:
            p = password
        if design_center_server_port is not None:  # if someone passed in a value, we'll use that (defaults to None)
    ...
    def create_empty_project(self, project_name: str) -> Project:
        ...
        if not self._atconn.connected():
            raise atscale_errors.UserError('Please establish a connection by calling connect() first.')

        existing_projects = self.atconn._get_projects()
        if len(existing_projects) > 0:
            for x in existing_projects: 
                if x['name'] == project_name: 
                    raise atscale_errors.UserError('Project name already taken, new project name must be unique')
        
        u = self._atconn._design_org_endpoint('/project/createEmpty')
        h = self._atconn._generate_headers()
        p = {
            "name": project_name}
        p = json.dumps(p)
        # this call will handle or raise any errors
        response = request_utils.post_request(url=u, data=p, headers=h)
    ...
    def select_project(self) -> Project:
        ...
        project_dict = input_utils.choose_id_and_name_from_dict_list(projects, 'Please choose a project:')
