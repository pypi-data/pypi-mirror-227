...
from atscale.utils.model_utils import _check_features, _perspective_check
...
def create_pct_error_calculation(data_model, new_feature_name: str, predicted_feature_name: str, actual_feature_name: str,
    ...
    features_dict = data_model.get_features()
    numerics = [name for name, metadata in features_dict.items() if metadata['feature_type'].upper() == FeatureType.NUMERIC.name]
...
def create_net_error_calculation(data_model, new_feature_name: str, predicted_feature_name: str, actual_feature_name: str,
    ...
    if new_feature_name in measure_list or new_feature_name in level_list:
        raise atscale_errors.UserError(
            f'Invalid name: \'{new_feature_name}\'. A feature already exists with that name')
...
def generate_time_series_features(data_model, dataframe, numeric_features: List[str], time_hierarchy: str, level: str, group_features: List[str] = None,
    ...
    level_list = list(get_dmv_data(
        model=data_model, id_field=Level.name).keys())
    if group_features:
        if type(group_features) != list:
            group_features = [group_features]
        _check_features(group_features, measure_list + level_list)
