...
def find_dataset_with_table(datasets: list,  table_name: str) -> str:
    ...
    Args:
        datasets (list[dict]): The datasets to looke for the table in
...
def _get_query_names_from_value_columns(project_dict: dict, cube_id: str, value_columns: List[str]):
    """ Takes in key columns and maps to features that have those columns as their value column
    query column
    Args:
        project_dict:
        cube_id:
        value_columns: This should be named key columns

    Returns:
        A dict from column name to hierarchy level that returns that column
    """
    ...
    cube_datasets = data_model_parser._get_cube_datasets(cube)
...
def _get_multi_key_query_names_from_features(project_dict: dict, cube_id: str, feature_list: List[str]) -> Dict[str,List[str]]:
    ...
    feature_key_list: dict = _get_feature_keys(project_dict=project_dict, cube_id=cube_id,
                                               join_features=feature_list)
    cols_to_map_list_of_list = [feature_key_list[x]['key_cols'] for x in
                                feature_key_list.keys()]  # list of key_cols from all features
    cols_to_map = list(set([item for sublist in cols_to_map_list_of_list for item in sublist]))

    query_names_to_map = _get_query_names_from_value_columns(project_dict=project_dict, cube_id=cube_id,
                                                             value_columns=cols_to_map)
...
def _get_feature_keys(project_dict: dict, cube_id: str, join_features: List[str]):
        key_dict = {}
        project_datasets = get_datasets(project_dict)
        # get project and cube attributes. degenerate dimensions are in the cube
        project_attributes = project_dict.get('attributes', {}).get('keyed-attribute', [])
            ...
            for dataset in datasets:
                for key_ref in dataset.get('logical', {}).get('key-ref', []):
                    if key_ref.get('id') == attribute_key_ref:
                        # if we match the key-ref id we know we have the right dataset and can grab the key columns
                        found_dataset = True
                        for column in key_ref.get('column', []):
                            key_cols.append(column)
                        break
                if found_dataset:
                    # if we are in the right dataset we now look through the attribute-refs to find the value column
                    ...
                    if not physical:
                        physical = get_dataset_from_datasets(project_datasets, dataset.get('id')).get('physical')
                    ...
                    # fill in the query instead if it is a qds
                        ...
                        for query in queries:
                            ...
                            break
            key_dict[feature] = {'key_cols':list(set(key_cols)),'value_col':value_col,'database':database,'schema':schema,'table_name':table_name,'query':sql}
        return key_dict
