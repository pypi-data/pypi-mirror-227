...
class BigQuery(SQLConnection):
    ...
    def write_df_to_db(self, table_name: str, dataframe: DataFrame,
        ...
        check_table_exists = self.submit_query(f"SELECT size_bytes FROM {self.gbq_project_id}.{self.dataset}.__TABLES__ WHERE table_id='{table_name}'")
        table_exists = False
        ...
        for col in dataframe.columns:
            ...
            if pd.notnull(first_valid_index):
                ...
                if isinstance(val_to_check, datetime.date) and not isinstance(val_to_check, datetime.datetime):
                    schema_edits.append({'name':col, 'type':'DATE'})

        try:             
            pandas_gbq.to_gbq(dataframe, f'{self.gbq_project_id}.{self.dataset}.{table_name}',
                          project_id=self.gbq_project_id, if_exists=if_exists.value, table_schema= schema_edits)
        except ArrowTypeError as err:
            if table_exists == False:
                sql = f"DROP TABLE {self.gbq_project_id}.{self.dataset}.{table_name}"
    ...
    def _create_table_path(self, table_name: str) -> str:
        ...
        """
        return f'{self.gbq_project_id}.{self._dataset}.{table_name}'