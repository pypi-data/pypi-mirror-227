...
#D
    def __init__(self, username:str, host:str, namespace:str, driver:str, schema:str, port:int=1972, password:str=None):
        ...
        if None in [username, host, namespace, driver, schema, port]:
            raise ValueError(
    ...
    def _format_types(self, dataframe: DataFrame) -> dict:
        ...
        for i in dataframe.columns:
            if str(type(dataframe[i].loc[~dataframe[i].isnull()].iloc[0])) in Iris.conversion_dict_iris:
                types[i] = Iris.conversion_dict_iris[str(
                    type(dataframe[i].loc[~dataframe[i].isnull()].iloc[0]))]
            else:
                types[i] = Iris.conversion_dict_iris['<class \'str\'>']
        return types
    ...
    def write_df_to_db(self, table_name: str, dataframe: DataFrame, if_exists: TableExistsActionType = TableExistsActionType.FAIL, chunksize: int = 250):
        import pyodbc
        ...
        if exists and if_exists == TableExistsActionType.REPLACE:
