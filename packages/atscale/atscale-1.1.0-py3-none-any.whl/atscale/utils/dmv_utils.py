...
from atscale.utils import enums
from atscale.utils import request_utils
from atscale.utils.enums import Measure, Level, Hierarchy, DMVColumnBaseClass

T = typing.TypeVar('T', enums.Level, enums.Hierarchy, enums.Measure)


def get_dmv_data(model: DataModel,
                 fields: List[T] = None,
    ...
    query = generate_dmv_query_statement(fields=fields,
                                         ...
                                         id_field=id_field)
    rows = submit_dmv_query(atconn=model.project.atconn, query=query, project_name=model.project.project_name,
...
def submit_dmv_query(atconn: Connection, project_name: str, model_name: str, query: str):
    ...
    url = atconn._dmv_query_endpoint()
    h = atconn._generate_headers(content_type='xml')
    response = request_utils.post_request(url=url, data=query_body, headers=h)
...
DMVCol = typing.TypeVar('DMVCol', Measure, Level, Hierarchy)

def _parse_dmv_helper(rows,
                      fields: List[DMVCol],
                      id_field: DMVCol,
                      filter_by: Dict[DMVCol, List[str]] = {}) -> Dict[str, dict]:
        ...
        for term in fields:
            ...
            if result.get(name):
                if isinstance(result[name], list):
                    ...
                    result[name] = [result[name], sub_dict]
            else:
...
def measures_from_dmv(rows) -> Dict[str, dict]:
    ...
    :rtype: dict
    """
    # todo: expression doesn't get returned in dmv query
