...
from atscale.utils.secrets_manager import SecretsManager
...
class Config():
    ...
    def __new__(cls):
        ...
        if cls._instance is None:
            ...
            config_files = [DEFAULT_CONFIG_PATH,
                            LOCAL_CONFIG_PATH, LOCAL_CRED_PATH]
            # can inspect found to see which files were found
            found = cls._instance._config_parser.read(config_files)
            # Load dotenv, but instead of reading directly to environment variables,
            # we'll store them in config now.
            vals = dotenv_values(find_dotenv())
            # add the section key - we'll default to putting .env values in the DEFAULT section
            vals = {
                'DEFAULT': vals}
            cls._instance._config_parser.read_dict(vals)
            cls._instance.sm = SecretsManager()
    ...
    def get(self, key: str, section: str = "DEFAULT", default_value: str = None) -> str:
        ...
        if self._config_parser.has_section(section) or section == self._config_parser.default_section:
            return self._config_parser.get(section, key, fallback=default_value)
        else:
            logger.warning(f'No section {section} exists.')
            return default_value

    def get_secret_json(self, key, section="DEFAULT", default_value=None):
        ...
        if val:
            ...
            return self.sm.get_secret_json(key)
