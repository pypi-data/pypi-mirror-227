...
class Project:
    ...
    def __init__(self, atconn: Connection, project_id: str, published_project_id: str = None):
        ...
        # if published is none, then default to the latest published (if it was ever published)
        if published_project_id is None:
            published_project = project_parser.parse_most_recent_published_project_for_project(
                project_dict, published_projects_list)
            if published_project is not None:  # if they ever published this project
                self.__published_project_id: Optional[str] = published_project.get('id')
                self.__published_project_name: Optional[str] = published_project.get('name')
            else:  # else it was never published so let's default to None
...
#D
    ...
    def atconn(self, value):
        ...
        """
        raise Exception('Value of atconn is final; it cannot be reset. Please construct a new Project instead')
#D
    @property
...
#D
    ...
    def project_name(self, value):
        ...
        """
        raise Exception('Value of project_name is final; it cannot be reset. Please construct a new Project instead')
#D
    @property
...
#D
    def select_published_project(self):
        ...
        published_project_dict = input_utils.choose_id_and_name_from_dict_list(published_project_list, 'Please choose a published project:')
        if published_project_dict is None:
...
#D
    def select_data_model(self):
        ...
        data_model_list = project_parser.get_data_models(self._get_dict())
        data_model_dict = input_utils.choose_id_and_name_from_dict_list(
            data_model_list, 'Please choose a data model:')
...
#D
    def publish(self, query_name: str=None) -> str:
        ...
        if query_name:
            ...
            data = {}
        u = f'{self.__atconn._design_org_endpoint()}/project/{self.project_id}/publish'
        h = self.__atconn._generate_headers()
        d = json.dumps(data)
        response = request_utils.post_request(url=u, data=d, headers=h)
        ...
        return self.__published_project_id
#D
    def unpublish(self, published_project_id: str) -> str:
        ...
        """
        u = f'{self.__atconn.server}:{self.__atconn.engine_port}/projects/orgId/{self.__atconn.organization}/schema/{published_project_id}'
        h = self.__atconn._generate_headers()
        response = request_utils.delete_request(url=u, headers=h)
    ...
    def _update_project(self, project_json: dict, publish=True):
        ...
        url = self.__atconn._design_org_endpoint(
            suffix=f'/project/{self.project_id}')
        h = self.__atconn._generate_headers()
        try:
            request_utils.put_request(url, data=json.dumps(project_json), headers=h)
            if publish is True:
                self.publish()
...
#D
    def delete(self) -> None:
        ...
        for published_project in published_project_list:
            ...
            self.unpublish(published_project_id)
        # now we delete the project
        u = self.__atconn._design_org_endpoint('/project/' + self.project_id)
        h = self.__atconn._generate_headers()
        # I'll let the delete_request raise any errors in
        # check_request which is called by not passing in raises=false
        response = request_utils.delete_request(url=u, headers=h)
    ...
    def _update_project_tables(self, tables=None, publish=True):
        ...
        for dataset in datasets:
            ...
            if 'tables' in dataset['physical']:
                ...
                url = f'{self.__atconn._warehouse_endpoint()}/conn/{conn}/tables/cacheRefresh'
                h = self.__atconn._generate_headers()
                response = request_utils.post_request(url, headers=h)
                for table in project_tables:
                    if tables is None or table['name'] in tables:
                        ...
                        url = f'{self.__atconn._warehouse_endpoint()}/conn/{conn}/table/{table["name"]}/info{info}'
                        h = self.__atconn._generate_headers()
                        response = request_utils.get_request(url, headers=h)
...
#D
    def clone(self, clone_name: str, publish: bool = True):
        ...
        cloned_dict = project_utils.clone_dict(self.__atconn, self.__project_id, clone_name)
    ...
    def create_snapshot(self, snapshot_name: str) -> str:
        ...
        tag = {
            'tag': snapshot_name}
        response = request_utils.post_request(url, data=json.dumps(tag), headers=self.__atconn._generate_headers())
    ...
    def delete_snapshot(self, snapshot_id: str):
        ...
        request_utils.delete_request(url, headers=self.__atconn._generate_headers())
    ...
    def restore_snapshot(self, snapshot_id: str):
        ...
        request_utils.get_request(url, headers=self.__atconn._generate_headers())
    ...
    def return_snapshot_id(self, snapshot_name: str = None):
        ...
        request_resp = request_utils.get_request(url, headers=self.__atconn._generate_headers()).text
    ...
    def delete_dataset(self,
        ...
        """  Deletes any dataset with the given name, case-sensitive from all cubes. Also deletes measures built from
        ...
        #Delete dataset from cube and find measures to delete
        features_to_delete = []
        for index, cube in enumerate(project_parser.get_cubes(project_dict=temp_json)):
            cube_datasets = data_model_parser._get_cube_datasets(cube_dict=cube)
            new_cube_datasets = []
    ...
    def _assert_published_project_id_not_none(self, method: Optional[str] = None):
        ...
        if self.__published_project_id is None:
            raise atscale_errors.UserError(f'The project needs to be published before calling th{method_name} method')
