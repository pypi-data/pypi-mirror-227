...
class Databricks(SQLAlchemyConnection):
    ...
    def __init__(self, host:str, catalog:str, schema:str, http_path:str, token:str = None, port:int = 443):
        ...
        Args:
            ...
            catalog (str): The catalog of the intented Databricks connections
        ...
        if catalog != 'hive_metastore':
            logger.warn(f'Catalogs other than "hive_metastore" are not fully supported. Current catalog is "{catalog}"')
    ...
    def catalog(self, value):
        if value != 'hive_metastore':
            logger.warn(f'Catalogs other than "hive_metastore" are not fully supported. Current catalog is "{value}"')
        self._catalog = value
        self.dispose_engine()
    ...
    def _create_table(self, table_name: str, types: dict, cursor):
        ...
        if not cursor.tables(table_name=table_name, table_types=['TABLE']).fetchone():
            operation = "CREATE TABLE `{}`.`{}`.`{}` (".format(self.catalog,
    ...
    def write_df_to_db(self, table_name: str, dataframe: DataFrame, if_exists: PandasTableExistsActionType = PandasTableExistsActionType.FAIL, chunksize: int = 250):
        ...
        if cursor.tables(table_name=table_name, schema_name=self.schema, catalog_name= self.catalog).fetchone():
            exists = True
        else:
            exists = False
    ...
    def _write_pysparkdf_to_spark_db(self, pyspark_dataframe, table_name: str,
    ...
    def _read_pysparkdf_from_spark_db(self, spark_session, query: str):
        """Writes the provided pyspark DataFrame into the provided table name via standard spark sql operation. Can pass in if_exists to indicate the intended behavior if
            the provided table name is already taken.
        ...
        try: 
            ...
            logger.error(f'Unable to set default catalog to {self.catalog}, please verify the catalog of the databricks object')
            raise 
        ...
        try: 
            ...
            logger.error(f'Unable to set default database to {self.schema}, please verify the schema of the databricks object')
            raise
    ...
    def _create_table_path(self, table_name: str) -> str:
