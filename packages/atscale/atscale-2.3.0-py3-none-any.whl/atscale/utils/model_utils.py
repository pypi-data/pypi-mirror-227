...
def _check_conflicts(
    ...
    if preexisting is not None:
        ...
        preexisting = dmh._get_unpublished_features(project_dict=project_dict,
    ...
    if len(conflicts) > 0:
        if errmsg:
            ...
            errmsg = f'Invalid feature names: {conflicts}. Query name collision with existing features.'
        raise atscale_errors.UserError(errmsg)


def _check_features(features: Union[list, dict, set], check_list: Union[list, dict, set], errmsg: str = None,
                    diff_in_msg: bool = True):
    ...
    if len(set_dif) > 0:
        if errmsg:
            ...
            raise atscale_errors.UserError(
                f'Features: {sorted(list(set_dif))} not in data model. Make sure each feature has been published and is'
                ' correctly spelled')
    return True


def _create_dataset_relationship(atconn,
                                 project_dict,
                                 cube_id,
                                 database,
                                 schema,
    ...
    Args:
        ...
        roleplay_features (list, optional): The roleplays to use on the relationships. List must be either
            ...
            Defaults to None.
        table_columns (list, optional): The columns in the table to use for the dataset as returned by get_table_columns. Defaults to None.
    ...
    if not dataset_id:  # then we have to create a project_dataset
        ...
        if table_columns is None:
            table_columns = atconn.get_table_columns(warehouse_id=warehouse_id,
                                                    ...
                                                    database=database,
                                                    schema=schema)
        existing_datasets = [x['name'] for x in project_parser.get_datasets(project_dict= project_dict)]
        ...
        project_dataset, dataset_id = project_utils.create_dataset(table_name, 
                                                                   warehouse_id, 
                                                                   table_columns, 
                                                                   database, 
                                                                   schema, 
...
def _create_hinted_agg_for_qds(project_dict: dict, cube_id: str, dataset_name: str):
    """ Flags the given qds dataset to create a hinted aggregate. 

    Args:
        project_dict (dict): The project_dict to mutate and return.
        cube_id (str): The id of the cube the dataset belongs to.
        dataset_name (str): The name of the dataset. This dataset must exist in project_datasets.

    Returns:
        Dict: The mutated project_dict
        """
    dataset_id = project_parser.get_dataset(project_dict=project_dict,
                                            dataset_name=dataset_name)['id']
    cube_dict = project_parser.get_cube(project_dict, cube_id)

    for dataset_ref in data_model_parser._get_dataset_refs(cube_dict=cube_dict):
        if dataset_ref['id'] == dataset_id:
            found = True
            dataset_ref.setdefault('properties', {})
            dataset_ref['properties']['create-hinted-aggregate'] = True
            break
    if not found:
        data_set_ref = templates.create_dataset_ref_dict(
            dataset_id, [], [], create_hinted_aggregate=True)
...
def _create_dataset_relationship_from_dataset(project_dict: dict,
    ...
    for join_feature, join_column, roleplay_feature in zip(join_features, join_columns, roleplay_features):
        ...
        # TODO: This intoduces a bug if the index was 0, we should add a test
...
def _create_semantic_model(atconn: Connection, dbconn: SQLConnection, table_name: str, project_dict: dict, cube_id: str, dataset_id: str, columns: list, generate_date_table: bool):
    ...
    Args:
        ...
        generate_date_table (bol): whether to generate a date dimension table. False will use degenerate dimensions for dates
    """
    date_cols = []
    ...
    dataset = project_parser.get_dataset(project_dict=project_dict,
    ...
    for (column_name, column_type) in date_cols:
        ...
        if not generate_date_table:
            ...
            # else:
                ...
                logger.warning(f"Unable to determine TimeLevels in create_semantic_model for column {column_name} and db type {dbconn.platform_type}. The error was{e}")
                #skip the rest and go to the next column in the loop
                continue
            dimension_utils.create_time_dimension_for_column(atconn=atconn, project_dict=project_dict, cube_id=cube_id, 
                dataset_id=dataset_id, column_name=column_name, time_levels=time_levels)
        else:
            ...
            if column_type != 'date':
                ...
                project_utils.add_calculated_column_to_project_dataset(
                        atconn=atconn, data_set=dataset, column_name=join_column, expression=enums.TimeLevels.Day.get_sql_expression(column_name, platform_type))
            if not date_key_feature:
                ...
                try:
                    date_table_name = db_utils.get_atscale_tablename(atconn=atconn, warehouse_id=warehouse_id, 
                                                                     database=database, schema=schema, table_name='atscale_date_table')
                except atscale_errors.UserError:
                    dbconn._generate_date_table()
                    date_table_name = db_utils.get_atscale_tablename(atconn=atconn, warehouse_id=warehouse_id, 
                                                                     database=database, schema=schema, table_name='atscale_date_table')
                columns = atconn.get_table_columns(warehouse_id=warehouse_id, table_name=date_table_name, database=database, schema=schema)
                date_dataset_dict, date_dataset_id = project_utils.create_dataset(date_table_name, warehouse_id, columns, database, schema)
                project_utils.add_dataset(project_dict, date_dataset_dict)
                date_key_feature = dimension_utils.create_time_dimension_from_table(project_dict, cube_id, date_dataset_id)
            if len(date_cols) > 1:
                roleplay_features=[f'{join_column} {{0}}']
            else:
                roleplay_features=['']
            dataset_name = project_parser.get_dataset(project_dict=project_dict,
                                                      dataset_id=dataset_id)['name']
            join_features = [date_key_feature]
            join_columns=[[join_column]]
            _create_dataset_relationship_from_dataset(project_dict=project_dict,cube_id=cube_id,dataset_name=dataset_name,
                                                      join_features=join_features,join_columns=join_columns,roleplay_features=roleplay_features)
...
def _get_fact_datasets(data_model_dict: dict, project_dict: Dict) -> List[dict]:
    ...
    fact_datasets = dictionary_parser.filter_list_of_dicts(dict_list=all_datasets,
                                                           by={
                                                               'id': id_set})
    return fact_datasets


def _get_fact_dataset(data_model_dict: Dict,
                      project_dict: Dict = None,
    ...
    return dictionary_parser._find_by_id_or_name(item_list=fact_dset_list,
                                                 item_id=dataset_id,
                                                 item_name=dataset_name)


def _get_dimension_datasets(data_model: DataModel, project_dict: Dict) -> List[dict]:
...
def _get_column_names(project_dict: Dict, dataset_name: str) -> List[str]:
    ...
    dataset_of_int = project_parser.get_dataset(project_dict=project_dict,
                                                dataset_name=dataset_name)
...
def _validate_mdx_syntax(atconn: Connection, expression: str, raises = True) -> str:
    """Passes an MDX expression to the engine for validation

    Args:
        atconn (Connection): the connection to use for validation
    ...
    data = {
        'formula': expression}
    response = atconn._submit_request(request_type=enums.RequestType.POST, url=url,
