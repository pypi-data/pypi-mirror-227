...
def supported_bulk_operations(function_to_check: Callable) -> Callable:
...
def bulk_create_aggregate_feature(atconn: Connection, project_dict: Dict, feature_dict: Dict,
                                  data_model_id: str, parameter_list: List[Dict]):
    ...
    """
    from atscale.data_model import DataModel
    from atscale.utils.feature_utils import _create_aggregate_feature_local
    ...
    validation_utils.validate_all_expected_params_bulk(passed_vars= working_parameter_list, 
    ...
    if dupes_found:
        starting_err_msg = f'The following duplicated {feature_name_column} parameters were passed. '\
    ...
    existing_query_names_index = [(index, value) for index, value in
                                   enumerate(features_to_create) if value in feature_dict]
    if existing_query_names_index:
        starting_err_msg = f'The following {feature_name_column} parameters were passed that collide with an '\
    ...
    for index, parameter_dict in enumerate(working_parameter_list):
        ...
        if dataset_of_int in valid_datasets:
            ...
            dset = model_utils._get_fact_dataset(data_model_dict, project_dict,
                                                 dataset_name= dataset_of_int)
            if not dset:
                invalid_datasets[index] = dataset_of_int
                continue
            else:
                dset_columns = model_utils._get_column_names(project_dict= project_dict,
                                                            dataset_name= dataset_of_int)
                if column_of_int not in dset_columns:
                    invalid_columns[index] = (column_of_int, dataset_of_int)
                    continue
                valid_datasets[dataset_of_int] = {'id': dset['id'], 'cols': dset_columns}

        parameter_dict['dataset_id'] = valid_datasets[dataset_of_int]['id']
    
    #raise the appropriate errors
    if invalid_datasets:
        starting_err_msg = 'The following fact_dataset_name parameters were passed do not exist in the Data Model.'
        for index_val, dataset in invalid_datasets.items():
            starting_err_msg +=f'\n\tFact Dataset {dataset} appeared at index: {index_val}'
        raise atscale_errors.UserError(starting_err_msg)
    if invalid_columns:
        starting_err_msg = 'The following column_name parameters were passed do not exist in the given dataset.'
    ...
    for parameter_dict in working_parameter_list:
        ...
        _create_aggregate_feature_local(**parameter_dict)


def bulk_create_calculated_column(atconn: Connection ,project_dict: Dict, feature_dict: Dict,
                                  data_model_id: str, parameter_list: List[Dict]):
    ...
    #raise the appropriate errors
    if invalid_datasets:
        starting_err_msg = f'The following dataset_name parameters were passed do not exist in the Data Model.'
        for index_val, dataset in invalid_datasets.items():
            starting_err_msg +=f'\n\tDataset {dataset} appeared at index: {index_val}'
        raise atscale_errors.UserError(starting_err_msg)
    if invalid_qds_datasets:
        starting_err_msg = f'The following dataset_name parameters were passed that match to a qds,'\
    ...
    for index, parameter_dict in enumerate(working_parameter_list):
        ...
        try:
            ...
            if 'SQL compilation error' in str(err):
                sql_compilation_errors.append(f'\n\tSQL Compilation Error '
                                              f'found at index {index}: {str(err)}')
            else:
                raise err
...
def bulk_create_calculated_feature(atconn: Connection ,project_dict: Dict, feature_dict: Dict,
                                  data_model_id: str, parameter_list: List[Dict]):
    ...
    """
    from atscale.data_model import DataModel
    from atscale.utils.feature_utils import _create_calculated_feature_local
    ...
    for index, parameter_dict in enumerate(working_parameter_list):
        ...
        mdx_invalid = model_utils._validate_mdx_syntax(atconn= atconn, expression= parameter_dict['expression'],
                                                     raises= False)
        if mdx_invalid:
    ...
    for parameter_dict in working_parameter_list:
        ...
        #left off here, need to test passing with ** and make sure it checks the key names as param names
        _create_calculated_feature_local(**parameter_dict)
