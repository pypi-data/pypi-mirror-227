...
from atscale.base.enums import LevelType, RequestType, FeatureType
from atscale.base import endpoints

logger = logging.getLogger(__name__)


def _generate_atscale_query(data_model, feature_list: List[str], filter_equals: Dict[str, str] = None,
                            ...
                            filter_not_equal: Dict[str, str] = None, filter_in: Dict[str, List[str]] = None,
                            ...
                            use_aggs: bool = True, gen_aggs: bool = True, raise_multikey_warning: bool = True) -> str:
    ...
    for maybe_tuple in order_by:
        if not (isinstance(maybe_tuple, tuple) and len(maybe_tuple) == 2 and isinstance(maybe_tuple[0], str)
                and isinstance(maybe_tuple[1], str) and maybe_tuple[1].upper() in ['ASC', 'DESC']):
    ...
    if error_items:
        raise atscale_errors.UserError(f'All items in the order_by parameter must be a tuple of a '
                                       f'feature name then "ASC" or "DESC". The following do not '
                                       f'comply: {error_items}')

    all_features = data_model.get_features()
    list_all = set(all_features)  # turns it into a set of the keys and has constant time lookup on average
    ...
    for f in feature_list:
        if f not in feature_set:
            ...
            repeats.append(f)
    if repeats:
        logger.info(f'The following feature names appear more than once in the feature_list parameter: {repeats}. '
    ...
    if raise_multikey_warning:
        ...
        features_to_validate = {f: categoricals[f].get('base_name', f) for f in feature_list if f in categoricals}
        ...
        if features_to_validate:
            ...
            for feature_name, base_name in features_to_validate.items():
                ...
                if len(key_columns) > 1:
                    logger.warning(f"Feature: {feature_name} has a compound key, "
    ...
    if ordering_strings:
        ...
        categorical_columns = [f'`{name}`' for name, metadata in all_features.items() if
                               metadata['feature_type'].upper() == FeatureType.CATEGORICAL.name]
...
def generate_db_query(data_model, atscale_query: str,
    ...
    atconn._post_atscale_query(query=inbound_query,
    ...
    url = endpoints._endpoint_query_view(atconn=atconn,suffix=
            ...
            for event in query_info['timeline_events']:
                if event['type'] == 'SubqueriesWall':
                    ...
                    if event['children'][0]['query_text_truncated']:
                        url = endpoints._endpoint_design_private_org(atconn=atconn, suffix=
