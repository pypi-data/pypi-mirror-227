...
from atscale.utils import query_utils, dmv_utils, model_utils, db_utils, validation_utils
from math import e
...
def write_udf_to_qds(data_model: DataModel,
    ...
    feature_query: str = query_utils.generate_db_query(data_model=data_model,
                                                       ...
                                                       use_aggs=False)
    categorical_inputs: List[str] = [feat for feat in feature_inputs if
                                     feat_dict[feat]['feature_type'] == 'Categorical']
    ...
    data_model.add_query_dataset(dataset_name=f'{new_feature_name}_QDS',
                                   query=qds_query,
...
def join_udf(data_model: DataModel,
    ...
    warehouse_id = validation_utils._validate_warehouse_id_parameter(project=data_model.project,
    ...
    warehouse_id = validation_utils._validate_warehouse_id_parameter(project=data_model.project,
                                                                     warehouse_id=warehouse_id)
    data_model.add_query_dataset(warehouse_id=warehouse_id,
                                   dataset_name=qds_name,
                                   query=qds_query,
                                   join_features=join_features,
                                   join_columns=join_columns)
    columns = data_model.project.atconn.get_query_columns(warehouse_id=warehouse_id,
                                                          query=qds_query)
    target_column_names = []
    for i in range(len(target_columns)):
        target_column_names.append(columns[i][0])
    for new_feature in target_column_names:
        data_model.create_aggregate_feature(fact_dataset_name=qds_name,
