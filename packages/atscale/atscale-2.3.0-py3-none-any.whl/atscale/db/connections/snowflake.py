...
class Snowflake(SQLAlchemyConnection):
    ...
    def __init__(self, username: str, account: str, warehouse: str, database: str, schema: str, 
                 password: str = None, role: str = None, authenticator: str = 'snowflake', 
        ...
        Args:
            ...
            private_key (bytes, optional): the private key to authenticate with instead of a password. Defaults to None to use password.
        """
        ...
        super().__init__()  # put super constructor after checking imports since base class doesn't prompt to install
        ...
        self._schema = schema
        if private_key:
            self._private_key = cryptocode.encrypt(private_key.decode('utf-8'), self.platform_type.value)
        else:
    ...
    def private_key(self, value):
        self._private_key = cryptocode.encrypt(value.decode('utf-8'), self.platform_type.value)
        self.dispose_engine()
    ...
    def _get_connection_url(self):
        ...
        if not self._private_key:
            ...
            connection_url = URL.create('snowflake', username=self._username,
                                password=password, host=self._account,
                                database=self._database + '/' + self._schema)
        else:
            connection_url = URL.create('snowflake', username=self._username, host=self._account,
                                database=self._database + '/' + self._schema)
    ...
    def _get_connection_parameters(self):
        ...
        if self._private_key:
            parameters['private_key'] = bytes(cryptocode.decrypt(self._private_key, self.platform_type.value), 'utf-8')
        return parameters
    ...
    def write_df_to_db(self, table_name, dataframe: pd.DataFrame, if_exists: PandasTableExistsActionType = PandasTableExistsActionType.FAIL, chunksize=10000):
        ...
        bad_time_types = [name for name, val in dataframe_datatypes.items() if val.name == 'datetime64[ns]']
    ...
    def _write_pysparkdf_to_external_db(self, pyspark_dataframe, jdbc_format: str, jdbc_options: Dict[str, str],
        ...
        super()._write_pysparkdf_to_external_db(pyspark_dataframe_renamed, jdbc_format, jdbc_options, fixed_table_name,if_exists)
