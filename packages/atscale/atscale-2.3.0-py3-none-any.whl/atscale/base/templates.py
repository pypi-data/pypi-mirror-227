...
from atscale.base.enums import Aggs, MappedColumnKeyTerminator, MappedColumnFieldTerminator, MappedColumnDataTypes, TimeLevels, LevelType
...
def create_dataset_ref_dict(dataset_id, key_refs=None, attribute_refs=None, allow_aggregates=True, create_hinted_aggregate=False):
    dataset = {
    ...
    if create_hinted_aggregate:
        dataset['properties']['create_hinted_aggregate'] = True
    return dataset
...
def create_keyed_attribute_dict(attribute_id: str, key_ref: str, name: str, visible, ordering: str = None, caption=None,
                                description=None, folder=None):
    if caption is None:
        caption = name
    keyed_attr = {
        'id': attribute_id,
...
def create_map_column_dict(columns: List[dict], field_terminator: MappedColumnFieldTerminator,
                           key_terminator: MappedColumnKeyTerminator,
                           first_char_delim: bool, map_key_type: MappedColumnDataTypes,
                           map_value_type: MappedColumnDataTypes, column_name: str):
    return {
        'columns': {
            'columns': columns
            },
        ...
        'map-key': {
            'type': map_key_type.value
            },
        'map-value': {
            'type': map_value_type.value
            },
        'name': column_name
        }
...
def create_hierarchy_level_dict(visible: bool, level_id: str, keyed_attribute_id: str, level_type: TimeLevels=None):
    properties = {
            'unique-in-parent': False,
            'visible': visible
        }
    if level_type:
        properties['level-type'] = level_type.atscale_type
    else:
...
def create_dimension_dict(hierarchy_dict: dict, dim_id: str, name: str,
                          visible: bool, time_dimension: bool = False, participating_datasets: List[str] = None):
    properties = {
            'visible': visible
        }
    if time_dimension:
...
def create_measure_dict(measure_id: str, measure_name: str, agg_type: Aggs,
    ...
    if agg_dict != {}:
        ...
        if key_ref is None:
            raise Exception(f'A key-ref id must be made and passed into this function in order to make a valid '
                            f'{agg_type.name} measure dict')
    else:
        properties = {
            'type': {
                'measure': {
                    'default-aggregation': agg_type.value}
                }
            }
...
def create_dataset_dict(dataset_id: str, dataset_name: str, table_name: str, 
...
def create_query_dataset_dict(dataset_id: str, dataset_name: str,
                              warehouse_id: str, columns: List[dict],
                              allow_aggregates: bool, query: str):
    dataset = {
        'id': dataset_id,
        'name': dataset_name,
        'properties': {
...
def create_query_for_post_request(query: str, project_name: str, organization: str, use_aggs=True, gen_aggs=False,
                                  fake_results=False,
    ...
    return {
        ...
        'aggregation': {
            'useAggregates': use_aggs,
            'genAggregates': gen_aggs
        },
