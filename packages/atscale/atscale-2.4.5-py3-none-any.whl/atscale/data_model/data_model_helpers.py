            ...
            if len(key_cols) == 1 and key_cols[0] != value_col and join_column[0] != key_cols[0]:
                if join_column[0] != value_col and join_column[0] != join_feature:
                    ...
                    if not value:
                        continue
                if use_spark:
                    df_key = db_utils._get_key_cols(key_dict[join_feature], spark_session=df.sparkSession)
                else:
                    df_key: pd.DataFrame = db_utils._get_key_cols(key_dict[join_feature], dbconn=dbconn)
