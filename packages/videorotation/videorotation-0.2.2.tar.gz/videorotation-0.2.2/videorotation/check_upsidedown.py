...
class VideoRotationAnalysis:
    ...
    def is_flipped(self, image_path):
        ...
        return bool(np.argmax(prediction)), prediction
    ...
    def check_if_upsidedown_for_video(self, video):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for path in frame_paths:
                flipped,score = self.is_flipped(path)
                scores_sums[0]+=score[0]
                scores_sums[1]+=score[1]
                result.append(flipped)
            print("predict: ", time.time()-time_s)

            return sum(1 for x in result if x) >= self.frames_threshold, sum(1 for x in result if x), scores_sums