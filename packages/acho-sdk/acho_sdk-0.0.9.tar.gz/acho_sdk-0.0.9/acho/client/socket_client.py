...
class SocketClient:
    ...
    @sio.event(namespace='/soc')
    async def notebook_detect(self, params):
        print(params)
        print('notebook detection request')
        app_version_id = params.app_version_id
        await self.sio.emit('notebook_ready', {'app_version_id': app_version_id, 'notebook_name': self.notebook_name})
