...
from .keys import LABEL, ATTRIBUTES, REF
...
class DatalakeClient:
    ...
    def create_system_label(self, label_dict):
        _label = {}
        _label[label_dict[LABEL]] = {}

        if ATTRIBUTES in label_dict:
            for key in label_dict[ATTRIBUTES]:
                attrs = label_dict[ATTRIBUTES][key]
                if len(attrs) == 0:
                    raise ValueError(f"No values for attribute {key}")
                _label[label_dict[LABEL]][key] = attrs

        response = self.datalake_interface.find_datalake_label_references(_label)
        _response = {
            "label_reference" : response[label_dict[LABEL]][REF]
        }
