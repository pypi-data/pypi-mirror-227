...
def add_message_to_db (data, table=messages_table):
    ...
    try:
        ...
        logger.debug(f"IN 'add message to db. TESTING_FLAG: {TESTING_FLAG}")
        
        event = data #.get('event', {})
        
        # No user when it's an action response to a block. Skip it.
        if not event.get('user'):
            logger.info(f"SKIPPING: block action event (or bot_id instead of user)")
            return None
        
        if not TESTING_FLAG and (event.get('user') in EXCLUDED_SLACK_IDS or event.get('user') == 'USLACKBOT' or event.get('bot_id')):
            logger.info(f"SKIPPING: MESSAGE FROM EXCLUDED BOT/USER: {event.get('user')}; {event.get('bot_id')}")
            return None
