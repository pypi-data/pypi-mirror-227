...
class YoutubeCommentCrawler(YoutubeBaseCrawler):
    ...
    async def crawl(self, video_url: str, n_target: (int, None) = None, ret_meta_info: bool = False):
        ...
        """
        assert YoutubeUrlParser.is_video_url(video_url) is True, f"The url {video_url} is not a video url."

        await self._browser_agent.go_video_page(video_url=video_url, initial_scroll=10)
