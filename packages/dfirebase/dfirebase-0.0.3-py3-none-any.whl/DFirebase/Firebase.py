...
class Firebase:
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.authDomain         = config.get('authDomain')
        self.databaseURL        = config.get('databaseURL')
        self.projectId          = config.get('projectId')
        self.storageBucket      = config.get('storageBucket')
        self.messagingSenderId  = config.get('messagingSenderId')
        self.appId              = config.get('appId')
        self.measurementId      = config.get('measurementId')
        self.serviceAccount     = config.get('serviceAccount')
        if self.serviceAccount is not None:
            ...
            service_account_type = type(self.serviceAccount)
            if service_account_type is str:
                self.credentials = ServiceAccountCredentials.from_json_keyfile_name(self.serviceAccount, scopes)
            if service_account_type is dict:
                self.credentials = ServiceAccountCredentials.from_json_keyfile_dict(self.serviceAccount, scopes)
