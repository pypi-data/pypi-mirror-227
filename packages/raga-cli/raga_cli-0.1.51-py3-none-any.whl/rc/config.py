    ...
    def validate_default_section(self, config):
        ...
        for option, default_value in DEFAULT_CONFIG_VALUES.items():
            if option not in default_section or default_section[option] == default_value:
                raise ValueError(f"Please update the value of '{option}' in the [{self.DEFAULT_PROFILE}] section of the config file. You can update using this command `sudo vim {self.CONFIG_FILE_PATH}`")
