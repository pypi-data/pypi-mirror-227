...
def get_scan_browser():
    js_bridge_scan={}
    js_bridge_scan["scan_mode"]="file"
    js_bridge_scan["history"] = []
    ...
    def while_window(event):
        ...
        if event == '-SCAN-':
            js_bridge_scan["last_scan"]=is_file(values['-DIR-'])
            scan_results = scan_directory(return_directory(), js_bridge_scan["scan_mode"])
        ...
        if event == '-MODE-':
            js_bridge_scan["scan_mode"] = 'folder' if js_bridge_scan["scan_mode"] == 'file' else 'file'
            window.Element('-MODE-').update(text=f"F{js_bridge_scan['scan_mode'][1:]}  Scan Mode")

        if event == "<-":
            # Navigate up to the parent directory
            if return_directory() not in js_bridge_scan["history"]:
                js_bridge_scan["history"].append(return_directory())
            directory = os.path.dirname(return_directory())  # This will give the parent directory
            window['-DIR-'].update(value=directory)
            window['-FILES-'].update(values=scan_directory(directory, js_bridge_scan["scan_mode"]))
        if event == "->":
            ...
            elif js_bridge_scan["history"]:  # If there's a directory in the history stack
                next_directory = js_bridge_scan["history"].pop()
                window['-DIR-'].update(value=next_directory)
                window['-FILES-'].update(values=scan_directory(next_directory, js_bridge_scan["scan_mode"]))
...
def create_window_manager(script_name='default_script_name',global_var=globals()):
    ...
    js_global_bridge = bridge.retrieve_global_variables(script_name, global_var)
    js_bridge = bridge.return_global_variables(script_name)
    return WindowManager(script_name, bridge),js_bridge,script_name
