...
class ForAll(Quantifier):
    ...
    def __new__(cls, function, *symbols, is_in_exists=False, **assumptions):
        ...
        if assumptions:
            ...
            return Inference(ForAll.__new__(cls, function, *symbols, evaluate=evaluate), **assumptions)
        ...
        return ExprWithLimits.__new__(cls, function, *symbols, evaluate=evaluate, **assumptions)
...
#         return '\N{FOR ALL}[%s](%s)' % (limits, p.doprint(self.expr))
        return 'All[%s](%s)' % (limits, p.doprint(self.expr))
