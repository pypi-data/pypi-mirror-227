...
def baseline_correction(spectra_df: pd.DataFrame, lambda_=100, porder=1, maxIter=100):
    """
    Apply the ALS algorithm to each row of a DataFrame.
    
    Parameters:
    - df (DataFrame): DataFrame where each row is a spectrum to be baseline corrected.
    - lambda_ (float): Regularization parameter for ALS.
    - p (float): Asymmetry parameter for ALS.
    - maxIter (int): Maximum number of iterations for ALS.
