...
class EdgeSoftmax_v2(th.autograd.Function):
    @staticmethod
    def forward(ctx, graph, efficient_score, dim, device):
        num_ecount = graph.get_edge_count()

        out = gp_apis.gp_edge_softmax_forward(graph, efficient_score, num_ecount, dim, 0, device)
    ...
    def backward(ctx, dZ):
        ...
        ctx.backward_cache = None
        num_ecount = graph.get_edge_count()
        grad_score = gp_apis.gp_edge_softmax_backward(graph, dZ, out, num_ecount, dim, 0, device)
...
def edge_softmax_v2(graph, efficient_score, num_vcount, dim, device):
    return EdgeSoftmax_v2.apply(graph, efficient_score, dim, device)
