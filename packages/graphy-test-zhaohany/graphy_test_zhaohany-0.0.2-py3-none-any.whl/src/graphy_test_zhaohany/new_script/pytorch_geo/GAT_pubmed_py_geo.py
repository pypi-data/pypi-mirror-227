...
def build_py_geo_graph(src, dst, num_node):
    # while the other for destination endpoints.
    edge_index = torch.tensor([src, dst], dtype=torch.long)
    adj = SparseTensor(row=edge_index[0], col=edge_index[1],sparse_sizes=(num_node, num_node)).t()
...
class GAT(torch.nn.Module):
    def __init__(self, in_channels, out_channels, num_heads):
        ...
        self.conv2 = GATConv(16 * 3, 16, heads= num_heads, dropout=0.0)
        self.conv3 = GATConv(16 * 3, 16, heads= num_heads, dropout=0.0)
        self.conv4 = GATConv(16 * 3, out_channels, heads=1, concat=False, dropout=0.0)
...
if __name__ == "__main__":
    ...
    graph = build_py_geo_graph(src, dst, num_node)
    # print("graph size", graph.size())
    g_end = datetime.datetime.now()
    diff = g_end - g_start
    print ('graph creation time is:', diff)
    graph=graph.to(cuda)
    net = GAT(input_feature_dim, 3, 3)
    #net = GCN(graph, input_feature_dim, 16, 6, 3, None, 1)
    net.to(cuda)
    feature = pubmed_util.read_feature_info("/home/ygong07/data/test2/pubmed/feature/feature.txt")
