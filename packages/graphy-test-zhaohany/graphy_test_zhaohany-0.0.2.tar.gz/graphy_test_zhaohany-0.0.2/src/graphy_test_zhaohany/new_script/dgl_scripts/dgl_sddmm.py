...
if __name__ == "__main__":
    ...
    #print("111", num_vcount, num_heads)
    score_max = torch.ones(num_vcount, num_heads)
    score_max = score_max.to(device)
    g_start = datetime.datetime.now()
    score = _gsddmm(graph._graph, 'sub', score, score_max, 'e', 'v')
    g_end = datetime.datetime.now()
    diff = g_end - g_start
    print ('sddmm time is:', diff) 
    #print("sddmm done ", score)
    res_load = torch.load('/home/ygong07/Graphy_workload_compare/GraphPy_workflow/new_script/test/sddmm2d_result.pt')
