...
if __name__ == "__main__":
    ...
    if test_gcn:
        G = gp_apis.load_graph_noeid(ofile);
    else:
        G = gp_apis.load_graph(ofile);
    ...
    for num_heads in num_heads_list:
        if test_gcn:
            ...
            torch.save(X, 'data_save/spmm_X.pt')
            X = X.to(device)
            norm = 1
            ...
            print("res is", res)
            torch.save(res, 'data_save/spmm_res.pt')
            """
            cal = 0
            b = res1.tolist()
            for each in b:
            ...
            torch.save(X, 'data_save/spmmw_op2d_X.pt')
            X = X.to(device)
            Y = torch.rand(num_ecount, num_heads)
            torch.save(Y, 'data_save/spmmw_op2d_Y.pt')
            Y = Y.to(device)
            g_start = datetime.datetime.now()
            ...
            print("res is", res)
            torch.save(res.to("cpu"), 'data_save/spmmw_op2d_res.pt')
            ...
            torch.save(X, 'data_save/sddmm2d_X.pt')
            X = X.to(device)
            Y = torch.ones(num_ecount, num_heads)
            torch.save(Y, 'data_save/sddmm2d_Y.pt')
            Y = Y.to(device)
            g_start = datetime.datetime.now()
            ...
            print("res",res)
            torch.save(res, 'data_save/sddmm2d_res.pt')

            # sddmme2d
            dim0 = num_ecount
            dim1 = 1
            X = torch.rand(num_vcount, num_heads, 1)
            torch.save(X, 'data_save/sddmme2d_X.pt')
            X = X.to(device)
            Y = torch.rand(num_vcount, num_heads, 1)
            torch.save(Y, 'data_save/sddmme2d_Y.pt')
            Y = Y.to(device)
            g_start = datetime.datetime.now()
            ...
            print("res is", res)
            torch.save(res.to("cpu"), 'data_save/sddmme2d_res.pt')
            ...
            torch.save(X, 'data_save/spmmw2d_X.pt')
            X = X.to(device)
            g_start = datetime.datetime.now()
            ...
            print("res is", res)
            torch.save(res.to("cpu"), 'data_save/spmmw2d_res.pt')
