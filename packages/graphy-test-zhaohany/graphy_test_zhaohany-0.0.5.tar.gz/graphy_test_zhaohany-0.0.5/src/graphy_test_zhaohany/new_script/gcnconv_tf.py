...
class GraphConv(layers.Layer):
    def __init__(self,
        ...
        if bias:
            ...
            self.bias = tf.Variable(initial_value=zeroinit(shape=(out_feats), dtype='float32'), trainable=True)
        else:
            self.bias = None
    ...
    def call(self, graph, feat, weight=None):
        ...
        if self._in_feats > self._out_feats:
            ...
            if weight is not None:
                feat1 = tf.matmul(feat, weight)
                #print(feat1)
            #dim = feat1.size(1)
            dim = feat1.shape.as_list()
            dim = dim[1]
            #print("333")
            # feature means the vertex input_feature, rst is the output 1d numpy array
            rst = sparse.gspmv(graph, feat1, self._norm, num_vcount, dim)
            #print(rst)
        else:
            #dim = feat.size(1)
            dim = feat.shape.as_list()
            dim = dim[1]
            # aggregate first then mult W
            rst = sparse.gspmv(graph, feat, self._norm, num_vcount, dim)
            if weight is not None:
