...
'''
IP Rotator using AWS API Gateway
'''

ip_generator = ip_gen.IPGenerator()
...
ALL_REGIONS = EXTRA_REGIONS + ['ap-east-1', 'af-south-1', 'eu-south-1', 'me-south-1']
...
class ApiGateway(rq.adapters.HTTPAdapter):
    def __init__(
        ...
        **kwargs,
        ...
        self.site = site[:-1] if site.endswith('/') else site
    ...
    def send(self, request, *args, **kwargs):
        ...
        if x_forwarded_for is None:
            x_forwarded_for = ip_generator.generate('US')
    ...
    def delete_gateway(self, region, endpoints=None):
        ...
        if endpoints is not None:
            endpoint_ids.extend(endpoint.split('.')[0] for endpoint in endpoints)
    ...
    def start(self, force=False, require_manual_deletion=False, endpoints=None):
    ...
    def shutdown(self, endpoints=None):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            ...
            futures.extend(
                executor.submit(self.delete_gateway, region=region, endpoints=endpoints)
                for region in self.regions
            )
            # Check outputs
            deleted = []
