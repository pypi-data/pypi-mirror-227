...
def sendgm(gid,text):
    gid = str(gid)
    url = f'http://{str(posturl[0])}:{str(posturl[1])}/send_group_msg?'
    data = str(url) + f'group_id={gid}&message={text}'
...
def _hasmsg(msg):
    if msg['message_type'] == 'private':
        t1 = Thread(target=pmsg,args=(msg['user_id'],msg['message'],msg['sender']))
        t1.start()
    else:
        t2 = Thread(target=gmsg,args=(msg['group_id'],msg['user_id'],msg['message'],msg['sender']))
        t2.start()
