...
class FirstBankStatement(BankStatementReport):
    ...
    def __init__(self, pdf_directory, password, min_salary, max_salary):
        if pdf_directory is None or pdf_directory == '':
            pdf_directory = "pdfs/mobilefirstbank.pdf"
            # pdf_directory = "pdfs/firstbank3.pdf"
            password = "34932"
    ...
    def get_transactions_table_rows_bank_version(self, reader, page=0, on_first_page=False, on_last_page=False):
        modified_rows = []
        if page == 0:
        ...
        if on_last_page is True:
            modified_rows.pop(-1)
        return modified_rows

    def get_transactions_table_rows(self, reader, page=0, on_first_page=False, on_last_page=False):
        # first_page_tables = reader.pages[0].extract_tables()
        # mobile version only has one table on first page
        if self.mode == 1:
            # print("Mobile")
            return self.get_transactions_table_rows_mobile_version(reader, page, on_first_page, on_last_page)
        # none mobile version has more than one table on first page
        if self.mode >= 2:
            # print("Bank")
            return self.get_transactions_table_rows_bank_version(reader, page, on_first_page, on_last_page)

        raise Exception("All methods have been exhausted without producing the desired result.")
        ...
        for page_num in range(num_pages):
            try:
                ...
                if page_num == num_pages - 1:
                    at_last_page = True
                new_rows = self.get_transactions_table_rows(reader, page_num, at_first_page, at_last_page)
