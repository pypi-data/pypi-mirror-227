...
class CustomInstall(install):
    def run(self):
        if 'COLAB_GPU' not in environ and 'KAGGLE_KERNEL_RUN_TYPE' not in environ:
            ...
            for filename in listdir(path.join(install_dir, 'hurllm')):
                if filename.startswith('core') and filename.endswith('.pyc'):
                    new_file_name = path.join(install_dir, 'hurllm', 'core.pyc')
                    rename(path.join(install_dir, 'hurllm', filename), new_file_name)
