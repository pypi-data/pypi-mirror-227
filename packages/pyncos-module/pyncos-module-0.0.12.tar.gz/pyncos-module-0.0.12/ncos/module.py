...
class ModuleBase:
  ...
  def __init__(self):
    self._broker = None
    self._id = self.__id_genertor__(DATA_ROOT_PATH, DATA_LOCAL_FILE)
  ...
  def __uninstall__(self, reason:str):
    self.send_to_ncos(MODULE_UNINSTALL_TOPIC.format(**{'moduleId': self.id}), dumps({'moduleId': self.id, 'reason': reason}))
  ...
  def set_will(self, content:str):
    self._broker.set_will(MODULE_UNINSTALL_TOPIC.format(**{'moduleId': self.id}), dumps({'moduleId': self.id, 'reason': content}))
