...
class ModuleBase:
  ...
  def __init__(self):
    self._broker = None
    self._identity = self.__id_genertor__(DATA_ROOT_PATH, DATA_LOCAL_FILE)
  ...
  def __uninstall__(self, reason:str):
    self.send_to_ncos(MODULE_UNINSTALL_TOPIC.format(**{'moduleId': self._identity}), dumps({'moduleId': self._identity, 'reason': reason}))
  ...
  def bind_ncos_broker(self, host:str, port:int):
    ...
    self._broker.attach_connected(on_connected)
    self._broker.attach_messaged(on_messaged)
    self._broker.connect()
    
    
  def set_will(self, content:str):
    self._broker.set_will(MODULE_UNINSTALL_TOPIC.format(**{'moduleId': self._identity}), dumps({'moduleId': self._identity, 'reason': content}))
  ...
  def __topic__(self, topic):
    return topic.replace('${moduleId}', self._identity).replace('${slotName}', self.__slot_name__)
