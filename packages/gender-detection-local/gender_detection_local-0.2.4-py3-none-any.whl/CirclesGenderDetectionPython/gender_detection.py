...
FACIAL_ANALYSIS_LOCAL_COMPONENT_ID = "13"
...
logger.init("Start logging in {FACIAL_ANALYSIS_LOCAL_COMPONENT_NAME}", object=object_to_insert)
...
def create_model(input_shape):
    ...
    logger.end("create_model", object= {"CNNmodel":CNNmodel.summary()})
    return CNNmodel
...
def capture_write(filename="image.jpeg", port=0, ramp_frames=30, x=178, y=218):
    ...
    logger.start("capture_write", object= {"filename":filename,"port":port,"ramp_frames":ramp_frames,"x":x,"y":y})
    ...
    del(camera)
    logger.end("capture_write", object= {"im":im})
    return im
...
class GenderClassifier:
    ...
    def predict_gender(self,model_path,image_path=None):
        ...
        """
        logger.start("predict_gender", object= {"model_path":model_path,"image_path":image_path})
        ...
        logger.info("Prediction Confidence (>0.5 Male,<=0.5 Female): "+str(prediction))
        ...
        logger.end("predict_gender", object= {"result":result})
        return result
    ...
    def train_model(self, save=True):
        ...
        """
        logger.start("train_model", object= {"save":save})
        ...
        logger.info("loss: ",loss," accuracy: ", accuracy)
        if save is True:
            model.save("gender_detection", save_format='h5')

        logger.end("train_model")
