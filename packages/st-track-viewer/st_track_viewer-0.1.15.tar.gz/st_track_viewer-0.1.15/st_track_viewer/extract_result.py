...
def extract_points(
    ...
    version_path = base_folder_path + next(
        filter(
            lambda folder: folder.startswith("version"), os.listdir(base_folder_path)
        )
    )
    folders = list(
    ...
    from_points: List[Tuple[int, int], Tuple[int, int]] = []
    to_points: List[Tuple[int, int], Tuple[int, int]] = []
    for points in point_dicts:
        keys = points.keys()
        from_points.append(points.values())
