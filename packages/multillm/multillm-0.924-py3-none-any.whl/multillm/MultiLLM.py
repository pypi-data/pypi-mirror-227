...
try:
    ...
    print('(MultiLLM:warning) redis disabled: {0}' .format(str(e)))
...
class MultiLLM(object):
    ...
    def run(self, prompt, action_chain, rank_chain, taskid=None):
        ...
        """
        responses = {}
        """ Connect to Redis if taskid is specified """
        redisConnected = False
        redis_conn = None
        ...
        with multiprocessing.Pool() as pool:
            ...
            for model_name, future in model_and_future:
                try:
                    ...
                    #publish to redis
                    if redisConnected:
                        MultiLLM.publish_to_redis(redis_conn=redis_conn,
                                         type="multillm", 
                                         event="response", 
                                         taskid=taskid, 
                                         result=response)
    ...
    @staticmethod
    def publish_to_redis(redis_conn=None, type="testguru", event="response", taskid=None, result=None):

        if redis_conn == None:
            return
        res = {"type": type, "event": event, "taskId": taskid, "result" : result}
