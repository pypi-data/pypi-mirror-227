...
from readyocr.entities import (Block, BoundingBox, Cell, Character, Document,
                               EntityList, Field, Figure, Image, Invoice, Key,
                               Line, LineItem, LineItemGroup, MergedCell, Page,
                               Paragraph, Table, Value, Word)
...
def _create_object(entity_json):
    ...
    if entity_json["class"] == "Table":
        ...
        entity = eval(entity_json["class"])(
            id=entity_json["id"], bbox=bbox, confidence=entity_json["confidence"]
        )
