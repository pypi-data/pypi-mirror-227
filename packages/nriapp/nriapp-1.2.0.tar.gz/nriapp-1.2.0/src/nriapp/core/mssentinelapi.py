...
#                        self.load_session()
    ...
    def get_incidents(self, incidentId = [], alertStatus=['New','InProgress', 'Resolved'], severity=[256, 128, 64, 32], pageIndex= 1, lookBackInDays = 60, pageSize = 50, sourceFilter=[], titleFilter=[], incident_list=[]):  #July 25, 2023 Changed the PageSize. It seems like the Defender changed the limit from 3000 to 0-50 only.
        ...
        if len(json.loads(response.text)) == pageSize:
            self.get_incidents(incidentId,pageIndex=pageIndex+1,lookBackInDays = lookBackInDays, pageSize=pageSize, incident_list=incident_list)
#        else:                                                          #July 26, 2023
...
#                } for i in json.loads(response.text)['Items']]
    ...
    def get_mail_metadata(self, startUtc, endUtc, networkMessageId):
        ...
        if object["ResultData"] != None:
            ...
            "dkimAuth"             : resultData['DkimAuthResult'],
            "spfAuth"              : resultData['SpfAuthResult'],
            "dmarcAuth"            : resultData['DmarcAuthResult'],
...
#                                networkMessageId = i.get("networkMessageId")
                                ...
                                if metadata:
                                    ...
                                    mailMessage["returnPath"] = metadata["returnPath"]
