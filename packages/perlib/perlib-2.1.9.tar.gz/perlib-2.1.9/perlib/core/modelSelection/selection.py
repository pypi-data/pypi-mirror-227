...
class ModelSelection:
    ...
    def is_seasonal(self):
        ...
        data = self.data.copy()
        data['mean'] = data[self.targetColumn]
        decomposition = sm.tsa.seasonal_decompose(data['mean'], model='additive')
    ...
    def analyze_trend(self,time_series_column):
        ...
        if time_series_column:
            ...
            significant_changes = (trend_diff > trend_diff.std() * 2).any()
            if trend_slope > 0 and significant_changes:
                return "Upward"
            elif trend_slope < 0 and significant_changes:
                return "Downward"
            else:
    ...
    def analyze_algorithm_feasibility(self, algorithm_name, time_series_column):
        ...
        if algorithm_name == "LSTM":
            ...
            if time_series_column:
                trend_results = self.analyze_trend(time_series_column)
                if trend_results == "Upward":
                    return True, f"Upward trend detected, {algorithm_name} can be used."
                elif trend_results == "Downward":
                    return True, f"Downward trend detected, {algorithm_name} can be used."
                else:
                    return False, f"No significant trend detected, {algorithm_name} is not recommended."
            else:
    ...
    def select_model(self):
        ...
        pca_X, pca_explained_variance, lda_X, lda_explained_variance = self.apply_dimensionality_reduction(self.data,self.data[self.targetColumn])
