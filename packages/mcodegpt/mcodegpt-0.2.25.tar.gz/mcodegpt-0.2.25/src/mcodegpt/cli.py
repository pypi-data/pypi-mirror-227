"""Command line interface for mcodegpt."""
import codecs
import json
import logging
import pickle
...
import mcodegpt.ontex.extractor as extractor
from mcodegpt import DEFAULT_MODEL, MODELS, __version__
from mcodegpt.clients import OpenAIClient
from mcodegpt.clients.pubmed_client import PubmedClient
from mcodegpt.clients.soup_client import SoupClient
from mcodegpt.clients.wikipedia_client import WikipediaClient
from mcodegpt.engines import create_engine
from mcodegpt.engines.embedding_similarity_engine import SimilarityEngine
from mcodegpt.engines.enrichment import EnrichmentEngine
from mcodegpt.engines.generic_engine import GenericEngine, QuestionCollection
from mcodegpt.engines.gpt4all_engine import GPT4AllEngine
from mcodegpt.engines.halo_engine import HALOEngine

# from mcodegpt.engines.hfhub_engine import HFHubEngine
from mcodegpt.engines.knowledge_engine import KnowledgeEngine
from mcodegpt.engines.mapping_engine import MappingEngine
from mcodegpt.engines.pheno_engine import PhenoEngine
from mcodegpt.engines.reasoner_engine import ReasonerEngine
from mcodegpt.engines.spires_engine import SPIRESEngine
from mcodegpt.engines.synonym_engine import SynonymEngine
from mcodegpt.evaluation.enrichment.eval_enrichment import EvalEnrichment
from mcodegpt.evaluation.resolver import create_evaluator
from mcodegpt.io.csv_wrapper import write_obj_as_csv
from mcodegpt.io.html_exporter import HTMLExporter
from mcodegpt.io.markdown_exporter import MarkdownExporter
from mcodegpt.utils.gene_set_utils import (
    GeneSet,
    _is_human,
    fill_missing_gene_set_values,
    parse_gene_set,
)
from mcodegpt.utils.gpt4all_runner import chain_gpt4all_model, set_up_gpt4all_model

__all__ = [
    "main",
]

from mcodegpt.io.owl_exporter import OWLExporter
from mcodegpt.io.rdf_exporter import RDFExporter
from mcodegpt.io.yaml_wrapper import dump_minimal_yaml
from mcodegpt.templates.core import ExtractionResult
...
def get_model_by_name(modelname: str):
    ...
    if not found:
        logging.warning(
            ...
            See all models with `mcodegpt list-models`"""
        )
...
def extract(
    ...
    **kwargs,
    ...
    Example:
        ...
        mcodegpt extract -t gocam.GoCamAnnotations -i gocam-27929086.txt
    ...
    Example:
        ...
        mcodegpt -extract -t gocam.GoCamAnnotations -T GeneOrganismRelationship "the mouse Shh gene"
...
def pubmed_annotate(search, template, output, output_format, limit, get_pmc, **kwargs):
    ...
    Example:
    mcodegpt pubmed-annotate -t phenotype "Takotsubo Cardiomyopathy: A Brief Review"
...
def enrichment(
    ...
    **kwargs,
    ...
    Usage:
        ...
        mcodegpt enrichment -r sqlite:obo:hgnc -U tests/input/genesets/dopamine.yaml
...
def clinical_notes(
    ...
    **kwargs,
    ...
    Example:
        ...
        mcodegpt clinical-notes -d "middle-aged female patient with diabetes"
        mcodegpt clinical-notes --description "middle-aged female patient with diabetes"\
