...
def __save__(db, class_data, name_table, key):
    for data in db.accept_columns(name_table):
      db.update_line(name_table=name_table, key1=key, value1=class_data.id, key2=data[0], value2=getattr(class_data, data[0]))
...
class MazgaDB:
    ...
    def select_class(self, name_table: str, key: str, value, class_data=None) -> object:
        ...
        if class_data:
            ...
            class_ = make_dataclass(cls_name=name_table.title(), fields=self.accept_columns(name_table), namespace={'__save__': lambda self1, key: __save__(self, self1, name_table, key)})
            return class_(*data)
