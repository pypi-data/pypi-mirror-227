...
if os.environ.get("USE_FAST"):
    ...
    from modeling_llama_fast import LlamaDecoderLayer, LlamaForTraining, RMSNorm, RotaryEmbedding
else:
    from modeling_llama import LlamaDecoderLayer, LlamaForTraining, RMSNorm, RotaryEmbedding
...
def main():
    ...
    if config.data.dataset is None:
        ...
        data_iterator = to_cuda(dataloader=dataloader, dpg=dpg, config=config)
        ...
        with main_rank_first(dpg.world_pg):
            ...
            assert (config.tokens.train_steps - start_iteration_step) * global_batch_size // dpg.dp_pg.size() < len(
                dataloader
            ), f"Dataset is too small for steps ({len(dataloader)} < {(config.tokens.train_steps - start_iteration_step) * global_batch_size // dpg.dp_pg.size()}), Try train_steps={len(dataloader) * dpg.dp_pg.size() // global_batch_size + start_iteration_step}"
