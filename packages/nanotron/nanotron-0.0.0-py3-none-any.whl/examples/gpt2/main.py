...
if os.environ.get("USE_FAST"):
    ...
    from modeling_gpt2_fast import GPTBlock, GPTForTraining, LayerNorm
else:
...
def main():
    ...
    # Init model
    model_name = config.model.hf_gpt2_model_name
    ...
    model, random_states = init_model(
        ...
        dpg=dpg,
        # TODO @thomasw21: Figure out why using torch DDP with accumulate_in_fp_32 and ZeRO-1 performs poorly.
    ...
    if checkpoint_path is not None:
        ...
        if isinstance(config.model.init_method, ExistingCheckpointInit):
            ...
            scaled_init_method_ = scaled_init_method_normal(config.model.init_method.std, model_config.n_layer)
