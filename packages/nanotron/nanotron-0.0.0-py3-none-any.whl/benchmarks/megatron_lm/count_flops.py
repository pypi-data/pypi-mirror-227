...
def get_flops_encoder_decoder(
    ...
    print(f"encoder_flops_fwd: {encoder_flops_fwd:2.3e}")
    print(f"decoder_flops_fwd: {decoder_flops_fwd:2.3e}")
    print(f"lm_head_flops_fwd: {lm_head_flops_fwd:2.3e}")
    print(f"model FLOPs per iteration: {model_flops:2.3e}")
    print(f"hardware FLOPs per iteration: {hardware_flops:2.3e}")
    return model_flops, hardware_flops


def get_flops_decoder(
    num_layers,
    hidden_size,
    num_heads,
    vocab_size,
    ...
    model_flops, hardware_flops = get_flops_encoder_decoder(
        num_layers_enc=num_layers,  # decoder-only doesn't have cross-attention
        num_layers_dec=0,
...
if __name__ == "__main__":
    iteration_time_in_sec = 10920 / 1000
    world_size = 8
    ...
    #     glu_activation=False,
    # )
    model_flops, hardware_flops = get_flops_decoder(
        num_layers=36,
        hidden_size=2816,
        num_heads=22,
        vocab_size=49152,
    ...
    print(f"Model TFLOPs: {model_flops_per_s:.2f} TFLOPs")
    print(f"Hardware TFLOPs: {hardware_flops_per_s:.2f} TFLOPs")
