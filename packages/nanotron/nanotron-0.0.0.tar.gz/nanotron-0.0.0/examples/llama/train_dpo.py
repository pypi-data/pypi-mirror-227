...
from config import (
    DPOPretrainDatasetsArgs as PretrainDatasetsArgs,
)
from dataloader import dpo_data_generator, to_cuda
from datasets.download.streaming_download_manager import xPath
from flops import get_flops_per_sec
from modeling_dpo import DPOForTraining
...
def main():
    ...
    assert isinstance(
        config.dpo, DPOArgs
    ), f"`config.dpo` must be defined in {config_file} if you want to run DPO training"
    ...
    model = init_model(
        model_builder=lambda: DPOForTraining(
            config=model_config,
            dpg=dpg,
    ...
    freeze_bottom_causal_layers(model, num_layers_unfrozen=config.dpo.num_layers_unfrozen)
    ...
    if checkpoint_path is not None:
        ...
        if isinstance(config.model.init_method, ExistingCheckpointInit):
            ...
            raise ValueError(f"Unsupported init_method '{config.model.init_method}' for DPO training")

    # Initialize ref_model from a pretrained model
    load_weights(model=ref_model, dpg=dpg, root_folder=config.model.init_method.path / "model")
    ...
    if isinstance(config.data.dataset, PretrainDatasetsArgs):
        ...
        data_iterator = dpo_data_generator(
            dpg=dpg,
            model=model,
            ref_model=ref_model,
            tokenizer=tokenizer,
            hf_dataset_name=config.data.dataset.hf_dataset_name,
            hf_dataset_split=config.data.dataset.hf_dataset_split,
            sequence_length=sequence_length,
            micro_batch_size=config.tokens.micro_batch_size,
            max_prompt_length=config.dpo.max_prompt_length,
            truncation_mode=config.dpo.truncation_mode,
            label_pad_token_id=config.dpo.label_pad_token_id,
        )()
                ...
                if iteration_step % config.logging.iteration_step_info_interval == 0:
                    ...
                    if (
                        ...
                        log_entries = [
                            ...
                            LogItem("dpo_loss", loss_avg_per_mbs, "1.6E"),
                            LogItem(
                                "rewards/chosen",
                                torch.tensor([output["rewards/chosen"] for output in outputs]).mean().item(),
                                "1.6E",
                            ),
                            LogItem(
                                "rewards/rejected",
                                torch.tensor([output["rewards/rejected"] for output in outputs]).mean().item(),
                                "1.6E",
                            ),
                            LogItem("lr", lr, ".3E"),
                        ...
                        if tb_writer is not None:
                            ...
                            log_entries.extend(
                                [
                                    LogItem(
                                        "rewards/accuracies",
                                        torch.tensor([output["rewards/accuracies"] for output in outputs])
                                        .mean()
                                        .item(),
                                        "1.6E",
                                    ),
                                    LogItem(
                                        "rewards/margins",
                                        torch.tensor([output["rewards/margins"] for output in outputs]).mean().item(),
                                        "1.6E",
                                    ),
                                    LogItem(
                                        "logps/rejected",
                                        torch.tensor([output["logps/rejected"] for output in outputs]).mean().item(),
                                        "1.6E",
                                    ),
                                    LogItem(
                                        "logps/chosen",
                                        torch.tensor([output["logps/chosen"] for output in outputs]).mean().item(),
                                        "1.6E",
                                    ),
                                    LogItem(
                                        "logits/rejected",
