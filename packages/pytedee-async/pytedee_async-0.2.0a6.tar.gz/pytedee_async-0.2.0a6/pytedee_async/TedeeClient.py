...
from .TedeeClientException import TedeeClientException, TedeeWebhookException
...
class TedeeClient(object):
    ...
    def parse_lock_properties(self, json: dict):
        ...
        if lock_properties is not None:
            ...
            state_change_result = json.get("stateChangeResult", 0)
    ...
    def _get_local_api_header(self) -> str:
        secure_token = self._calculate_secure_local_token()
        return {"Content-Type": "application/json", "api_token": secure_token}
    
    async def _local_api_call(self, path: str, http_method: str, json_data=None):
        if self._use_local_api:
            try:
                _LOGGER.debug("Getting locks from Local API...")
                r = await http_request(self._local_api_path + path, http_method, self._get_local_api_header(), self._timeout, json_data)
                return True, r
                ...
                if self._personalToken is None:
                    ...
                    _LOGGER.warn(f"Error while calling local API endpoint {path}, retrying with cloud call. Full error {ex}")
                    _LOGGER.debug(f"Trace: {ex}", exc_info=1)
        return False, None
