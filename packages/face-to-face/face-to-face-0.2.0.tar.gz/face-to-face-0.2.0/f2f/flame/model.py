...
FLAME_MODEL_PATH = (
    Path(__file__).parents[1] / "assets" / "flame_2020" / "generic_model.pkl"
)
FLAME_LANDMARK_EMBEDDING_PATH = (
...
def onnx_export() -> None:
    ...
    torch.onnx.export(
        ...
        FLAME_MODEL_PATH.parent / "generic_model.onnx",
        opset_version=13,
...
def to_tensor(array: Union[Tensor, npt.NDArray], dtype=torch.float32) -> Tensor:
    if isinstance(array, np.ndarray):
        return torch.from_numpy(array).to(dtype=dtype)
    return array.to(dtype=dtype)


def to_np(array: Any, dtype=np.float32) -> npt.NDArray:
...
class FLAME(nn.Module):
    ...
    parents: Tensor
    """shape (5,) [-1, 0, 1, 1, 1], (global, neck, jaw, eye_left, eye_right), describes the kinematic tree for the model"""
    lbs_weights: Tensor
    """shape (5023, 5) linear blend skinning weights that represent how much the rotation matrix of each part affects each vertex"""
    ...
    """shape (79, 17, 3) list of 17 contour barycentric weights for y angle (-39, 39) degrees"""
    static_lm_faces_idx: Tensor
    """shape (68,)"""
    static_lm_bary_coords: Tensor
    """shape (68, 3)"""
    neck_kin_chain: Tensor
    """shape (2,) [1, 0], kinematic tree chain of the neck to consider relative rotation"""

    def __init__(
        self,
        flame_model_path: str = FLAME_MODEL_PATH,
        flame_lmk_embedding_path: str = FLAME_LANDMARK_EMBEDDING_PATH,
    ):
        super().__init__()
...
class FLAMETexture(nn.Module):
    ...
    def __init__(self, flame_texture_path: str) -> None:
        ...
        tex_space = np.load(flame_texture_path)
