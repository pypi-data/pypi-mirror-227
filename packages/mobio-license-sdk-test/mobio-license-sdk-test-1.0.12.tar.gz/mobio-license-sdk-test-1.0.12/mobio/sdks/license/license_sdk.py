...
class MobioLicenseSDK(object):
    ...
    def config(
        ...
        self.admin_host = admin_host if admin_host else Mobio.ADMIN_HOST
        self.module_encrypt = module_encrypt
        self.module_use = module_use
        self.license_key = license_key if license_key else Mobio.LICENSE_KEY_SALT
        if module_use:
            self.request_header = {
        ...
        if module_use and module_encrypt:
            ...
            #     self.module_valid = False
            self.module_valid = True
        self.redis_uri = redis_uri if redis_uri else Cache.REDIS_URI
    ...
    @sdk_pre_check
    def check_allowed_quantity_attribute(
    ...
    @sdk_pre_check
    def check_license_module_has_expired(
            self, merchant_id: str, module: str
    ):
        json_license = CryptUtil.get_license_info(self.license_key, merchant_id)
        if not json_license:
            return True
        result = PackageUtils(json_license=json_license).check_license_module_expired(
            module=module
        )
        return result
