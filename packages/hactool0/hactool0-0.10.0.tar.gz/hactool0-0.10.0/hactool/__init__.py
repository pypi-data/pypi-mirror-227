...
from termtables import print_table
...
def shuffle(_list:list):
    ...
    for i in range(len(_list)):
        ran = randint(b=len(_list)-1)
        output.append(_list[ran])
        del _list[ran]
    return output
...
def fileget(fname,dtype=str,delimiter='',comments='#',encoding='utf-8'):
    ...
    try:
        return (loadtxt(fname=fname,delimiter=delimiter,dtype=dtype,comments=comments,encoding=encoding))
...
def tprint(content:list,header:list):
    ...
    print_table(content,header)

# 0.10.0
def pick(options:list=[],title:str='',indicator:str='> ',default_index:int=0):
    '''
    same as pick.pick
    ...
    return PICK(options=options, title=title, indicator=indicator, default_index=default_index)
