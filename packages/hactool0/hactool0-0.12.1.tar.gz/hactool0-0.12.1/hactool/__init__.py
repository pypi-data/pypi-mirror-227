...
def fileget(fname,dtype=str,delimiter='',comments='#',encoding='utf-8'):
    ...
    if os.path.exists(fname): return list(loadtxt(fname=fname,delimiter=delimiter,dtype=dtype,comments=comments,encoding=encoding))
...
# 0.10.0
def table(content:list,header:list,tablefmt:str="fancy_grid"):
    '''
    return a table
    ...
    return tabulate.tabulate(content,header,tablefmt=tablefmt)
...
def playsound(path:str):
    ...
    try: Playsound.playsound(path)
    except SoundException as e: raise SoundException(e)
...
class Path:
    ...
    # 0.8.0
    def info(self):
        self.full = os.path.abspath(self.path)
        self.drive = os.path.splitdrive(self.path)[0]
        self.dir = os.path.dirname(self.path)
        self.file = os.path.basename(self.path)
        self.exists = os.path.exists(self.path)
        if self.exists==True: self.size = os.path.getsize(self.path)
        else: self.size = 0
        if os.path.isfile(self.path): self.type='file'
        elif os.path.isdir(self.path): self.type='dir'
