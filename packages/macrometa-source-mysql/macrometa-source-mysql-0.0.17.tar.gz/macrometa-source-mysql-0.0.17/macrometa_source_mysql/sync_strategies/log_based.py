...
from macrometa_source_mysql import connection
from macrometa_source_mysql.connection import connect_with_backoff, make_connection_wrapper, MySQLConnection
from macrometa_source_mysql.discover_utils import discover_catalog, desired_columns, should_run_discovery
...
LOGGER = singer.get_logger('macrometa_source_mysql')
...
def _run_binlog_sync(
    ...
    while True:
        binlog_event = reader.fetchone()  # Fetch the next event
        if binlog_event is not None:
            ...
            if (log_file > end_log_file) or (end_log_file == log_file and log_pos >= end_log_pos):
                LOGGER.debug('BinLog reader last processed position: (file: %s, pos:%s).', log_file, log_pos)
                # There are cases when a mass operation (inserts, updates, deletes) starts right after we get the Master
                # binlog file and position above, making the latter behind the stream reader, and it causes some data
                # loss in the next run by skipping everything between end_log_file and log_pos,
            ...
            if isinstance(binlog_event, RotateEvent):
                ...
                if not catalog_entry:
                    ...
                    if diff:
                        ...
                        LOGGER.info('Stream `%s`: Difference detected between event and schema: %s', tap_stream_id,
                                    diff)
        ...
        # TODO: Grainier -> Move to state maintenance
...
def create_binlog_stream_reader(
    ...
    kwargs = {
        ...
        'report_slave': socket.gethostname() or 'macrometa',  # this is so this slave appears in SHOW SLAVE HOSTS;
        'only_events': [WriteRowsEvent, UpdateRowsEvent, DeleteRowsEvent],
    }

    # only fetch events pertaining to the schemas in filter db.
    if config.get('database'):
        kwargs['only_schemas'] = config['database'].split(',')
    ...
    if config['use_gtid']:
        ...
        # Set to True to make the reader wait for new events
