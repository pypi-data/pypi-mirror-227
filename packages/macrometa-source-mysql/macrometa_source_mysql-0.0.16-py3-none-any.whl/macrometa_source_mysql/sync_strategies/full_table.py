...
from macrometa_source_mysql.connection import connect_with_backoff
from macrometa_source_mysql.sync_strategies import common, log_based

LOGGER = singer.get_logger('macrometa_source_mysql')


def generate_bookmark_keys(catalog_entry, config):
    md_map = metadata.to_map(catalog_entry.metadata)
    stream_metadata = md_map.get((), {})
    replication_method = stream_metadata.get('replication-method', config['replication_method'])
    ...
    if replication_method == 'FULL_TABLE':
        ...
        bookmark_keys = base_bookmark_keys.union(log_based.BOOKMARK_KEYS)
...
def sync_table(mysql_conn, catalog_entry, config, state, columns, stream_version):
    common.whitelist_bookmark_keys(generate_bookmark_keys(catalog_entry, config), catalog_entry.tap_stream_id, state)
