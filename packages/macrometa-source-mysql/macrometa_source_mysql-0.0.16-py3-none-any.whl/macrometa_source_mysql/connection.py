...
class MySQLConnection(pymysql.connections.Connection):
    def __init__(self, config, require_database=True):
        ...
        # patched out method to its original spot.
        ...
        self.use_self_signed_ssl = config.get("ssl_ca") and config.get("ssl_cert") and config.get("ssl_key")

        if self.use_self_signed_ssl:
            LOGGER.info("Using custom certificate authority")

            # Generate a random directory with certificates.
            self.temp_dir = tempfile.mkdtemp()

            with open(os.path.join(self.temp_dir, "ca.pem"), "wb") as ca_file:
                ca_file.write(config["ssl_ca"].encode('utf-8'))

            with open(os.path.join(self.temp_dir, "cert.pem"), "wb") as cert_file:
                cert_file.write(config["ssl_cert"].encode('utf-8'))

            with open(os.path.join(self.temp_dir, "key.pem"), "wb") as key_file:
                key_file.write(config["ssl_key"].encode('utf-8'))

            ssl_arg = {
                "ca": os.path.join(self.temp_dir, "ca.pem"),
                "cert": os.path.join(self.temp_dir, "cert.pem"),
                "key": os.path.join(self.temp_dir, "key.pem"),
            }
            ...
            if config.get("internal_hostname"):
                ...
                ssl.match_hostname = lambda cert, hostname: MATCH_HOSTNAME(cert,
                                                                           parsed_hostname)  # pylint: disable=W1505
        ...
        self.session_sqls = config.get("session_sqls", DEFAULT_SESSION_SQLS).split(",")
