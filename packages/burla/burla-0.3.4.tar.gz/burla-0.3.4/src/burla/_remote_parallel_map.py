...
from burla._auth import auth_headers_from_local_config, credentials_exist, AuthException, login
from burla._env_inspection import get_pip_packages, get_imported_modules
...
def _start_job(
    ...
    imported_modules = get_imported_modules(pickled_function)
    ...
    response = requests.post(f"{_BURLA_SERVICE_URL}/v1/jobs/", json=payload, headers=headers)
    if response.status_code == 401:
        raise AuthException()
    else:
        response.raise_for_status()
...
def remote_parallel_map(
    ...
    if (not credentials_exist()) and IN_COLAB:
        login()
    else:
        raise AuthException()
