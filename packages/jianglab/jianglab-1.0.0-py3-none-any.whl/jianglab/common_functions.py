...
def add_frame_time_to_pkl_data(pkl_data, frame_channel_num = 1, exclued_init_frame_num = 4,\
    ...
    """
    peaks = get_frame_timestamp(pkl_data, frame_channel_num=frame_channel_num,\
...
def add_frame_time_to_pkl_file_list(pkl_raw_file_list):
    for file in pkl_raw_file_list:
        ...
        with open(file, 'rb') as f:
            pkl_data = pickle.load(f)
        add_frame_time_to_pkl_data(pkl_data)
...
def add_burst_mask_to_pkl_file_list(pkl_raw_file_list, burst_detector = burst_detector, post_fix = "_burst"):
...
def add_sta_to_pkl_file_list(pkl_raw_file_list, video):
    for file in pkl_raw_file_list:
        generate_sta(video, file, frame_range = 60, post_frame_range = 0, \
                    pre_pad_frame = 180, post_fix = "_sta")
        

def convert_spike_to_frame(pkl_data, unit_id, pre_pad_frame = 180, range = (0,36000), bins=9000):
    ...
    spike_idx = spike_idx - pre_pad_frame
    spike_idx = spike_idx[[spike > 0 for spike in spike_idx ]]
    #firing_rate= plt.hist(spike_idx, range = (0,36000), bins=9000)[0]
    firing_rate = np.histogram(spike_idx, range = range, bins=bins)
    pkl_data["units_data"][unit_id]["firing_rate_as_frame"] = firing_rate
    return data, firing_rate