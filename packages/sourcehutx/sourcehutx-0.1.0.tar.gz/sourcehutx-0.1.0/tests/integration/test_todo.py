...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_whoami(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_get_tracker_ref(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_get_tracker(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_list_trackers(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
        # List trackers
        trackers = [
            tracker
            async for tracker in client.list_trackers("sircmpwn", max_pages=None)
        ]
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_list_tickets(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_list_tracker_labels(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
        tracker = await client.get_tracker_ref("gotmax23", "fedrq-copy")
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_create_tracker_and_label(authed_client: SrhtClient):
    gotten: Any
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_subscription(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_nonexistant(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_ticket_nonexistant(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
        with pytest.raises(ResourceNotFoundError):
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_default_acl(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
        default = dict(
            browse=True,
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_default_acl_update(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
        default = dict(
            browse=True,
        ...
        expected = todo.TrackerDefaultACL(client=client, **default)
        created = await client.create_tracker(
            "tester-acl", visibility=VISIBILITY.UNLISTED
        )
        try:
...
@pytest.mark.asyncio
@vcr.use_cassette
async def test_todo_acls(authed_client: SrhtClient):
    async with authed_client:
        client = todo.TodoSrhtClient(authed_client)
        tracker = await client.create_tracker(
            "test-acl2", visibility=VISIBILITY.UNLISTED
        )
        try:
