...
def test_umlaut(test_dir, symbol_regex,time_regex):
    error_test = CharacterCheck.check_symbols(join(test_dir, "annotation_symbols.csv"), 0, symbol_regex,time_regex, -1)[0]
    error_expected = Error(
        ...
        error_type=ErrorType.IllegalSymbol,
        line_number=6,
        check_id=0
    )
...
def test_illegal_symbol(test_dir, symbol_regex, time_regex):
    error_test = CharacterCheck.check_symbols(join(test_dir, "annotation_symbols.csv"), 0, symbol_regex,time_regex, -1)[1]
    error_expected = Error(
        join(test_dir, "annotation_symbols.csv"),
