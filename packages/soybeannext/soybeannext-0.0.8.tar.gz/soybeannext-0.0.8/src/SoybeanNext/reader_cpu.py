...
class Reader_CPU:
    ...
    # the CPU implement of SNPfilter method
    def SNPfilter(self, dataframe):
        ...
        if isMissing:
            ...
            df_missing = pd.DataFrame(None, columns=missing, index=self.indexes, dtype=np.int32)
            df_missing[:] = 3
            # concat the original dataframe and filling dataframe
            df_filter = pd.concat([dataframe, df_missing], axis=1)
