...
def init_global_config() -> None:
    ...
    # Access token
    cfg.add_item(TokenItem(key='access_token', env_key='EB_ACCESS_TOKEN'))
    # Path to the file that stores the access token
    cfg.add_item(
        PathItem(
            key='access_token_path', env_key='EB_ACCESS_TOKEN_PATH'))
    # Access key ID
    cfg.add_item(PlainConfigItem(key='ak', env_key='EB_AK'))
    # Secret access key
    cfg.add_item(PlainConfigItem(key='sk', env_key='EB_SK'))
    ...
    cfg.add_item(PositiveValueItem(key='timeout', env_key='EB_TIMEOUT'))
...
class PlainConfigItem(_ConfigItem[Any]):
    DATA_TYPE = None
    _FACTORY = lambda x: x
...
class TokenItem(_ConfigItem[str]):
    DATA_TYPE = str
    _FACTORY = str
...
class PathItem(_ConfigItem[str]):
    DATA_TYPE = str
    _FACTORY = str
...
class PositiveValueItem(_ConfigItem[float]):
    DATA_TYPE = float
    _FACTORY = float
