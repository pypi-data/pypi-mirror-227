...
def parse_args(*args, **kwargs):
    ...
    # Global arguments
    parser.add_argument(
        '--access-token', type=str, help="Access token to use.")
    parser.add_argument(
        '--access-token-path',
        type=str,
...
class ChatCompletionHelper(_ResourceCLIHelper):
    ...
    def add_create_arguments(cls, parser):
        ...
        parser.add_argument(
            '--stream',
            action='store_true',
            help="Whether to stream messages.")
        parser.add_argument(
            '--temperature',
            type=float,
...
class ImageV1Helper(_ResourceCLIHelper):
    ...
    def create(cls, args):
        ...
        for idx, item in enumerate(resp.data['imgUrls'], 1):
            logger.info(f"\tImage {idx}: {item['image']}")
...
class ImageV2Helper(_ResourceCLIHelper):
    ...
    def create(cls, args):
        ...
        resp = cls.get_resource_class().create(**kwargs)
        for idx_t, task_item in enumerate(resp.data['sub_task_result_list'],
                                          1):
            logger.info(f"Subtask {idx_t}:")
            ...
            if status == 'SUCCESS':
                ...
                for idx_i, image_item in enumerate(
                    ...
                    logger.info(f"\tImage {idx_i}: {info}")
...
class ModelHelper(_ResourceCLIHelper):
    ...
    def list(cls, args):
        ...
        for model_name, model_desc in model_info_list:
            logger.info(f"{model_name}\t{model_desc}")
