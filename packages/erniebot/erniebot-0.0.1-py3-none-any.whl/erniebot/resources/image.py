...
class _Image(EBResource):
    ...
    def _prepare_paint(self, kwargs: Dict[str, Any]) -> Tuple[
        str,
        Optional[ParamsType],
        Optional[HeadersType],
        Optional[float]]:      # yapf: disable
        raise NotImplementedError

    def _prepare_fetch(self, resp_p: EBResponse) -> Tuple[str, Optional[
            ParamsType], Optional[HeadersType]]:
        raise NotImplementedError
...
class ImageV1(_Image):
    ...
    def _prepare_fetch(self, resp_p: EBResponse) -> Tuple[
            str,
            Optional[ParamsType],
            Optional[HeadersType]]:    # yapf: disable
        # url
        if self.api_type == APIType.YINIAN:
            url = '/getImg'
        else:
