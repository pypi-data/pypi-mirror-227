...
class _GlobalAuthCache(metaclass=Singleton):
    ...
    def __init__(self) -> None:
        super().__init__()
        self._cache: Dict[Any, str] = dict()
        self._last_update_time: Dict[Any, float] = dict()
    ...
    def retrieve_entry(self, api_type: str,
                       key: Any) -> Tuple[Any, Optional[str]]:
        with self._cond:
    ...
    def add_or_update_entry(
        ...
        if timestamp - self._last_update_time.get(
                key, -self._MIN_UPDATE_INTERVAL) > self._MIN_UPDATE_INTERVAL:
            if callable(value):
                try:
                    val = value()
    ...
    def _constr_key_pair(self, key1: str, key2: Any) -> Tuple[str, Any]:
        return (key1, key2)
    ...
    def _init_access_token(self,
        ...
        if token is None:
            ...
            if cached_token is not None:
                ...
                logger.info(
                    "Access token is not set. It will be requested based on other configs."
                )
