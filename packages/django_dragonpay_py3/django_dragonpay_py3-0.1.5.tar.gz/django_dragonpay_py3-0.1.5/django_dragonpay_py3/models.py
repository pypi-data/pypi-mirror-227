...
class DragonpayPayoutUser(models.Model):
    id = models.CharField(
        primary_key=True, max_length=40, verbose_name="Payout User ID"
    )
    ...
    class Meta:
        app_label = "django_dragonpay_py3"
...
class DragonpayTransaction(models.Model):
    ...
    currency = models.CharField(max_length=3, choices=CURRENCIES, default="PHP")
    ...
    def fetch_status(self):
        ...
        if status != self.status:
            logger.info(
                "[%s] status updated to %s", self.id, DRAGONPAY_STATUS_CODES[status]
            )
...
class DragonpayPayout(models.Model):
    ...
    def create_from_dict(cls, details):
        ...
        if isinstance(details, dict):
            ...
            logger.debug("Payout txn [%s] saved to database", details["txn_id"])
            ...
            for i, detail in enumerate(details):
                logger.debug(
                    "[%d] Payout txn [%s] saved to database", i, details["txn_id"]
                )
