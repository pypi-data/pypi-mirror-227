...
from olunicodenormalizer import langs
...
#-------------------------unicode ops-----------------------------------------------------------------------------   
    ...
    def fixNoSpaceChar(self):
        for idx,d in enumerate(self.decomp):
            if idx==0 and self.decomp[idx] in ["\u1C70","\u200d"]:
                self.decomp[idx]=None
            else:
                if idx<len(self.decomp)-1 and self.decomp[idx] in ["\u1C70","\u200d"] and self.decomp[idx+1] in [self.lang.connector]+["\u1C70","\u200d"]:
                    # if the next char is either connector or is repeated
                    self.decomp[idx+1]=None
                if idx>0 and self.decomp[idx] in ["\u1C70","\u200d"] and self.decomp[idx-1] in [None]+["\u1C70","\u200d"]:
