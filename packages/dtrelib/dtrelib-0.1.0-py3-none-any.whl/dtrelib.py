...
class Query(object):
    def __init__(self, h, p, u, pw, db, h1, p1, u1, pw1, db1, redis, useCache, cacheExpire, logger):
        # соединяемся с базой данных
        try:
            #do not change this parameters directly!
            self.__mainClient = Client(h, port=p, user=u, password=pw, secure=False, verify=True,
                                 database=db, compression=True)   # клиент для Select-выражений
            self.__serviceClient = Client(h1, port=p1, user=u1, password=pw1, secure=False, verify=True,
                                 database=db1, compression=True)  # клиент для Create/Drop/Insert-выражений
            self.__redis = redis
            self.__logger = logger
            self.__resultset = ResultSet()
            self.__directUsage = 0
            self.__cacheUsage = 0
            self.__mustCaching = useCache

            # this parameters can be responsible changed during runtime
            self._expiration = cacheExpire
            self._projectId = -1
            self._udr = None
            ...
            exit(-1)
    def __del__(self):
        self.__mainClient.disconnect()
        self.__serviceClient.disconnect()
    ...
    def sqlCommand(self):
        self.__serviceClient.execute(self.definition)

    def sqlExecute(self, with_column_types=False):
        if not with_column_types:
            self.__resultset.data = self.__mainClient.execute(self.definition)
            self.__resultset.columns = None
            self.__resultset.rowCount = len(self.__resultset.data)
        else:
            raw_data = self.__mainClient.execute(self.definition, with_column_types=True)
            ...
            if logFilesString:
                ...
                if self.__resultset.isCached():
                    ...
                    if not self._udr:
                        ...
                        self.definition = f"create table {id_name} ON CLUSTER '{{cluster}}'"\
                        ...
                        self.definition = f"drop table {id_name} ON CLUSTER '{{cluster}}'"
                        self.sqlCommand()
