...
class ElementMixin:
    def __init__(self, **data: Any) -> None:
        elements_data: list[dict, T] = data.get('elements', [])

        if contain_only_dict(elements_data):
            _elements: List[T] = []
            for element_data in elements_data:
                element_type = get_element_type(element_data)
                element: T = element_type(**element_data)
                _elements.append(element)
            data['elements'] = _elements

        super().__init__(**data)  # type: ignore

    def get_element_by_action_id(self, action_id: str) -> T:
        """
...
class AttachmentBody(ElementMixin, BaseModel):
    ...
    #
    # def __init__(self, **data: Any) -> None:
    #     elements_data: list[dict | AttachmentElementType] = data.get('elements', [])
    #
    #     if contain_only_dict(elements_data):
    #         _elements: list[AttachmentElementType] = []
    #         for element_data in elements_data:
    #             element_type = get_element_type(element_data)
    #             element: AttachmentElementType = element_type(**element_data)
    #             _elements.append(element)
    #         super().__init__(elements=_elements)
    #     else:
    #         super().__init__(**data)
    #
    # def get_element_by_action_id(self, action_id: str) -> AttachmentElementType:
    #     """
    #     Get element by action_id if it exists, otherwise raise ValueError
    #     """
