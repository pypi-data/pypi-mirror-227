...
def set_service():
    ...
    tbl = fb1.tools.get_synthetic_data_types(size=10, column_name='base')
    _ = fb1.tools.get_noise(size=10, num_columns=2, canonical=tbl, column_name='base')
    fb1.run_component_pipeline()
    pprint(viewer('feature_build', 'task1'))
...
def viewer(capability: str, task: str):
    ...
    return ast.literal_eval(tbl.to_pylist()[0]).get(capability,{}).get(task,{})
