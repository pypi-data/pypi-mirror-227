...
class DataInterface:
    ...
    def add_rp_map(self, style, rp_map):
        ...
        with h5.File(self.path, 'a') as data:
            ...
            group.create_dataset('data', data = rp_map.data, dtype = float32, compression=COMPRESSION)
    ...
    def add_profile_data(self, technology, dataset_id, profile, rp_maps, rp_map_styles, 
        ...
        with h5.File(self.path, 'a') as data:
            ...
            hits = data.create_dataset(profile_path, data = profile, dtype = float16, compression=COMPRESSION)
            self.set_attributes(hits, metadata)
            ...
            for rp_map, style in zip(rp_maps, rp_map_styles):
                ...
                rp_matrix_col = data.create_dataset(rp_matrix_path, data = rp_map.dot(profile), dtype = float32, compression=COMPRESSION)
