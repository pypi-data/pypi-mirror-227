...
class SparseVector:
    def __init__(self,
                 *,
                 indices: List[int],
                 values: VectorValueType) -> None:
        if indices is None or values is None:
            raise DashVectorException(
                code=DashVectorCode.InvalidArgument,
                reason='DashVectorSDK Get Invalid SparseVector and indices/values could be None')
        if not isinstance(indices, list) or not isinstance(values, list):
            raise DashVectorException(
                code=DashVectorCode.InvalidArgument,
                reason=f'DashVectorSDK Get Invalid SparseVector and indices/values must be List[int]/List[int, float]')
        if len(indices) != len(values):
            raise DashVectorException(
                code=DashVectorCode.InvalidArgument,
                reason='DashVectorSDK Get Invalid SparseVector and indices size is different with values')
        if len(indices) == 0:
            raise DashVectorException(
                code=DashVectorCode.InvalidArgument,
                reason='DashVectorSDK Get Invalid SparseVector and indices/values is empty')
        if not isinstance(indices[0], int) \
                or type(values[0]) not in (int, float):
            raise DashVectorException(
                code=DashVectorCode.InvalidArgument,
                reason='DashVectorSDK Get Invalid SparseVector and indices/values must be List[int]/List[int, float]')
