...
class MessageType(Enum):
    ...
    PHOTO = "Photo"
    AUDIO = "Audio"
    NEWS = "News Message"  # not 'News' to distinguish from old news messages
    NOT_SPECIFIED = None
    ...
    def __post_init__(self):
        ...
        if self.message_type:
            if isinstance(self.message_type, MessageType) and self.message_type != MessageType.NOT_SPECIFIED:
                self.type = self.message_type.value.lower().replace(' ', '_')
            elif isinstance(self.message_type, str):
                self.type = self.message_type.lower().replace(' ', '_')
        if self.author:
