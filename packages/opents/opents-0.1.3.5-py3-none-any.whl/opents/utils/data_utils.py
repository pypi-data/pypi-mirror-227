...
class RandomSplitOpenAllDataset(RandomSplitOpenDataset):
    ...
    """
    def __init__(self, x_all, y_all, train_size_rate, test_size_rate, open_label_rate, train_random_state=42, test_random_state=42, open_random_state=42):
        super().__init__(x_all, y_all, train_size_rate, test_size_rate, open_label_rate, train_random_state, test_random_state, open_random_state)
    
    def load(self):
        x_train, y_train, x_val, y_val, x_test, y_test = super().load()
        x_train, y_train, x_val, y_val, x_test, y_test = x_train.numpy(), y_train.numpy(), x_val.numpy(), y_val.numpy(), x_test.numpy(), y_test.numpy() 
        ...
        x_train, y_train, x_val, y_val, x_test, y_test, last_x_test, last_y_test = torch.tensor(x_train), torch.tensor(y_train).long(), torch.tensor(x_val), torch.tensor(y_val).long(), torch.tensor(x_test), torch.tensor(y_test).long(), torch.tensor(last_x_test), torch.tensor(last_y_test).long()
        
        return x_train, y_train, x_val, y_val, x_test, y_test, last_x_test, last_y_test
