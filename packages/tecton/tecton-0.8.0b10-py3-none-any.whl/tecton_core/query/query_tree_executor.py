...
class QueryTreeExecutor:
    ...
    def _exec_qt_node(
        self, qt_node: NodeRef, query_tree_step: QueryTreeStep, prev_step_output: Optional[QueryTreeOutput] = None
    ) -> QueryTreeOutput:
        if conf.get_bool("DUCKDB_DEBUG"):
    ...
    def _execute_staging_stage(self, qt_node: NodeRef) -> QueryTreeOutput:
        ...
        staging_nodes_to_process = self._get_staging_nodes_to_process(qt_node, QueryTreeStep.PIPELINE)
    ...
    def _process_agg_join(
        ...
        staging_nodes_to_process = self._get_staging_nodes_to_process(qt_node, QueryTreeStep.AGGREGATION)
        # If there are no ODFVs, can run the entire aggregation query tree
        aggregation_root = qt_node
        if len(staging_nodes_to_process) > 0:
            # There are ODFVs. There is only one expected staging node that is inserted right before the ODFV
            assert len(staging_nodes_to_process) == 1
            aggregation_root = staging_nodes_to_process[0]

        output_df_pa = compute.run_sql(aggregation_root.to_sql(), return_dataframe=True)
        odfv_input_df = output_df_pa.to_pandas()
        return QueryTreeOutput(staged_data=output.staged_data, odfv_input_df=odfv_input_df)

    def _get_staging_nodes_to_process(
        self,
        qt_node_ref: NodeRef,
