...
def _construct_new_plan_request_v1(
    workspace_name: str,
    upgrade_all: bool,
    fco_args: List[fco_args_pb2.FcoArgs],
    repo_source_info: repo_metadata_pb2.FeatureRepoSourceInfo,
    suppress_recreates: bool,
    debug: bool,
) -> metadata_service_pb2.NewStateUpdateRequest:
    request = metadata_service_pb2.NewStateUpdateRequest(
    ...
    if debug:
        request.blocking_dry_run_mode = True
    else:
        request.enable_eager_response = True

    return request


def _construct_new_plan_request_v2_from_v1(
    request_v1: metadata_service_pb2.NewStateUpdateRequest,
    no_color: bool,
    json_out: bool,
) -> metadata_service_pb2.NewStateUpdateRequestV2:
    return metadata_service_pb2.NewStateUpdateRequestV2(
        request=request_v1.request,
        blocking_dry_run_mode=request_v1.blocking_dry_run_mode,
        enable_eager_response=request_v1.enable_eager_response,
        no_color=no_color,
...
def _construct_query_plan_request_v2_from_v1(
    request_v1: metadata_service_pb2.QueryStateUpdateRequest,
    no_color: bool,
    json_out: bool,
) -> metadata_service_pb2.QueryStateUpdateRequestV2:
    return metadata_service_pb2.QueryStateUpdateRequestV2(
        state_id=request_v1.state_id, workspace=request_v1.workspace, no_color=no_color, json_output=json_out
    )
...
def _get_validation_errors(
    query_response: Union[
        metadata_service_pb2.QueryStateUpdateResponse, metadata_service_pb2.QueryStateUpdateResponseV2
    ],
    enable_server_side_rendering: bool,
) -> List[state_update_pb2.ValidationMessage]:
    if enable_server_side_rendering:
...
def update_tecton_state(
    ...
    if apply and plan_id:
        ...
        request_query = metadata_service_pb2.QueryStateUpdateRequest(state_id=state_id, workspace=workspace_name)
        ...
        with printer.safe_yaspin(Spinners.earth, text=f"{server_side_msg_prefix}: Initializing.") as sp:
            try:
                ...
                if new_plan_response.HasField("eager_response"):
                    ...
                    seconds_slept = 0
                    request_query = metadata_service_pb2.QueryStateUpdateRequest(
                        workspace=workspace_name, state_id=new_plan_response.state_id
                    )
                    while True:
                        response_query = _execute_query_state_update_v1_or_v2(
                            request_query,
