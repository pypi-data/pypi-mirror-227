...
def build_materialization_querytree(
    ...
    staging_s3_prefix: Optional[str] = None,
) -> NodeRef:
    ...
    Args:
        ...
        staging_s3_prefix: If set, the results will be staged to a location with the given prefix.
    """
    ...
    if staging_s3_prefix:
        tree = StagingNode(
            input_node=tree,
            staging_table_name=f"{fdw.name}",
            staging_location=QueryTreeStagingLocation.S3,
