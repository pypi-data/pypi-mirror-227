...
def test_debug_logs(cli_runner, csv_file, option):
    ...
    ret = cli_runner(f"{csv} -d {option} 2023")
    assert "year=2023" in ret.stdout


def test_raise_exception_no_data_from_csv(cli_runner, csv_file):
    csv = csv_file("sample.csv")
    ret = cli_runner(f"{csv} -yr 2099")
    assert "error: no data extracted from csv file" in ret.stdout
