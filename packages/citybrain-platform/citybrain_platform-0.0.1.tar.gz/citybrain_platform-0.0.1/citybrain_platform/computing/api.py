...
class Computing():
    ...
    @classmethod
    def create_table(cls, name: str, columns: List[data_types.Column], partition_columns: List[data_types.Column] = []) -> bool:
    ...
    @classmethod
    def get_job_status(cls, job_id: str) -> data_types.JobStatus:
        result = cls.__client().job_status(job_id=job_id)
        job_status = from_dict(data_class=data_types.JobStatus, data=result)
        return job_status
