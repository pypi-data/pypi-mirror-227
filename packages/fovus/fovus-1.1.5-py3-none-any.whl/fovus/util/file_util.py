...
class FileUtil:
    ...
    @staticmethod
    def compare_s3_hash(filepath, s3_object, chunk_size=5 * 1024 * 1024) -> bool:
        ...
        if len(md5_bytes_list) < 1:
            return "" == s3_object_hash

        if len(md5_bytes_list) == 1:
            return md5_bytes_list[0].hex() == s3_object_hash
        ...
        if "-".join([s3_md5_string, str(len(md5_bytes_list))]):
            return True

        return s3_object_hash == FileUtil.get_file_hash(filepath)
