...
class Trainer:
    ...
    def create_model(experiment_name: str, model_type: str, project_id: str, outcome_type: str, session: Session) -> Model:
        ...
        if outcome_type not in ["regression", "occurence"]:
            raise ValueError("outcome_type must be one of: 'regression', 'occurence'")
    ...
    def _run_model_job(self, model_job_id: str) -> str:
        """Helper function to run a model job.

        Args:
            model_job_id (str): The model job id.
        ...
        try:
            ...
            raise InvalidModelJobError(f"Model job {model_job_id} does not exist: {e}") from e

    def _wait_for_model_job(self, model_job_id: str, timeout, interval) -> ModelJob:
        """Helper function to wait for a model job to finish.

        Args:
            model_job_id (str): The model job id.
            timeout (int, optional): How long to wait for job to complete in seconds.
            interval (int, optional): How often to check in seconds.
        ...
        while True:
            ...
            time.sleep(interval)
            model_job = self.session.api_instance.get_one_base_model_job_controller_model_job(id=model_job_id)
    ...
    def train(self,
        ...
        Returns:
            str: The id of the model job
        """
        # create the model job
        new_model_job_to_run = self._create_model_job(model_job, dataset_id, config=model_job_config)

        # run the model job
        new_model_job_to_run_id = new_model_job_to_run.id
        self._run_model_job(new_model_job_to_run_id)

        # wait for the model job to finish
        if wait:
            self._wait_for_model_job(new_model_job_to_run_id, wait_timeout, wait_interval)

        return new_model_job_to_run_id

    def get_metrics(self):
        # TODO CMD17 from https://dbc-5c784d6d-d7a7.cloud.databricks.com/?o=1189584077923919#notebook/3744383587579554/command/3744383587579708
        pass