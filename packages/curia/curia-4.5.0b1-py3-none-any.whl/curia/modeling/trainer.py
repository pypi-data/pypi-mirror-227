...
class Trainer:
    ...
    def _wait_for_model_job(self, model_job_id: str, timeout: int, interval: int, print_status: bool) -> ModelJob:
    ...
    def train(self,
        ...
        # run the model job
        new_model_job_to_run_id = json.loads(new_model_job_to_run.data)['id']
        self._run_model_job(new_model_job_to_run_id)

        # wait for the model job to finish
        if wait:
            self._wait_for_model_job(new_model_job_to_run_id, wait_timeout, wait_interval, print_status=print_status)
