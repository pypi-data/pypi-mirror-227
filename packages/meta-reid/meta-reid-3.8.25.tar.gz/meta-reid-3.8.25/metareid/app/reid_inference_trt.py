...
class ReidInferenceTrt:
    ...
    def index(self, features, min_score, recall_num=1):
        if self.query_features is None or recall_num >= len(self.query_features):
            return [], []
        ...
        if self.use_gpu:
            ...
            index_cossim = fast_dot(features, self.query_features.T)[0]
            max_idx = np.where(index_cossim > np.sort(index_cossim)[-recall_num - 1]) and np.where(
                index_cossim > min_score)
            maximum = index_cossim[max_idx]

            return max_idx[0], maximum
