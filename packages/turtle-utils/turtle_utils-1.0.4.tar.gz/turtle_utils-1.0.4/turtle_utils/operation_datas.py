...
from turtle_utils.exception_util import opeartionTypeError, sheetNameNotFoundError
from turtle_utils.other_util import otherUtil
from turtle_utils.wrapper_util import exec_time_wrapper


class operationExcel(object):
    def __init__(self, file_path, file_name, operation_type='read',
                 data_only=False, sheet_name='data'):
        ...
        if not operation_type in ('read', 'write', 'read_write'):
            raise opeartionTypeError(msg=f"opeartion_type错误,预期:'read','write','read_write',实际:{operation_type}")
        if operation_type in ('read', 'read_write'):
            self.check_file_address_exists(sheet_name=self.sheet_name)
            if operation_type == 'read':
                read_only = True
            else:
                read_only = False
            # data_only=True,读取数据时保留exlce单元格格式
            self.workbook = openpyxl.load_workbook(self.file_address, data_only=data_only,read_only=read_only)
    ...
    # 获取指定范围行数据
    # @exec_time_wrapper(round_num=5)
    def get_row_values_all(self,start_row,end_row=None):
        row_values = self.data.iter_rows(min_row=start_row,max_row=end_row,values_only=True)
        return row_values
        
    # @exec_time_wrapper(round_num=5)
    # 获取指定范围列数据
    def get_col_values_all(self,start_col,end_col=None):
        col_values = self.data.iter_rows(min_col=start_col,max_col=end_col,values_only=True)
        return col_values
    ...
    def get_row_values(self, row):
        try:
            return list(self.get_row_values_all(start_row=1))[row]
        except IndexError:
            return []
    ...
    def get_col_data(self, col):
        try:
            return list(self.get_col_values_all(start_col=1))[col]
        except IndexError:
            return []
...
if __name__ == '__main__':
    ...
    opx = operationExcel(file_path='',file_name='test.xlsx',operation_type='read_write',
                         data_only=False,sheet_name='test1')
    opx.write_value(1,1,'test')
    opx.save_workbook()
    opx.workbook.close()
    # opx.save_workbook()
