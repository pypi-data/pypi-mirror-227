...
import traceback
import platform
from turtle_utils.operation_logging import operationLogging
...
# 命令执行封装
def excute_command(command, command_type='subPopen', clog_exec=True, result_match_expression=None):
    # clog_exec:windows环境将start字符串拼接在命令前，防止程序运行阻塞，默认为阻塞
    # command_type：指定命令类型
    # result_match_expression：结果匹配表达式
    result = None
    if not clog_exec:
        if system_type == 'Windows':
    ...
    try:
        ...
        # 解决：1.使用临时文件tempfile扩展缓存区；2.去掉不必要输出，以减少输出量的大小
            ...
            if isinstance(result, (tuple, list, set)) and result[0] is None and result[1] == b'':
            ...
            if not result:
                return result
            if len(result) > 1:
                if result_match_expression:
                    out_result = []
                    for i in result:
                        _exec_info_match = re.search(str(result_match_expression), str(i))
                        if _exec_info_match:
                            out_result.append(_exec_info_match.group())
                            break
                else:
                    out_result = [i.split(line_break)[0] if i.split(line_break) else i for i in result if i]
            else:
                out_result = [i.split(line_break) for i in result if i][0]
            result = [i for i in out_result if i]
        return result
    except:
        traceback.print_exc()
