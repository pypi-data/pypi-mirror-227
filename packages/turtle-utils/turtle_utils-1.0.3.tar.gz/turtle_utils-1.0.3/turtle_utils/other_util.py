...
from turtle_utils.operation_logging import get_project_rootpath
...
class otherUtil(object):
    ...
    @classmethod
    def encry_rsa(cls,private_key_pem, encry_data, is_hex=False):
        '''

        :param private_key_pem: 加密密钥
        :param encry_data: 加密文本
        :param is_hex: 是否输出十六进制
        :return:
        '''
        # 使用固定的密钥对进行签名
        ...
        signature = private_key.sign(
            str(encry_data).encode(),
            padding.PKCS1v15(),
            hashes.SHA256()
        )

        # 打印签名结果
        return signature.hex() if is_hex else signature
