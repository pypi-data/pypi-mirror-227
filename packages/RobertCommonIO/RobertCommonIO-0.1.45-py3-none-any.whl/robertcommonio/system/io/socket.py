...
class SocketAccessor:
    ...
    def start(self, thread: bool = True):
        ...
        if self.config.POOL == 0:
            if thread is True:
                self.thread = Thread(target=asyncore.loop, name=f"socket loop")
            else:
                asyncore.loop()
        else:
            if thread is True:
                self.thread = Thread(target=asyncore.loop, name=f"socket loop", kwargs={'timeout': self.config.TIME_OUT})
            else:
