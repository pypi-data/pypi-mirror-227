...
class AIModelsBuilder:
    ...
    def with_config_file(self,config_file: str) -> 'AIModelsBuilder':
    ...
    def _get_identifier(self):
        ...
        if self.data_env is not None:
            try:
                load_dotenv(self.data_env)
                return os.getenv(self.data_env)
            except:
    ...
    def _execute_create(self) -> requests.Response:
        ...
        if response.status_code == 201 and file_config:
            all_identifiers = self.find_all().build().execute().json()
            identifiers = [item['identifier'] for item in all_identifiers if item['name'] == self.file_name]
            try:
                self._read_config_file().get('id', 'model_id')
                file_config.set('id', 'model_id', identifiers[0])
    ...
    def _execute_save(self) -> Union[requests.Response, int]:
        load_dotenv(self.data_env)
        model_id = os.getenv(self.data_env)
        if model_id is not None or self.config_file is not None:
            if model_id is not None:
                self.identifier = model_id
            else:
                config = configparser.ConfigParser()
                config.read(self.config_file)
                model_id = config.get('id', 'model_id', fallback=None)
                self.identifier = model_id
    ...
    def _execute_set_identifier(self) -> None:
        try:
            file_config = self._read_config_file()
            self._read_config_file().get('id', 'model_id')
            file_config.set('id', 'model_id', self.identifier)
            with open(self.config_file, 'w', encoding='utf-8') as configfile:
                file_config.write(configfile)
