...
class AITransformersBuilder:
    ...
    def _get_identifier(self) -> str:
        ...
        if self.config_file is not None:
            try:
                return self._read_config_file().get('id', 'transformer_id')
        ...
        if response.status_code == 201 and file_config:
            ...
            if result is not None:
                try:
                    ...
                    file_config.set('id', 'transformer_id', result['identifier'])
                ...
                raise ValueError('No id found, so could not set transformer_id')
        return response
    ...
    def _execute_save(self) -> Union[requests.Response, int]:
        ...
        transformer_id = os.getenv('TRANSFORM_ID')
        if transformer_id is None:
            config = configparser.ConfigParser()
            config.read(self.config_file)
            transformer_id = config.get('id', 'transformer_id', fallback=None)
