...
class RulesBuilder:
    ...
    def with_config_file(self, config_file) -> 'RulesBuilder':
    ...
    def _get_identifier(self):
        ...
        if self.config_file is not None:
            try:
                return self._read_config_file().get('id', 'rule_id')
    ...
    def _execute_create(self):
        ...
        if response.status_code == 201:
            # Si la regla se creó exitosamente (código de estado 201), buscamos el identificador de la regla recién creada
            ...
            if self.config_file:
                ...
                if file_config:
                    try:
                        ...
                        self._read_config_file().get('id', 'rule_id')
                        file_config.set('id', 'rule_id', identifier)
    ...
    def _execute_save(self):
        ...
        if rule_id is None:
            config = configparser.ConfigParser()
            config.read(self.config_file)
            rule_id = config.get('id', 'rule_id', fallback=None)
    ...
    def _execute_set_identifier(self):
        try:
            file_config = self._read_config_file()
            self._read_config_file().get('id', 'rule_id')
            file_config.set('id', 'rule_id', self.rule_data["identifier"])
            with open(self.config_file, 'w', encoding='utf-8') as configfile:
                file_config.write(configfile)
