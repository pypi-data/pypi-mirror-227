...
def get_morningstar_data_sets(universe: Optional[str] = None) -> DataFrame:
    ...
    Args:
        universe (:obj:`str`, `optional`): The Morningstar code for an investment universe.

    Returns:
        DataFrame: A DataFrame object with Morningstar data sets data. The DataFrame columns include:
...
def get_data_point_settings(data_point_ids: List[str]) -> DataFrame:
    """Returns a DataFrame of settings for a given set of data points. This settings DataFrame can then be manipulated
    to reflect the specific settings to be used for data retrieval.

    Args:
        data_point_ids (:obj:`list`): A list of unique identifiers for data points. The format is an array of data point id.
            Example: ["OS01W", "HP010"]

    Returns:
        DataFrame: A DataFrame object with data point settings data. The DataFrame columns include:
...
def investments(keyword: str, count: int = 50, only_surviving: bool = True) -> DataFrame:
    ...
    Args:
        ...
        count (obj:`int`, `optional`): Maximum no. of investments to return that match the keyword.
        only_surviving (obj:`bool`, `optional`): Include Only Surviving investments.
    Returns:
        DataFrame: A DataFrame object with investments data. The DataFrame columns include:
    ...
    :Output:
        ...
        =================================    =============  ============= ================ =============== ================ ============ =============== =============== ============ ==================
        Fixed.zone AS	                        0P0001PHEP         FIXED      US97183A6156                             CZK      EX$$$$XPRA          CZE            ST                         0P0001PHEP
        Fixed Income Allocation CL I1           FS0000G0QH                    US97183A6156      97183A615              USD                          USA            CZ      FS0000G0QH         0P0001JW76
        =================================   ==============  ============= ================   =============== ============== ============ =============== =============== ============ ==================
...
def get_morningstar_category(universe: str) -> DataFrame:
    """Returns a DataFrame of Morningstar Categories for the given universe.
    search.

    Args:
        universe (:obj:`str`): The Morningstar code for an investment universe.

    Returns:
        DataFrame: A DataFrame object with Morningstar Category info. The DataFrame columns include:

        * name
        * value
        * globalBroadCategoryId
        * globalBroadCategoryName
        * regionName
...
def firms(keyword: str, universe: Optional[str] = None) -> DataFrame:
    ...
    Returns:
        DataFrame: A DataFrame object with firms data. The DataFrame columns include:
...
def get_brandings(keyword: str, find_by: str, universe: Optional[str] = None) -> DataFrame:
    ...
    Args:
        keyword(obj:`str`): Keyword to search for investments.
        find_by(obj:`str`): Search condition to search for investments. Valid values are `name_begin` and `name_contain`.
        universe(obj:`str`, `optional`): Maximum no. of investments to return that match the keyword.

    Returns:
        DataFrame: A DataFrame object with Branding Name and Branding Id. The DataFrame columns include:
...
def portfolio_managers(keyword: str = "", universe: str = "FO") -> DataFrame:
    ...
    Args:
        keyword (:obj:`str`): String to match one or more manager names. If the keyword is empty or not passed to the function, the function will return a Dataframe with all managers.

        universe (:obj:`str`): The Morningstar code for an investment universe. Use `get_all_universes` to see a full list of codes, or view the documentation `here <./reference.html#analyticslab.direct.get_all_universes>`_.


    Returns:
        DataFrame: A DataFrame object with managers data. The DataFrame columns include:
...
def companies(keyword: str = "", universe: str = "ST") -> DataFrame:
    ...
    Args:
        keyword (:obj:`str`): String to match one or more company names. If the keyword is empty or not passed to the function, the function will return a Dataframe with all companies.

        universe (:obj:`str`): The Morningstar code for an investment universe. Use `get_all_universes` to see a full list of codes, or view the documentation `here <./reference.html#analyticslab.direct.get_all_universes>`_.


    Returns:
        DataFrame: A DataFrame object with companies data. The DataFrame columns include:
...
def get_investment_universes(include_category_universes: bool = False) -> DataFrame:
    """Returns all investment universe lookups that are currently catalogued in investment_universes.
    ...
    Returns:
        DataFrame: A DataFrame object with universe lookup names and ids.  The DataFrame columns include:
