...
def get_investment_list(list_id: str) -> DataFrame:
    """Returns all investments in a specified list.
    Currently, this function does not support lists that combine investments and user-created portfolios.

    Args:
        list_id (:obj:`str`): The unique identifier of the saved investment list from the Workspace module in Morningstar
            Direct. The format is GUID. For example, "EBE416A3-03E0-4215-9B83-8D098D2A9C0D".

    Returns:
        DataFrame: A DataFrame object with the special investment list data. The DataFrame columns include:
...
def get_investment_lists() -> DataFrame:
    ...
    Returns:
        DataFrame: A DataFrame object with all lists data. The DataFrame columns include:
...
def save_investment_list(list_name: str, investment_ids: List[str], overwrite_if_exists: bool) -> DataFrame:
    ...
    Args:
        list_name (:obj:`str`): A name of the list
        investment_ids (:obj:`list`): An array of investment ids (SecId). For example: ["F00000YVYF","FOUSA00CFV"]
