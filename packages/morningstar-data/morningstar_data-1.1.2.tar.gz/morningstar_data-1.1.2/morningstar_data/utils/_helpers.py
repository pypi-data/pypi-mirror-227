...
from ..direct._exceptions import BadRequestException
...
def _get_user_id_from_uim_token() -> str:
    ...
    try:
        ...
        signing_key = jwks_client.get_signing_key_from_jwt(uim_jwt)
        ...
        _logger.error(f"Error decoding UIM token: {err}")
        raise
...
def _retrieve_credentials() -> Dict[str, str]:
    ...
    try:
        ...
        raise ForbiddenError("User does not have access to deliver function feature") from None
