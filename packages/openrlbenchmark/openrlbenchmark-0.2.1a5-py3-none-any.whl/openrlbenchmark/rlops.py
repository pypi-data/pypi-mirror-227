        ...
        if not self.offline:
            ...
            with self.offline_db.bind_ctx([OfflineRun, OfflineRunTag, Tag]):
                cond = (
                    ...
                    & (OfflineRun.config[self.custom_env_id_key.replace(".value", "")] == self.env_id)
                    & (OfflineRun.config[self.custom_exp_name_key.replace(".value", "")] == self.exp_name)
                )
                if self.username:
    ...
    if args.rliable:
        ...
        if args.rc.sample_efficiency_plots:
            ...
            fig_sample_efficiency, axes_sample_efficiency = plt.subplots(
                ncols=2,
                nrows=2,
                figsize=tuple([2 * item for item in figsize]),
