...
class AlignmentData:
    ...
    def setup(self) -> None:
        ...
        # speed filter
        self.traj_to.apply_index(self.traj_to.speed >= self.settings.preprocessing.min_speed)
        self.traj_from.apply_index(self.traj_from.speed >= self.settings.preprocessing.min_speed)

        if self.settings.preprocessing.time_start != 0 or self.settings.preprocessing.time_end != 0:
            time_span = (
                self.traj_from.tstamps[0] + self.settings.preprocessing.time_start,
                self.traj_from.tstamps[0] + self.settings.preprocessing.time_end,
            )
        ...
        match_trajectories(
            traj_test=self.traj_from, traj_ref=self.traj_to, settings=self.settings.preprocessing.matching_settings
        )
    ...
    def observation_groups(self) -> dict[str, tuple[Union[int, None], Union[int, None]]]:
        ...
        if not self.settings.estimation_of.time_shift_enabled:
            speed_indices = (None, None)
        elif self.settings.estimation_of.leverarm_enabled:
            speed_indices = (9, 12)
        else:
            speed_indices = (6, 9)
        ...
        return {
            ...
            "XYZ_TO": (3, 6),
            "POSITIONS": (0, 6),
            "ROLL_PITCH": (6, 8) if self.settings.estimation_of.leverarm_enabled else (None, None),
            "YAW": (8, 9) if self.settings.estimation_of.leverarm_enabled else (None, None),
            "RPY": (6, 9) if self.settings.estimation_of.leverarm_enabled else (None, None),
            "SPEED": speed_indices,
        }
    ...
    def _create_obs_vector(
        ...
        """
        if self.settings.estimation_of.leverarm_enabled and rot_from is None:
            raise ValueError(
        ...
        if self.settings.estimation_of.leverarm_enabled and rot_from is not None:
        ...
        if self.settings.estimation_of.time_shift_enabled:
            ...
            speed[:, not self.settings.estimation_of.time_shift_filter] = 0
    ...
    def _create_var_vector(self) -> np.ndarray:
        ...
        variances_xyz_from = np.ones((self.number_of_epochs, 3)) * self.settings.stochastics.var_xy_from
        variances_xyz_from[:, 2] = np.ones((self.number_of_epochs,)) * self.settings.stochastics.var_z_from

        variances_xyz_to = np.ones((self.number_of_epochs, 3)) * self.settings.stochastics.var_xy_to
        variances_xyz_to[:, 2] = np.ones((self.number_of_epochs,)) * self.settings.stochastics.var_z_to

        variances_rpy_body = np.ones((self.number_of_epochs, 3)) * self.settings.stochastics.var_roll_pitch
        variances_rpy_body[:, 2] = np.ones((self.number_of_epochs,)) * self.settings.stochastics.var_yaw

        variances_speed_to = np.ones((self.number_of_epochs, 3)) * self.settings.stochastics.var_speed_to
    ...
    def rpy_from(self) -> np.ndarray:
        if not self.settings.estimation_of.leverarm_enabled:
