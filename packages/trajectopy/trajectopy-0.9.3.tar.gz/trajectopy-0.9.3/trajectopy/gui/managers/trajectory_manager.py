...
class TrajectoryEntryPair:
    ...
    def comparison_settings(self) -> ComparisonSettings:
        """
        This function merges the comparison settings from the entry and the request.
        The request does not contain all settings because it is only used to specify the comparison type.
        More detailed settings are trajectory specific and are stored in the entry.
        """
        comparison_type = self.request.comparison_type
        comparison_type.matching_settings.max_distance = (
            self.entry.settings.comparison.type.matching_settings.max_distance
        )
        comparison_type.matching_settings.max_time_diff = (
            self.entry.settings.comparison.type.matching_settings.max_time_diff
        )
        return ComparisonSettings(
            type=self.request.comparison_type,
            relative_pair_distance=self.entry.settings.comparison.relative_pair_distance,
            relative_pair_time_difference=self.entry.settings.comparison.relative_pair_time_difference,
            use_all_pose_pairs=self.entry.settings.comparison.use_all_pose_pairs,
        )
...
class TrajectoryManager(QObject):
    ...
    @staticmethod
    def operation_compare(entry_pair: TrajectoryEntryPair) -> tuple[ResultEntry]:
        """
        ...
        comparison_result = compare_trajectories(
            ...
            settings=entry_pair.comparison_settings,
        )

        if isinstance(comparison_result, AbsoluteTrajectoryDeviations):
            return (AbsoluteDeviationEntry(deviations=comparison_result),)
        if isinstance(comparison_result, RelativeTrajectoryDeviations):
            return (RelativeDeviationEntry(deviations=comparison_result),)

        raise ValueError(f"Somehow got a comparison result of type {type(comparison_result)}")
    ...
    def operation_align(entry_pair: TrajectoryEntryPair) -> tuple[TrajectoryEntry, AlignmentEntry]:
        ...
        entry_pair.entry.settings.alignment.preprocessing.matching_settings = (
            entry_pair.comparison_settings.type.matching_settings
        )
    ...
    def operation_adopt_first_pose(entry_pair: TrajectoryEntryPair) -> tuple[TrajectoryEntry, ...]:
        ...
        traj_test, traj_ref = match_trajectories(
            traj_test=entry_pair.entry.trajectory,
            traj_ref=reference_entry.trajectory,
            settings=entry_pair.comparison_settings.type.matching_settings,
        )
