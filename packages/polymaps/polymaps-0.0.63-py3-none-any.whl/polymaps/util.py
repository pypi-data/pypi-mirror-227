...
def build_peo_peo_diblock(dop_peo, dop_ppo):
    monomer_mass_df, monomer_df, monomer_bond_df, monomer_angle_df, monomer_dihedral_df, monomer_improper_df, \
    [[monomer_xlo, monomer_xhi], [monomer_ylo, monomer_yhi], [monomer_zlo, monomer_zhi]], \
    monomer_lj_coeff, monomer_bond_coeff, monomer_angle_coeff, monomer_dihedral_coeff, monomer_improper_coeff = open_lmp_data('./eo_monomer.lmp', viz=False)
    monomer_cgenff = ['CG321', 'HGA2', 'HGA2', 'OG301', 'CG321', 'HGA2', 'HGA2', 'X']
    ...
    monomer_mass_df2['type'] = monomer_mass_df2['type'] + polymer_mass_df['type'].max() - 1
    monomer_df2['type'] = monomer_df2['type'] + polymer_mass_df['type'].max() - 1
    polymer_mass_df = pd.concat([polymer_mass_df.drop(polymer_mass_df.tail(1).index), monomer_mass_df2], axis=0, ignore_index=True)
    polymer_mass_df.index = polymer_mass_df.index + 1
...
def write_dfs_to_lmp_data(data_fname, mass_df, df, bond_df, angle_df, dihedral_df, improper_df, padding=1.0, force_cube=False):
...
""",
    ...
    bond_df[bond_df_columns + ['comment']].to_string(header=None, index=None) + '\n',
    angle_df[angle_df_columns + ['comment']].to_string(header=None, index=None) + '\n',
    dihedral_df[dihedral_df_columns + ['comment']].to_string(header=None, index=None) + '\n',
    improper_df[improper_df_columns + ['comment']].to_string(header=None, index=None) + '\n']
...
def viz_3d_scatter(xyz, markersize=10):
    xyzmin = xyz.min()
    xyzmax = xyz.max()
...
def gen_sphere_points(R, dx=5):
    ...
    for theta in np.linspace(0, np.pi, int(np.pi * R / dx) + 1):
        for phi in np.linspace(0, 2 * np.pi, int(2 * np.pi * R * np.sin(theta) / dx) + 1):
...
def attempt_to_pack_cube_box(solute_xyz, Lx, target_density, solute_total_mass, M_solvant, distance_limit=2.1, density_convergence=0.02, max_iter=50, verbose=False):
    ...
    last_3_Nx = [-1, -2, -3]
    N_solvant = int(((Volume * target_density * N_Avogadro) - solute_total_mass) / M_solvant)
    Nx = int((N_solvant) ** (1./3.) * (1 + compensation_coefficient))
    solvant_spacing = Lx / Nx

    grid3D = (np.array([np.repeat(np.arange(Nx), Nx*Nx), 
                        np.tile(np.repeat(np.arange(Nx), Nx), Nx), 
                        np.tile(np.arange(Nx), Nx*Nx)]).T) * solvant_spacing
    ...
    N_solvant_guessed = len(good_point_indices)
    curr_density = (solute_total_mass + (N_solvant_guessed * M_solvant)) / Volume / N_Avogadro
    best_case = {"solvant_coordinates": grid3D[good_point_indices, :], 
                 "N_solvant_guessed": N_solvant_guessed, 
    ...
    while np.abs(target_density - curr_density) > density_convergence:
        ...
        good_point_indices = list(set([x for x in range(0, grid3D.shape[0])])-set(idx1))
        N_solvant_guessed = len(good_point_indices)
        curr_density = (solute_total_mass + (N_solvant_guessed * M_solvant)) / ((Lx * 1e-8) ** 3) / N_Avogadro
        if np.abs(best_case["density"] - target_density) > np.abs(curr_density - target_density):
            best_case["density"] = curr_density
            best_case["N_solvant_guessed"] = N_solvant_guessed
            best_case["solvant_coordinates"] = grid3D[good_point_indices, :]
        if solvant_spacing < distance_limit:
            _upper_bound = compensation_coefficient
            compensation_coefficient = compensation_coefficient * 0.9
            if verbose:
                print("N_solvant_guessed:", N_solvant_guessed)
                print("solvant number difference:", np.abs(N_solvant_guessed - N_solvant))
...
def combine_solvent_solute(solute_dfs, solvant_dfs):
    ...
    solvant_mass_df, total_solvant_atom_df, total_solvant_bond_df, total_solvant_angle_df, total_solvant_dihedral_df, total_solvant_improper_df = solvant_dfs
    
    total_mass_df = pd.concat([solute_mass_df, solvant_mass_df], axis=0).reset_index(drop=True)
    total_mass_df.index = total_mass_df.index + 1
    total_mass_df["type"] = total_mass_df.index

    if type(total_solvant_atom_df) != type(None):
        total_solvant_atom_df.loc[:, ["type"]] = total_solvant_atom_df.loc[:, ["type"]] + total_solute_atom_df["type"].max()
        total_atom_df = pd.concat([total_solute_atom_df, total_solvant_atom_df], axis=0).reset_index(drop=True)
        total_atom_df.index = total_atom_df.index + 1
        total_atom_df["id"] = total_atom_df.index
    ...
    if type(total_solvant_bond_df) != type(None):
        total_solvant_bond_df.loc[:, ["at1", "at2"]] = total_solvant_bond_df.loc[:, ["at1", "at2"]] + total_solute_atom_number
        total_solvant_bond_df.loc[:, ["type"]] = total_solvant_bond_df.loc[:, ["type"]] + total_solute_bond_df["type"].max()
        total_bond_df = pd.concat([total_solute_bond_df, total_solvant_bond_df], axis=0).reset_index(drop=True)
        total_bond_df.index = total_bond_df.index + 1
        total_bond_df["id"] = total_bond_df.index
    else:
        total_bond_df = total_solute_bond_df
    
    if type(total_solvant_angle_df) != type(None):
        total_solvant_angle_df.loc[:, ["at1", "at2", "at3"]] = total_solvant_angle_df.loc[:, ["at1", "at2", "at3"]] + total_solute_atom_number
        total_solvant_angle_df.loc[:, ["type"]] = total_solvant_angle_df.loc[:, ["type"]] + total_solute_angle_df["type"].max()
        total_angle_df = pd.concat([total_solute_angle_df, total_solvant_angle_df], axis=0).reset_index(drop=True)
        total_angle_df.index = total_angle_df.index + 1
        total_angle_df["id"] = total_angle_df.index
    else:
        total_angle_df = total_solute_angle_df
        
    if type(total_solvant_dihedral_df) != type(None):
        total_solvant_dihedral_df.loc[:, ["at1", "at2", "at3", "at4"]] = total_solvant_dihedral_df.loc[:, ["at1", "at2", "at3", "at4"]] + total_solute_atom_number
        total_solvant_dihedral_df.loc[:, ["type"]] = total_solvant_dihedral_df.loc[:, ["type"]] + total_solute_dihedral_df["type"].max()
        total_dihedral_df = pd.concat([total_solute_dihedral_df, total_solvant_dihedral_df], axis=0).reset_index(drop=True)
        total_dihedral_df.index = total_dihedral_df.index + 1
        total_dihedral_df["id"] = total_dihedral_df.index
    else:
        total_dihedral_df = total_solute_dihedral_df

    if type(total_solvant_improper_df) != type(None):
        total_solvant_improper_df.loc[:, ["at1", "at2", "at3", "at4"]] = total_solvant_improper_df.loc[:, ["at1", "at2", "at3", "at4"]] + total_solute_atom_number
        total_solvant_improper_df.loc[:, ["type"]] = total_solvant_improper_df.loc[:, ["type"]] + total_solute_improper_df["type"].max()
        total_improper_df = pd.concat([total_solute_improper_df, total_solvant_improper_df], axis=0).reset_index(drop=True)
        total_improper_df.index = total_improper_df.index + 1
        total_improper_df["id"] = total_improper_df.index
    else:
...
def read_ff2pandas(fname):
    ...
    if len(bond_coeffs) > 0:
        ...
        bond_coeff_df = pd.read_csv(io.StringIO("\n".join(bond_coeff_data_strs)), sep=r"\s+", header=None, 
    ...
    if len(angle_coeffs) > 0:
        ...
        angle_coeff_df = pd.read_csv(io.StringIO("\n".join(angle_coeff_data_strs)), sep=r"\s+", header=None, 
    ...
    if len(dihedral_coeffs) > 0:
        ...
        dihedral_coeff_df = pd.read_csv(io.StringIO("\n".join(dihedral_coeff_data_strs)), sep=r"\s+", header=None, 
                                    names=["dihedral_coeff", "type", "Kchi", "n", "delta"]).fillna("")
        dihedral_coeff_df["#"] = "#"
    ...
    if len(improper_coeffs) > 0:
        ...
        improper_coeff_df = pd.read_csv(io.StringIO("\n".join(improper_coeff_data_strs)), sep=r"\s+", header=None, 
    ...
    return global_settings, pair_coeff_df, bond_coeff_df, angle_coeff_df, dihedral_coeff_df, improper_coeff_df