...
def write_dfs_to_lmp_data(data_fname, mass_df, df, bond_df, angle_df, dihedral_df, improper_df, padding=1.0):
...
def attempt_to_pack_cube_box(solute_xyz, Lx, target_density, solute_total_mass, M_solvant, distance_limit=2., density_convergence=0.02, max_iter=50, verbose=False):
    ...
    grid3D = (np.array([np.repeat(np.arange(Nx), Nx*Nx), 
              ...
              np.tile(np.arange(Nx), Nx*Nx)]).T - np.array([(Nx-1)/2., (Nx-1)/2., (Nx-1)/2.])) * solvant_spacing
    from scipy.spatial import distance_matrix
    distmat = distance_matrix(grid3D, solute_xyz)
    idx1, idx2 = (distmat < distance_limit).nonzero()
    N_solvant_guessed = grid3D.shape[0] - len(np.array([idx1, idx2]).astype(int).T.tolist())
    curr_density = (solute_total_mass + (N_solvant_guessed * M_solvant)) / Volume / N_Avogadro
    best_case = {"solvant_coordinates": grid3D[idx1, :], "N_solvant_guessed": N_solvant_guessed, "density": curr_density}
    while np.abs(target_density - curr_density) > density_convergence:
        ...
        if verbose:
            ...
            print("Nx:", Nx)
        solvant_spacing = Lx / Nx
        grid3D = np.array([np.tile(np.arange(Nx), Nx*Nx), 
                           np.tile(np.repeat(np.arange(Nx), Nx), Nx), 
                           np.repeat(np.arange(Nx), Nx*Nx)]).T * solvant_spacing
        distmat = distance_matrix(grid3D, solute_xyz)
        idx1, idx2 = (distmat < distance_limit).nonzero()
        N_solvant_guessed = grid3D.shape[0] - len(np.array([idx1, idx2]).astype(int).T.tolist())
        ...
        if np.abs(best_case["density"] - target_density) > np.abs(curr_density - target_density):
            ...
            best_case["solvant_coordinates"] = grid3D[idx1, :]
