...
def mix_ff_files(solute_ff_fname, solvent_ff_fname, total_atom_type_label_map):
    ...
    solvent_self_lj_coeff["cgenff"] = solvent_atom_type_labels
    ...
    mixed_pair_coeff = pd.DataFrame(mixed_lj_coeff, columns=["pair_coeff", "type1", "type2", "epsilon", "sigma", "epsilon14", "sigma14", "#", "comment"])
        ...
        for sj in solvent_s:
            if si[0] == sj[0]:
                ...
                if si[1] != sj[1]:
                    ...
                    mixed_settings.append(list(filter(None, [si[0], si[1], si[2]])))
    ...
    if "pair_style" in hybrid_settings.keys():
        ...
        total_pair_coeff = total_pair_coeff[[total_pair_coeff.columns[0], total_pair_coeff.columns[-1]] + total_pair_coeff.columns[2:-1].tolist()]
    else:
        total_pair_coeff = pd.concat([solute_pair_coeff, mixed_pair_coeff, solvent_pair_coeff], axis=0)
        total_pair_coeff["type1"] = total_pair_coeff["type1"].astype(int)
        total_pair_coeff["type2"] = total_pair_coeff["type2"].astype(int)
    ...
    if "bond_style" in hybrid_settings.keys():
        ...
        total_bond_coeff = total_bond_coeff[[total_bond_coeff.columns[0], total_bond_coeff.columns[-1]] + total_bond_coeff.columns[2:-1].tolist()]
    else:
        total_bond_coeff = pd.concat([solute_bond_coeff, solvent_bond_coeff], axis=0).reset_index(drop=True)
        total_bond_coeff.index = total_bond_coeff.index + 1
    ...
    if "angle_style" in hybrid_settings.keys():
        ...
        total_angle_coeff = total_angle_coeff[[total_angle_coeff.columns[0], total_angle_coeff.columns[-1]] + total_angle_coeff.columns[2:-1].tolist()]
    else:
        total_angle_coeff = pd.concat([solute_angle_coeff, solvent_angle_coeff], axis=0).reset_index(drop=True)
        total_angle_coeff.index = total_angle_coeff.index + 1
    ...
    if "dihedral_style" in hybrid_settings.keys():
        ...
        total_dihedral_coeff = total_dihedral_coeff[[total_dihedral_coeff.columns[0], total_dihedral_coeff.columns[-1]] + total_dihedral_coeff.columns[2:-1].tolist()]
    else:
        total_dihedral_coeff = pd.concat([solute_dihedral_coeff, solvent_dihedral_coeff], axis=0).reset_index(drop=True)
        total_dihedral_coeff.index = total_dihedral_coeff.index + 1
    ...
    if "improper_style" in hybrid_settings.keys():
        ...
        total_improper_coeff = total_improper_coeff[[total_improper_coeff.columns[0], total_improper_coeff.columns[-1]] + total_improper_coeff.columns[2:-1].tolist()]
    else:
        total_improper_coeff = pd.concat([solute_improper_coeff, solvent_improper_coeff], axis=0).reset_index(drop=True)
        total_improper_coeff.index = total_improper_coeff.index + 1
