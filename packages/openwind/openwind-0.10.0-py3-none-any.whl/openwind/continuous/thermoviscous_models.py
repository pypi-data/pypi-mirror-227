...
def losses_model(losses=True, loss_factor_alpha=1.0):
    ...
    if loss_factor_alpha != 1.0:
        assert losses in [True, 'bessel', 'bessel_new'] \
            or losses.startswith('diffrepr'), \
            ("loss_factor_alpha only available for ThermoviscousBessel or "
    ...
    if losses in [True, 'bessel']:
        return ThermoviscousBessel(use_new_stokes_number=False,
                                   loss_factor_alpha=loss_factor_alpha)
...
class ThermoviscousBessel(ThermoviscousModel):
    ...
    def __init__(self, use_new_stokes_number, loss_factor_alpha=1.0):
        self.use_new_stokes_number = use_new_stokes_number
        self.loss_factor_alpha = loss_factor_alpha
...
class ThermoviscousDiffusiveRepresentation(ThermoviscousModel):
    ...
    def __init__(self, diff_repr_coefs='diffrepr', loss_factor_alpha=1.0):
        ...
        # Multiply nondimensional frequency by loss_factor_alpha**2
        # This is equivalent to multiplying ai and bi (but not a0) by alpha**2.
        # See [Thibault2] equation (11)
        self._a0 *= loss_factor_alpha**-2
        self._bi *= loss_factor_alpha**2
