...
class RecordLinksField(JSONField):
    ...
    def process_formdata(self, valuelist):
        ...
        if valuelist:
            try:
                ...
                # Check if at least the basic structure of the data and the record ID
                # are valid and discard the data otherwise.
                if not isinstance(self.data, list):
                    ...
                    for item in self.data:
                        ...
                        if (
                            ...
                            or not type(item.get("record")) is int
                        ):
                            self.data = self.default
                            self._validation_errors = {}
                            break
    ...
    def to_dict(self):
        ...
        # Replace all validation messages that can realistically occur when using the
        # field to match the usual form validation.
        for error_data in data["errors"].values():
            term_error_msgs = error_data.get("term", [])

            for index, error_msg in enumerate(term_error_msgs):
                if error_msg.startswith("Longer than maximum length"):
                    translated_msg = ngettext(
                        "Field cannot be longer than %(num)d character.",
                        "Field cannot be longer than %(num)d characters.",
                        max_term_len,
                    )
                    term_error_msgs[index] = translated_msg
...
FORM_MSG_MISSING_DATA = "Missing data for required field."
FORM_MSG_RANGE_GTE = "Must be greater than or equal to %(min)s."
...
class ChunkForm(KadiForm):
    ...
    index = IntegerField(
        validators=[
            InputRequired(message=FORM_MSG_MISSING_DATA),
            NumberRange(
                min=Chunk.Meta.check_constraints["index"]["range"]["min"],
                message=FORM_MSG_RANGE_GTE,
            ),
        ]
    )
    ...
    blob = FileField(validators=[DataRequired(message=FORM_MSG_MISSING_DATA)])
...
class DirectUploadForm(KadiForm):
    ...
    name = StringField(
        filters=[normalize],
        validators=[
            DataRequired(message=FORM_MSG_MISSING_DATA),
            Length(
