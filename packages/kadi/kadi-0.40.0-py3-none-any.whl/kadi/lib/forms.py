...
class DynamicSelectField(CustomFieldMixin, _SelectField):
    ...
    default value (``None``) instead of trying to coerce them. In addition, the instance
    variable ``initial`` can be used to set an initial value to prefill the selection
    with.
    """
