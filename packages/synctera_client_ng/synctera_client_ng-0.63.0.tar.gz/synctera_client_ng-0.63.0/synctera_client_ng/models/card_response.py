...
class CardResponse(BaseModel):
    ...
    oneof_schema_1_validator: Optional[VirtualCardResponse] = None
    # data type: PhysicalCardResponse
    oneof_schema_2_validator: Optional[PhysicalCardResponse] = None
    actual_instance: Any
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, VirtualCardResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `VirtualCardResponse`")
        else:
            match += 1
        # validate data type: PhysicalCardResponse
        if not isinstance(v, PhysicalCardResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PhysicalCardResponse`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CardResponse with oneOf schemas: PhysicalCardResponse, VirtualCardResponse. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in CardResponse with oneOf schemas: PhysicalCardResponse, VirtualCardResponse. Details: " + ", ".join(error_messages))
        else:
            return v
