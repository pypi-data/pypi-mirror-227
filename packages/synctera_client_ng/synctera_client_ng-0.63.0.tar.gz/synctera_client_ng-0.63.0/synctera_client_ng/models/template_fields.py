...
class TemplateFields(BaseModel):
    ...
    oneof_schema_1_validator: Optional[TemplateFieldsChargeSecured] = None
    ...
    oneof_schema_3_validator: Optional[TemplateFieldsDepository] = None
    actual_instance: Any
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = TemplateFields.construct()
        error_messages = []
        match = 0
        # validate data type: TemplateFieldsChargeSecured
        if not isinstance(v, TemplateFieldsChargeSecured):
        ...
        if not isinstance(v, TemplateFieldsLineOfCredit):
        ...
        if not isinstance(v, TemplateFieldsDepository):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TemplateFieldsDepository`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TemplateFields with oneOf schemas: TemplateFieldsChargeSecured, TemplateFieldsDepository, TemplateFieldsLineOfCredit. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TemplateFields with oneOf schemas: TemplateFieldsChargeSecured, TemplateFieldsDepository, TemplateFieldsLineOfCredit. Details: " + ", ".join(error_messages))
        else:
            return v
