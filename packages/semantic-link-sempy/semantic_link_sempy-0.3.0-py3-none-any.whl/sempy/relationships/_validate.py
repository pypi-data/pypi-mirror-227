...
def list_relationship_violations(
    ...
    searches for inconsistencies with the specified relationship r.multiplicity.
    ...
    relationships : pandas.DataFrame
        ...
        "multiplicity", "from_table", "from_column", "to_table", "to_column", which matches the
...
def _list_relationship_violations(
    ...
    for r in relationships.itertuples():
        ...
        from_df_stats = stats_cache(r.from_table)
        to_df_stats = stats_cache(r.to_table)

        from_col_stats = from_df_stats[r.from_column]
        to_col_stats = to_df_stats[r.to_column]

        violation_keys = (r.multiplicity, r.from_table, r.from_column, r.to_table, r.to_column)

        # Empty entities are a frequent result of failed ingestion, and they deserve separate
        # clear diagnostic messages
        if from_col_stats.nrows == 0:
            errors.append((*violation_keys, "source empty", f"{r.from_table}[{r.from_column}] empty"))
        if to_col_stats.nrows == 0:
            errors.append((*violation_keys, "target empty", f"{r.to_table}[{r.to_column}] empty"))
        ...
        if r.multiplicity == Multiplicity.ONE_TO_ONE:
            if from_col_stats.max_value_count > 1:
                errors.append((*violation_keys, "source not PK", f"{r.from_table}[{r.from_column}] not unique"))
        if r.multiplicity in [Multiplicity.MANY_TO_ONE, Multiplicity.ONE_TO_ONE]:
            if to_col_stats.max_value_count > 1:
                errors.append((*violation_keys, "target not PK", f"{r.to_table}[{r.to_column}] not unique"))
        ...
        if null_count > 0:
            errors.append((*violation_keys, "null FK", f"{r.from_table}[{r.from_column}] has {null_count} nulls"))
        ...
        if from_col_stats.nunique > 0 and n_merged / from_col_stats.nunique < coverage_threshold:
            ...
            message = f"{n_missing} out of {from_col_stats.nunique} values in {r.from_table}[{r.from_column}] not present in {r.to_table}[{r.to_column}]: {missing_keys_list}"
            errors.append((*violation_keys, "partial join", message))

    if len(errors) == 0:
        print("No violations")

    return pd.DataFrame(errors, columns=['multiplicity', 'from_table', 'from_column', 'to_table', 'to_column', 'type', 'message'])
