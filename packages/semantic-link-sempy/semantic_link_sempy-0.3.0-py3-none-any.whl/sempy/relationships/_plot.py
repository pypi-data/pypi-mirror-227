...
def plot_relationship_metadata(
    ...
    metadata_df : pandas.DataFrame, default=None
        ...
        "from_table", "from_column", "to_table", "to_column", which matches the
    ...
    for r in metadata_df.itertuples():
        if r.multiplicity == Multiplicity.ONE_TO_ONE:
            attrs = {"dir": "both"}
        elif r.multiplicity == Multiplicity.MANY_TO_MANY:
            attrs = {"dir": "none"}
        else:
            attrs = {}
        ...
        relationship_columns[r.from_table].add(r.from_column)
        relationship_columns[r.to_table].add(r.to_column)

        if include_columns == 'none':
            # Edge is between tables
            g.edge(f'{r.from_table}', f'{r.to_table}', color='black', colorscheme="accent8", **attrs)
        else:
            ...
            g.edge(f'{r.from_table}:{r.from_column}', f'{r.to_table}:{r.to_column}', color='black', colorscheme="accent8", **attrs)
