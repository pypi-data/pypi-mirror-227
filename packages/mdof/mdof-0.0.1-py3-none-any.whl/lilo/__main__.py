# gets called when running lilo from the command line:
#    python -m lilo ...
#
import ast
import sys
...
HELP = """
lilo [-p|w <>...] <method> <event> <inputs> <outputs>
lilo [-p|w <>...] <method> -i <inputs> -o <outputs>
...
def parse_time(argi, config, channels, method=None):
    ...
    import lilo.spec
    from lilo.modal import spectrum_modes
    f = {
        "response": lilo.spec.response_transfer,
        "fourier":  lilo.spec.fourier_transfer,
    }[method]
...
#   config.update({"p"  :  5, "orm":  4})
    ...
    try:
        realization = lilo.system(inputs=inputs, outputs=outputs, full=True, method=method, **config)
        ss_modes = lilo.modal.system_modes(realization,dt,decimation=config.get("decimate",1))
