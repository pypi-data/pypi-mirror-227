...
from pymarks.datatypes import Bookmark
from pymarks.datatypes import RecordFromDB

if typing.TYPE_CHECKING:
    from pymarks.datatypes import RecordForDB

log = logging.getLogger(__name__)


def parse_record_from_db(b: RecordFromDB) -> Bookmark:
    return Bookmark(id=b[0], url=b[1], title=b[2], tags=b[3], desc=b[4])
...
def update_bookmark(cursor: Cursor, b: Bookmark) -> None:
    ...
    cursor.execute(query, (b.url, b.title, tags, b.desc, b.id))
...
def create_bookmark(cursor: Cursor, b: RecordForDB | Bookmark) -> Bookmark:
    ...
    tags = utils.parse_tags(b.tags)
    cursor.execute(
        query, (b.url, b.title, tags, b.desc, dt.now(tz=timezone.utc))
    )
