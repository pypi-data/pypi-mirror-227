...
class FactorEvaluator(FactorBacktest):
    ...
    def evaluate(self):
        ...
        facind = FactorIndicator(self.curraw.hist, self.fwd, self.sd, self.ed)
        self.ic = facind.ic()
...
class FactorIndicator:
    ...
    def __init__(self, fac, fwd, sd, ed):
        self.fac = fac
        self.fwd = fwd
        self.sd = sd
        self.ed = ed
    ...
    def ret(self):
        ...
        df_merge = pd.merge(df_fac, self.fwd, on=["K", "D"], how="left")
        df_merge = df_merge.rename(columns={self.fwd_ret_name: self.fac_fwd})
        # 计算分组收益率：日度
        self.ret_all = (
    ...
    def summary_turnover(self):
        ...
        turnover_LS = pd.DataFrame(
            [dict_turnover["LS"][self.fac_fwd].mean() * 100],
            columns=["turnover"],
            index=[self.fac_fwd],
        )
        turnover_top = pd.DataFrame(
            [dict_turnover["top"][self.fac_fwd].mean() * 100],
            columns=["turnover"],
            index=[self.fac_fwd],
        )
        turnover_bottom = pd.DataFrame(
            [dict_turnover["bottom"][self.fac_fwd].mean() * 100],
            columns=["turnover"],
            index=[self.fac_fwd],
        )
    ...
    def graph_coverage(self):
        ...
        df_cv[self.fac_fwd] = df_cv[self.fac_fwd].apply(lambda x: int(x))
        return df_cv
