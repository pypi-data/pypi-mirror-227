...
def check_input(self):
    ...
    if self.format_output.lower() not in self.options["available_format_output"]:
        err += [f"OfficeAELF.__init__() : formatting = {self.format_output}, not in {self.options['available_format_output']} ! I'll use '{self.options['available_format_output'][0]}'",]
        self.format_output = self.options['available_format_output'][0]
    #endIf

    if self.calendar.lower() not in self.options["available_calendar"]:
        err += [f"OfficeAELF.__init__() : calendar = {self.calendar}, not in {self.options['available_calendar']} !  I'll use '{self.options['available_calendar'][0]}'",]
        self.calendar = self.options['available_calendar'][0]
    #endIf

    if self.source.lower() not in self.options["available_source"]:
        err += [f"OfficeAELF.__init__() : source = {self.source}, not in {self.options['available_source']} ! I'll use '{self.options['available_source'][0]}'",]
        self.source = self.options['available_source'][0]
    #endIf

    if self.office.lower() not in self.options["available_office"]:
        err += [f"OfficeAELF.__init__() : office = {self.office}, not in {self.options['available_office']} ! STOP'",]
        self.office = None
    #endIf
