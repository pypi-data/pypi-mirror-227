...
def add_data(
    ...
    *,
    ...
    df_row_count = DEFAULT_APPROX_MAX_ROWS
...
def format_dataframe(
    ...
    ) and column_spec.feature_influence_col_name:
        # Rename feature influence column name to NLP_INFLUENCE_COL_NAME ('__truera_influences__')
        data = data.rename(
            columns={
                column_spec.feature_influence_col_name: NLP_INFLUENCE_COL_NAME
            }
        )
        column_spec = column_spec.copy(
            feature_influence_col_name=NLP_INFLUENCE_COL_NAME
        )
