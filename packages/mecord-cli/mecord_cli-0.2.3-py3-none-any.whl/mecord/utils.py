...
def notifyCounterIfNeed():
    ...
    if now_hour == 0 and len(data) > 2:
        ...
        notifyWechatRobot({
            "msgtype": "markdown",
            "markdown": {
                "content": f"机器<<font color=\"warning\">{socket.gethostname()}</font>>日报 \n\n\
                                >过去24小时执行任务<<font color=\"warning\">{s_cnt+f_cnt}</font>> \n\
                                >成功<font color=\"warning\">{s_cnt}</font> \n\
                                >失败<font color=\"warning\">{f_cnt}</font>"
            }
        })
