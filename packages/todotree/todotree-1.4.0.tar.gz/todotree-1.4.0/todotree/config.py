...
class Config:
    ...
    def _read_from_yaml(self, yaml_content: str):
        ...
        self.__section_main(yaml_object)
        self.__section_paths(yaml_object)
        self.__section_localization(yaml_object)
        self.__section_decorators(yaml_object)
        self.__section_tree(yaml_object)

    def __section_tree(self, yaml_object):
        if yaml_object.get('tree') is not None:
    ...
    def __section_decorators(self, yaml_object):
        if yaml_object.get('decorators') is not None:
            self.console = ConsolePrefixes(
                yaml_object['decorators'].get('enable_colors', self.console.enable_colors),
                yaml_object['decorators'].get('info', self.console.console_good),
                yaml_object['decorators'].get('warning', self.console.console_warn),
                yaml_object['decorators'].get('error', self.console.console_error))

    def __section_localization(self, yaml_object):
        if yaml_object.get('localization') is not None:
    ...
    def __section_paths(self, yaml_object):
        if yaml_object.get('paths') is not None:
            self.todo_folder = Path(yaml_object['paths'].get('folder', self.todo_folder)).expanduser()
            self.todo_file = Path(
                yaml_object['paths'].get('todo_file', Path(self.todo_folder) / "todo.txt")).expanduser()
            self.done_file = Path(
                yaml_object['paths'].get('done_file', Path(self.todo_folder) / "done.txt")).expanduser()
            self.project_tree_folder = Path(
                yaml_object['paths'].get('project_folder', self.project_tree_folder)).expanduser()
            self.addons_folder = Path(
                yaml_object['paths'].get('addons_folder', self.addons_folder)).expanduser()

    def __section_main(self, yaml_object):
        if yaml_object.get('main') is not None:
            self.enable_wishlist_folder = yaml_object['main'].get('enable_wishlist_folder',
                                                                  self.enable_wishlist_folder)
            self.enable_project_folder = yaml_object['main'].get('enable_project_folder',
