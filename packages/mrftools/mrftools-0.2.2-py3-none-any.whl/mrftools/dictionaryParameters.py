...
Tissue = np.dtype([('T1', np.float32), ('T2', np.float32), ('M0', np.float32)])

# From https://mri-q.com/uploads/3/4/5/7/34572113/normal_relaxation_times_at_3t.pdf
WHITE_MATTER_3T = np.array([(0.800, 0.040, 1.)],dtype=DictionaryEntry)
GREY_MATTER_3T = np.array([(1.200, 0.065, 1.)],dtype=DictionaryEntry)
CSF_3T = np.array([(3.000, 0.500, 1.)],dtype=DictionaryEntry)
FAT_3T = np.array([(0.250, 0.068, 1.)],dtype=DictionaryEntry)
MUSCLE_3T = np.array([(1.100, 0.044, 1.)],dtype=DictionaryEntry)
...
class DictionaryParameters:
    ...
    @staticmethod
    def Import(filename, name, version="dev"):
        if ".mrf" in filename:
            infile = h5py.File(filename, "r")
            dictionaryParametersGroup = infile["dictionaryParameters"][name]
            new_dictionary_parameters = DictionaryParameters(name, dictionaryParametersGroup["entries"][:], version=version)
            infile.close()
