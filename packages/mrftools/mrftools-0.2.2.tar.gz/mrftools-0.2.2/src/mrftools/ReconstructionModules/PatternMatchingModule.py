from ..types import ReconstructionModuleIOType, MapData
from ..dictionaryParameters import DictionaryEntry, Tissue
...
def PatternMatchingViaMaxInnerProductWithInterpolation(combined, dictionary, simulation, voxelsPerBatch=500, b1Binned=None, device=None,):
    if(device==None):
        if torch.cuda.is_available():
            ...
            device = torch.device("cpu")
    combined = torch.moveaxis(combined,-1,0) 
...
class PatternMatchingModule(ReconstructionModule):
    ...
    def ProcessImageToMap(self, inputData):
        if(self.mode == "interpolated"):
            outputData = PatternMatchingViaMaxInnerProductWithInterpolation(inputData, self.reconstructionParameters.simulation.dictionaryParameters, self.reconstructionParameters.simulation, self.voxelsPerBatch, None, self.device)
        else:
            return None
