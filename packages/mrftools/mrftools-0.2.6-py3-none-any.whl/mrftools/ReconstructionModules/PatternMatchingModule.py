...
class PatternMatchingModule(ReconstructionModule):
    ...
    def ProcessImageToMap(self, inputData):
        ...
        if(self.mode == "interpolated"):
            outputData = PatternMatchingViaMaxInnerProductWithInterpolation(combined, self.reconstructionParameters.simulation.dictionaryParameters, self.reconstructionParameters.simulation, self.voxelsPerBatch, None, self.device)
        else:
            return None
