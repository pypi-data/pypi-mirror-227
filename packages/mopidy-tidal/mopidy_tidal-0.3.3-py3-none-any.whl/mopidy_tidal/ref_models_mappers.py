...
from tidalapi import Album, Mix, Page, PageLink
...
def create_explore_directory(tidal_explore):
    res = [create_explore_entry(m) for m in tidal_explore]
    # Remove None/Unsupported entries
    res_filtered = [i for i in res if i is not None]
    return res_filtered


def create_explore_entry(tidal_explore):
    if hasattr(tidal_explore, "api_path"):
        # Objects containing api_path are usually pages and must be processed further
        return Ref.directory(
            uri="tidal:page:" + tidal_explore.api_path, name=tidal_explore.title
        )
    elif hasattr(tidal_explore, "artifact_id"):
        # Objects containing artifact_id can be viewed directly
        explore_id = tidal_explore.artifact_id
        name = f"{tidal_explore.short_header} ({tidal_explore.short_sub_header})"
        if tidal_explore.type == "PLAYLIST":
            return Ref.directory(uri="tidal:playlist:" + explore_id, name=name)
        else:
...
def create_mixed_directory(tidal_mixed):
    return [create_mixed_entry(m) for m in tidal_mixed]


def create_page_directory(tidal_page):
    if isinstance(tidal_page, PageLink):
        res = [create_mixed_entry(m) for m in tidal_page.get()]
        # Remove None/Unsupported entries
        res_filtered = [i for i in res if i is not None]
        return res_filtered
...
def create_mixed_entry(tidal_mixed):
    if isinstance(tidal_mixed, Mix):
        ...
        if hasattr(tidal_mixed, "api_path"):
            ...
            # Unsupported type (eg. individual track)
            return None
