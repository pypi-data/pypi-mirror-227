...
from azureml.rag.utils.logging import get_logger, safe_mlflow_log_metric, track_activity
...
class EmbeddingsContainer:
    ...
    def _get_embeddings_internal(self, input_documents: Union[Iterator[Document], BaseLoader, DocumentChunksIterator]) -> OrderedDict:
        ...
        for document in input_documents:
            ...
            if mtime \
                ...
                safe_mlflow_log_metric("Documents Reused", len(documents_embedded.keys()), logger=logger, step=int(time.time() * 1000))
                continue
            ...
            safe_mlflow_log_metric("Documents to embed", len(documents_to_embed), logger=logger, step=int(time.time() * 1000))
    ...
    def as_faiss_index(self, engine: str = "langchain.vectorstores.FAISS"):
        ...
        if engine == "langchain.vectorstores.FAISS":
            ...
            FaissClass = FAISS
            import_faiss_or_help_me = dependable_faiss_import
        elif engine == "azureml.rag.indexes.faiss.FaissAndDocStore":
            from azureml.rag.docstore import FileBasedDocstore
            from azureml.rag.indexes.faiss import FaissAndDocStore, import_faiss_or_help_me
