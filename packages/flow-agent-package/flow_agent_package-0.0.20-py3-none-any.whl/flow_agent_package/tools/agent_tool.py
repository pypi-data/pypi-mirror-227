...
from flow_agent_package.tools.constants import format_response, intent_prompt, intent_prompt_with_history, extract_input
...
class ArbitrationAgent:
  ...
  def __init__(self, arbitration_method, agent_connection, deployment_name, return_direct, skills, history):
  ...
  def use_langchain(self, query):
    ...
    for skill in self.skills:
      ...
      lang_tool = Tool(
        name=skill.name,
        func=run_func,
        description=skill.tool_description, #TODO: Potentially use openai function desc - check with Prakhar
  ...
  def use_semantic_kernel(self, query):
    ...
    orchestratorPlugin = kernel.import_skill(
        OrchestratorPlugin(kernel, self.return_direct), "OrchestratorPlugin"
    )
...
def agent(
  ...
  arbitration_agent = ArbitrationAgent(arbitration_method, llm_connection, deployment_name, return_direct, agent_skills, chat_history)
