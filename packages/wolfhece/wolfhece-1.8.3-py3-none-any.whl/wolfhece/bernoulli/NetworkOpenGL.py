...
#D:\ProgThomas\wolf_oo\Sources-Thomas3\Modules
#python -m numpy.f2py -c -m fib3 D:\ProgThomas\wolf_oo\Sources-Thomas3\Modules\Python_Test.f90 --fcompiler=intelvem --f90exec=C:\Program Files (x86)\Intel\oneAPI\compiler\2021.1.1\windows\bin\intel64
#python -m numpy.f2py -c -m fib3 D:\ProgThomas\wolf_oo\Sources-Thomas3\Modules\Python_Test.f90 --fcompiler=ifort
#f2py -c --f90exec=%IFORT_COMPILER19%bin\intel64
#f2py -c D:\ProgThomas\wolf_oo\Sources-Thomas3\Modules\Python_Test.f90 -m fib4 --fcompiler=intelvem -L"C:\Program Files (x86)\Intel\oneAPI\compiler\2021.1.1\windows\compiler\lib\intel64_win" -L"C:\Program Files (x86)\Intel\oneAPI\mkl\2021.1.1\lib\intel64" --build-dir "D:\ProgThomas\wolf_oo\Sources-Thomas3\Python\PyWolfObjects"
#python -m numpy.f2py -c D:\ProgThomas\wolf_oo\Sources-Thomas3\Modules\Python_Test.f90 -m fib8 --fcompiler=intelvem -L"C:\Program Files (x86)\Intel\oneAPI\compiler\2021.1.1\windows\compiler\lib\intel64_win" 
...
#python -m numpy.f2py -c Python_Test.f90 -m Pyth_bern --fcompiler=intelvem -L"C:\Program Files (x86)\Intel\oneAPI\compiler\2021.1.1\windows\compiler\lib\intel64_win" 
...
class Window_Test(wx.Frame):
    ...
    def InitUI(self):
        ...
        SaveNet=fileMenu.Append(wx.ID_SAVE, '&Save')
        Launch=fileMenu.Append(wx.ID_ANY, '&Launch Simulation')
        fileMenu.AppendSeparator()
       
        imp = wx.Menu()
        self.Network0D=False
        NWoE=imp.Append(wx.ID_ANY, 'Import EPANET')
        ...
        tundo = self.toolbar.AddTool(wx.ID_UNDO, '',wx.Bitmap('D:\ProgThomas\wolf_oo\Sources-Thomas3\Python\PyWolfObjects\exit8.png'))
        tredo = self.toolbar.AddTool(wx.ID_REDO, '',wx.Bitmap('D:\ProgThomas\wolf_oo\Sources-Thomas3\Python\PyWolfObjects\exit7.png'))
        self.toolbar.EnableTool(wx.ID_REDO, False)
        self.toolbar.AddSeparator()
        texit = self.toolbar.AddTool(wx.ID_EXIT, '', wx.Bitmap('D:\ProgThomas\wolf_oo\Sources-Thomas3\Python\PyWolfObjects\exit3.png'))
    ...
    def OpenNetwork(self, e):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if Test==1:
                ...
                print('-The graph of the network is included')
                ...
                if(self.Zones!=False):
                    ...
                    print('-List of nodes and edges in the network are identified')
                    end  = time.time()
                    TTTime[0]=end-start
                    title_frame='Network 0D'
                    #Affichage du réseau
                    start = time.time()
                    self.Show_Network()
                    end  = time.time()
                    ...
                    print('-General Parameters of the network are included')
                    #Patterns dans le menu à ajouter
                    self.Patterns_Exchange_Menu()
                    print('-Patterns of the network are included')
                    end  = time.time()
                    TTTime[2]=end-start
    ...
    def Menu_Network(self):
        ...
        NewPattern=AddMenu.Append(wx.ID_ANY, '&New Pattern')
        ...
        self.menubar.Append(ResearchMenu, '&General')
        self.SetMenuBar(self.menubar)
        Element=ResearchMenu.Append(wx.ID_ANY, '&Trouver Element')
        Recherche=ResearchMenu.Append(wx.ID_ANY, '&Rechercher Element')
        ...
        ZoomELem=ResearchMenu.Append(wx.ID_ANY, '&Zoom')
        self.Bind(wx.EVT_MENU, self.Zoom_Elem, ZoomELem)
        PauseZoomELem=ResearchMenu.Append(wx.ID_ANY, '&Pause Zoom')
        self.Bind(wx.EVT_MENU, self.PauseZoom_Elem, PauseZoomELem)
        IDPause=self.menubar.FindMenuItem('General','Pause Zoom')
        self.menubar.Enable(IDPause,False)

    #Ouverture de fichier ".vecz"
    def ImportEPanet(self, e):
        dlg = wx.FileDialog(self,"Choose .inp file", wildcard="inp (*.inp)|*.inp|all (*.*)|*.*")
        if dlg.ShowModal() == wx.ID_OK:
    ...
    def Prepare_Network(self,pathname):
        ...
        self.GeneralParam=Wolf_Param(self)
        Test=1
    ...
    def analyse_params(self):
        ...
        if(not(hasattr(self,'mypalo'))):
            ...
            self.mypalo.nb,self.mypalo.values,self.mypalo._segmentdata,self.mypalo.colorsflt=self.mypalo.export_palette_matplotlib_new('RdYlGn')
...
class NewEdgeFrame(wx.Frame):
    ...
    def __init__(self, title, parent):
       ...
       if(NbNodes<2):
           print('The number of nodes in the network is not sufficient to create links')
       else:
    ...
    def Saved_Edge(self):
       ...
       if(Ident_Type==1):
           ...
           print('Le type de l élément n est pas valide')
       return Test
            #Remplacement via les données réelles
    ...
    def OnCloseWindow(self, event):
       #On va rechercher si un évènement de modification a été identifié
        if(self.modified_grid):
            print('Grid is modified')
        else:
            print('Grid is not modified')
        if(self.modified_grid):
...
def Import_vecz(filename):
    ...
    if(os.path.isfile(filename)):
        ...
        print('The .vecz file is not found.')
        Zones=False
    return Zones
...
def Define_Node_Pos(Nodes,Type_Node,List_Nodes): 
    ...
    for NodeName in Nodes[Type_Node]:
        ...
        if(dist<0.1):
            ...
            print('Node %s is not connected to the network' % Nodes[Type_Node][NodeName]['IndPos'])
...
def Add_Node_Time3(Nodes,Type_Node,LocDir,Param):
    ...
    if(File_To_Read):
        ...
        print('The following file does not exist : %s' % LocFile)
    return Nodes
...
def Export_Node_Attributes(Nodes,Path):
    ...
    if(not(os.path.isdir(Directory[0]))):
        try:
            ...
            print ("Creation of the directory %s failed" % Directory[0])
    ...
    if(bool(Nodes['IncJunctions'])):
        ...
        if(not(os.path.isdir(Directory[1]))):
            try:
                ...
                print ("Creation of the directory %s failed" % Directory[1])
    #Deuxième étape est donc d'ajouter les différents fichiers d'attributs
    Attributes=[[],[]]  
    Type=['Reservoirs','IncJunctions']
    ListFilename=[[],[]]
...
if __name__=='__main__':
    ...
    #wolfpy.implement_network(filename,filename2)
    app = wx.App()
    ex = Window_Test(None)
    ex.Show()
    app.MainLoop()
