...
class LambdaExecutor:
    ...
    #     #
    #     # TODO: Too slow, need to use thread pool to speed upload
    #     # maybe using a thread pool e.g. https://www.linkedin.com/pulse/using-python-upload-files-s3-parallel-tom-reid
    #     #
    ...
    def __upload_files_threaded(
            self,
            local_directory,
            object_prefix
    ):
        print('Uploading files using threading pool')
        # maybe using a thread pool e.g. https://www.linkedin.com/pulse/using-python-upload-files-s3-parallel-tom-reid
        #
        all_files = []
        for subdir, dirs, files in os.walk(local_directory):
        ...
        all_files
        #
        s3_client = self.__s3.get_client(
            access_key_id=self.__output_bucket_access_key,
            secret_access_key=self.__output_bucket_secret_access_key
        )
        all_uploads = [] # https://stackoverflow.com/questions/56639630/how-can-i-increase-my-aws-s3-upload-speed-when-using-boto3
        try:
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                futures = [executor.submit(
                    self.__s3.upload_file_with_client, # TODO: problem here is that I am creating a new s3_client for each file
                    s3_client,
                    all_file[0],
    ...
    def execute(self, input_message):
        ...
        try:
            ...
            self.__upload_files_threaded(store_name, output_zarr_prefix)
            print(datetime.now())
            #######################################################################
