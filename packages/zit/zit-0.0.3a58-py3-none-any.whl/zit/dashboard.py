...
def check_and_install_node_nvm():
    ...
    if reason:
        ...
        try:
            nvm_version = run_subprocess(
                f'. {os.path.expanduser("~/.nvm/nvm.sh")} && nvm --version', cwd=None, shell=True, return_stdout=True
            )
        ...
        installed = False
        try:
            node_version = run_subprocess(
                f'. {os.path.expanduser("~/.nvm/nvm.sh")} && node --version', cwd=None, shell=True, return_stdout=True
            )
        ...
        if not installed:
            ...
            try:
                ...
                os.makedirs(os.path.expanduser("~/.nvm/.cache/bin"), exist_ok=True)
                logger.info("Installing Node.js 18.4.0")
                run_subprocess(
                    "curl -L -O https://zityspace-public.s3.cn-north-1.amazonaws.com.cn/node-v18.4.0-linux-x64.tar.gz",
                    cwd=os.path.expanduser("~/.nvm/.cache/bin"),
                    shell=True,
                )

                os.makedirs(os.path.expanduser("~/.nvm/versions/node/v18.4.0"), exist_ok=True)
                logger.info("Downloaded Node.js 18.4.0 tarball, extracting...")
                run_subprocess(
                    (
                        "tar zxf"
                        f" {os.path.join(os.path.expanduser('~/.nvm/.cache/bin'), 'node-v18.4.0-linux-x64.tar.gz')} -C"
                        f" {os.path.expanduser('~/.nvm/versions/node/v18.4.0/')} --strip-components=1 && ."
                        f" {os.path.expanduser('~/.nvm/nvm.sh')} && nvm install 18.4.0 && nvm use 18.4.0"
                    ),
                    cwd=None,
                    shell=True,
                )
