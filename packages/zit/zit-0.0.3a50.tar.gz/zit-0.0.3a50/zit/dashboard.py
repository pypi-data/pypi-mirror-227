...
def check_and_install_node_nvm():
    ...
    if reason:
        ...
        try:
            nvm_version = run_subprocess("nvm --version", cwd=None, shell=True, return_stdout=True)
            ...
            try:
                ...
                run_subprocess(install_command, cwd=None, shell=True)
                logger.info(
                    "NVM has been installed successfully. Please restart your terminal and re-run the'zit dashboard"
                    " install'"
                )
                sys.exit(1)
            except SubprocessError as e:
        ...
        installed = False
        try:
            node_version = run_subprocess("node --version", cwd=None, shell=True, return_stdout=True)
