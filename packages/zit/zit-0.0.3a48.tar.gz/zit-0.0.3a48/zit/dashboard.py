...
def check_and_install_node_nvm():
    ...
    if reason:
        ...
        try:
            nvm_version = run_subprocess(load_command, cwd=None, shell=True, return_stdout=True)
...
def check_and_install_npm():
    try:
        ...
        install_command = (
            f'bash -c "set -o pipefail && . {os.path.expanduser("~/.nvm/nvm.sh")} && nvm use 18.4.0 && curl -qL'
...
def check_and_install_pnpm():
    try:
        ...
        install_command = f'. {os.path.expanduser("~/.nvm/nvm.sh")} && npm install -g pnpm'
