...
def vary_e_field_in_certain_direction(c1, c2, c3, var_range, type_coordinates = "cartesian", type_space = "linear"):
    ...
    if type_space == "linear":                                               #Creating the space for the variation
        ...
        #if type_space.lower() == "log":
            #base = 2
        #else: 
            #base = float(type_space.replace("log", ""))
            #type_space = "log"
        #var_range[2] = int(var_range[2])
        #space = np.logspace(np.log(var_range[0])/np.log(base), np.log(var_range[1])/np.log(base), var_range[2], base = base)
