...
from .utils import binomial, grid_spacing, normalize, rbf_interpolate, RBF_DEG, Spacing
...
class FoilGeom:
    ...
    def _evaluate(self):
        ...
        if self.n_side is not None:
            deg = RBF_DEG[self.spacing_kwargs.get("interp", "linear")]
            x_upper = grid_spacing(self.n_side, **self.spacing_kwargs)
            x_lower = x_upper[:]
            y_upper = np.array(
                [rbf_interpolate(xi, x_upper, y_upper, deg) for xi in x_upper]
            )
            y_lower = np.array(
                [rbf_interpolate(xi, x_lower, y_lower, deg) for xi in x_lower]
            )

        if self.closedTE:
            y_upper -= x_upper * dte / 2
            y_lower += x_lower * dte / 2
    ...
    def _check_isfoil(self):
        ...
        """
        x = grid_spacing(100, spacing=Spacing.COSINE)
        yup = np.array([rbf_interpolate(xi, self.x_upper, self.y_upper) for xi in x])
        ylo = np.array([rbf_interpolate(xi, self.x_lower, self.y_lower) for xi in x])
        dy = yup - ylo
        # TODO: why is this failed in action but passed the local test?!
