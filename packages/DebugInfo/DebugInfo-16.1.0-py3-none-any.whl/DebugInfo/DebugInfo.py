...
class 调试模板:
    ...
    def 表格列数(self) -> int:
        if not self.__表格:
            ...
            return max(1, max([len(行) for 行 in self.__表格 if isinstance(行, list or tuple)]))
    ...
    def 表格列宽(self) -> list[int]:
        ...
        展开的表格: list[list[str] or callable] = self.__表格展开()

        # 计算 展开的表格 中每一行中列数的最大值
        总列数: int = 0
        if 展开的表格:
            总列数 = max(1, max([len(行元素) for 行元素 in 展开的表格 if isinstance(行元素, list or tuple)]))
        ...
        各列最长字符串显示长度 = [0] * 总列数
        for 行元素 in 展开的表格:
            if not isinstance(行元素, list or tuple):
                # 如果行元素不是 list 或者 tuple, 则不参与计算列宽
                continue
    ...
    def 添加分隔行(self, 填充字符: str = '-', 修饰方法: _Callable[[str], str] = None, 重复: bool = None) -> None:
        ...
        :param 修饰方法: 可以为这一行的内容指定一个修饰的方法
        :param 重复: 填充字符是否自动重复, 以匹配表格的宽度
        :return: None
        """
        self.__表格.append(self.__表格分隔器(填充字符=填充字符, 重复=重复, 修饰方法=修饰方法))
        return None

    def 修改指定行(self, 行号: int, 列表: list[str] = None):
        """
    ...
    def 左右颠倒表格(self):
        ...
        """
        if self.__表格:
            表格最大行数: int = max(1, max([len(表格行) for 表格行 in self.__表格 if isinstance(表格行, list or tuple)]))

            临时表格: list = []
            for 表格行 in self.__表格:
                if not isinstance(表格行, list or tuple):
                    临时表格.append(表格行)
                else:
    ...
    def 表格宽度(self, 列间距: list[int] or int = None) -> int:
        ...
        # 计算 展开的表格 中每一行中列数的最大值, 不计算非 list 或者 tuple 行
        总列数: int = 0
        if 展开的表格:
            总列数 = max(1, max([len(行元素) for 行元素 in 展开的表格 if isinstance(行元素, list or tuple)]))
    ...
    def __修饰最后一行(self, 方法: _Callable[[str], str] or list[_Callable[[str], str]] = None) -> None:
        if not self.__表格:
            return None
        if not isinstance(self.__表格[-1], list or tuple):
        ...
        if 修饰方法表:
            if not isinstance(self.__表格[-1], list or tuple):
                元素 = str(self.__表格[-1]).strip()
                for 方子 in 修饰方法表:
                    元素 = str(方子(元素)).strip()
                self.__表格[-1] = 元素
            else:
                for 序号 in range(len(self.__表格[-1])):
                    元素: str = str(self.__表格[-1][序号]).strip()
