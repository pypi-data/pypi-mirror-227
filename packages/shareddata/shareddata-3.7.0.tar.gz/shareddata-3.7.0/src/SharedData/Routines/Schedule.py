...
class Schedule:
    ...
    def LoadSchedule(self):
        ...
        sched['status'] = np.nan
        sched['lastmsg'] = np.nan
        sched['runmsgts'] = pd.NaT
        sched['lastmsgts'] = np.nan
    ...
    def UpdateRoutinesStatus(self):
        ...
        idx = (idx) & ((sched['status'].isnull()) | (sched['isrealtime'])
                       | (sched['status'] == 'WAITING DEPENDENCIES'))
        dfpending = sched[idx]
    ...
    def RunPendingRoutines(self):
        ...
        dfpending = (dfpending) | (sched[sched['status'] == 'START'])
        dfpending = (dfpending) | (sched[sched['status'] == 'RESTART'])
