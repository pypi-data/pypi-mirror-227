...
class PluginOptions(Namespace):
    ...
    * `ignore_auto_headers=false`:
    ...
    !!!note "Rationale"
        Users are encourage to use the plugin's identifiers but:

        1. They could use bare anchors to write links within the current document.
        1. They could have added the plugin to an old project using the "old-fashion" addresses.

        By essence, neither of these situations is problematic. However, when
        [`ignore_auto_headers`][mkdocs_addresses.config_plugin.PluginOptions.ignore_auto_headers]
        is set to true (which is the default value...), the plugin cannot anymore know about the
        ...
        in mind this is discouraged because doing so it significantly reduces the level of
