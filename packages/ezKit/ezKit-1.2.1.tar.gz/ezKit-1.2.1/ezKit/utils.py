...
def git_clone(
    ...
    try:
        ...
        # 获取应用程序Git仓库
        if v_true(debug, bool):
            logger.info(f'[{info}]Git仓库: {git_repository}')
            logger.info(f'[{info}]本地仓库: {local_repository}')
        ...
        if result_code == 0:
            logger.success(f'[{info}]克隆成功') if v_true(debug, bool) else next
            if v_true(debug, bool):
                for i in result_info:
                    logger.success(f'[{info}]{i}') if v_true(debug, bool) else next
            return True
        else:
            logger.error(f'[{info}]克隆失败') if v_true(debug, bool) else next
            if v_true(debug, bool):
                for i in result_info:
                    logger.error(f'[{info}]{i}') if v_true(debug, bool) else next
            return False
    except Exception as e:
        logger.error(f'[{info}]克隆失败') if v_true(debug, bool) else next
        logger.exception(e) if v_true(debug, bool) else next
