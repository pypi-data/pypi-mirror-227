...
from chemchef.engine.indexing import FuzzyIndex, FuzzyIndexCollection, Exact, Fuzzy
...
def test_exact_query() -> None:
    ...
    expr = Exact(field="Name", target="Alice")
...
def test_exact_query_with_nonexistent_field() -> None:
    indexes = create_fuzzy_index_collection()
    expr = Exact(field="Non-existent", target="foo")
...
def test_and_query() -> None:
    indexes = create_fuzzy_index_collection()
    expr = Fuzzy(field="Name", target="BBB") & Exact(field="Properties", target="British")
    results = expr.to_query().run(indexes)
    assert results == {1}
...
def test_and_query_v2() -> None:
    indexes = create_fuzzy_index_collection()
    expr = Fuzzy(field="Name", target="BBB") & Exact(field="Properties", target="American")
    results = expr.to_query().run(indexes)
    assert results == set()
