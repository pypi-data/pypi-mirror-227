...
def parse_assistant_message(message_contents: str) -> SynonymsHyponymsHypernymsTuple:
...
class OpenAIKeywordSimilarityTest(AbstractKeywordSimilarityTest):
    ...
    def find_matches(self, target_keyword: str, candidate_keywords: Iterable[str]) -> set[str]:
        ...
        response_tuple = self._parse_response(response)
    ...
    @staticmethod
    def _parse_response(response: AssistantMessage) -> SynonymsHyponymsHypernymsTuple:
        logging.debug("Response from ChatGPT for similarity test: %s", response.content)
        return parse_assistant_message(response.content)
