...
class FuzzyIndex:
    ...
    def exact_query(self, target_keyword: str) -> set[int]:
        """
        :param target_keyword: The target keyword
        :return: The set of document ids that are associated with precisely this keyword
        """
        if target_keyword in self._keywords_to_document_ids:
            return set(self._keywords_to_document_ids[target_keyword])
        else:
            return set()
...
class FuzzyIndexCollection:
    ...
    def exact_query(self, field_name: str, target_keyword: str) -> set[int]:
        """
        :param field_name: name of field to run query on
        :param target_keyword: The target keyword
        :return: The set of document ids that are associated with this keyword
        """
        if field_name not in self._fields_to_fuzzy_indexes.keys():
            raise KeyError(f"Field name does not exist in index: {field_name}")

        return self._fields_to_fuzzy_indexes[field_name].exact_query(target_keyword)
