...
def build_session_from_model(model: BaseModel,
    ...
    """
    session_id = GlobalSettingsModelSingleton.get().session.id
    session_description = GlobalSettingsModelSingleton.get().session.description

    default_id = session_ctx.get('id', 'default_id')
    default_description = session_ctx.get('description', 'default description')
    if isinstance(model, BaseActionModel):
        ...
        session_model = SessionModel(id=session_id, description=session_description,
                                     specifications=[specification_model])
        ...
        session_model = SessionModel(id=session_id, description=session_description, specifications=[model])
    else:
...
def build_session_from_tags(target_tags: Set[str],
    ...
    for template_path in get_all_template_paths(manifests_folder_path):
        ...
        if model_name and model_name == 'SpecificationModel' and target_tags <= tags:
            ...
            specification_model: SpecificationModel = deserialise_manifest_to_model( # noqa - must be SpecificationModel
                template_path,
                manifests_folder_path=manifests_folder_path,
                context=None,
                json_dump=False,
