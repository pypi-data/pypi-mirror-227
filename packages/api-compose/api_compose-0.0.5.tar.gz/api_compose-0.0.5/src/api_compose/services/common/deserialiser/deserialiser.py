...
def deserialise_manifest_to_dict(
    ...
    context_merged = overlay_dict(overlayed_dict=context_merged,
                                  overlaying_dict=dict(GlobalSettingsModelSingleton.get().cli_context))
    ...
    str_, is_success, exec = jinja_engine.set_template_by_file_path(template_file_path=str(relative_template_path),
