...
from utils.config import get_conf, get_default, DefaultLocalIp, CommonConf
...
class _RegistryConfig(Cache):
    def __init__(self):
        ...
        # {
        #     "tag_a on server": {
        #         AIA: {Service("10.0.0.1", "9004")},
        #         AIB: {Service("10.0.0.2", "9004")},
        #     },
        #     "proj_name": {
        #         "ProjA": {Service("10.0.0.2", "9004"), Service("10.0.0.2", "9004")}
        #     }
        # }
    ...
    def _load_server(self, conf):
        ...
        service_tag = get_default(conf, "service_tag", {
            f"{DefaultLocalIp}:{CommonConf.port}": {}
        })
    ...
    def _load_client(self, conf):
        # 客户端
        address = get_default(conf, "address", {"ip": DefaultLocalIp, "port": CommonConf.port})
    ...
    def filter_server(self, tags: dict[str, set]) -> set[Service]:
        """
        tag对应能力
        服务器如果标了某项tag，表示该服务器只能处理该类型已标识的能力
        如果没标，则表示该类型的能力都能支持
        如果服务器没有任务标签，则表示通用服务器
        :param tags: 需要满足的tag
        :return:
        """
        result = set()
        for tag, values in tags.items():
            ...
            if not source:
                # 如tag_name，客户端使用的tag项，服务端没有该tag项（下面无任何机器），忽略该tag限制
