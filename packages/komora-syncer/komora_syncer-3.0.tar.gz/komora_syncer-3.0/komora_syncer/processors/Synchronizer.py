...
class Synchronizer:
    ...
    def __prepare_assigned_regions(self):
        ...
        municipalities_on_sites = [[site.address.municipalityName, site.address.districtName] for site in self.sites if site.address]
        self.municipalities_assigned = [muni for muni in self.municipalities_all if [muni.name, muni.districtName] in municipalities_on_sites]

        districts_on_assigned_municipalities = set(municipality.districtName for municipality in self.municipalities_assigned)
        self.districts_assigned = [dis for dis in self.districts_all if dis.name in districts_on_assigned_municipalities]
