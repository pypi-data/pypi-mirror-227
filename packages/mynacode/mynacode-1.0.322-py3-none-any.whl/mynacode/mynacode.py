    ...
    if model:
      ...
      if goal == 'maximize':
        if metric_value > prev_max:
          ...
          results_dict.update({'val_metric_name': metric_name})
          results_dict.update({'best_val_'+metric_name: metric_value})
          results_dict.update({'best_metric_index': index_count})
          results_dict.update({'best_val_epoch': current_epoch+1})
          results_dict.update({'val_metric_goal': 'maximize'})
          model_global = model
        ...
        if metric_value < prev_min:
          ...
          results_dict.update({'best_metric_index': index_count})
          results_dict.update({'best_val_epoch': current_epoch+1})
          results_dict.update({'val_metric_goal': 'minimize'})
          model_global = model
          save_best_results = True
...
def data(train_set=[], train_labels=[], test_set=[], test_labels=[], val_set=[], val_labels=[], dataset_name="", #checks if data is same as prev
  ...
  if (len(train_labels) > 0) and (os.path.exists(newest_data_folder + '/train_labels.pkl')):
      percent_20 = int((20/100)*len(train_labels))
      
      with open(newest_data_folder + '/train_labels.pkl', 'rb') as f:
          x = dill.load(f)

          if not np.array_equal(x[:percent_20], train_labels[:percent_20]):
          ...
          if self.metric_name not in logs.keys():
            ...
            if self.goal == 'maximize':
              if logs.get(self.metric_name) > prev_max:
                ...
                results_dict.update({'val_metric_name': self.metric_name})
                results_dict.update({'best_val_'+self.metric_name: logs.get(self.metric_name)})
                results_dict.update({'best_metric_index': index_count})
                results_dict.update({'best_val_epoch': epoch+1})
                results_dict.update({'val_metric_goal': 'maximize'})
