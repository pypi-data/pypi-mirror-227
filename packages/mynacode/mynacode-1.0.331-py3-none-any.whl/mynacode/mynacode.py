...
def data(train_set=[], train_labels=[], test_set=[], test_labels=[], val_set=[], val_labels=[], dataset_name="", #checks if data is same as prev
  ...
  print(newest_data_folder.split('/')[-1], newest_data_folder.split('/')[-1].split('_')[0])
  

  if (newest_data_folder == None) or (newest_data_folder.split('/')[-1].split('_')[0] != dataset_name):
  ...
  if save_new_data_flag == 1:   
      ...
      while os.path.exists(current_dir+'/data/'+dataset_name):  
          if len(str(dataset_name)) == 0:
              dataset_name = "dataset_" + ''.join(random.choice(string.ascii_uppercase + string.ascii_lowercase + string.digits) for _ in range(6))
          else:
      ...
      if len(train_set) > 0:
        config_dict.update({'train_set': newest_data_folder + '/train_set.pkl'})
      if len(val_set) > 0:
        config_dict.update({'val_set': newest_data_folder + '/val_set.pkl'})
      if len(test_set) > 0:
        config_dict.update({'test_set': newest_data_folder + '/test_set.pkl'})
      if len(train_labels) > 0:
        config_dict.update({'labels_train': newest_data_folder + '/train_labels.pkl'})
      if len(val_labels) > 0:
        config_dict.update({'labels_val': newest_data_folder + '/val_labels.pkl'})
      if len(test_labels) > 0:
        config_dict.update({'labels_test': newest_data_folder + '/test_labels.pkl'})
...
def torch_data(train_dataloader=None, val_dataloader=None, test_dataloader=None, save_once=True, label_index=1,
         problem_type = 'binary classification',  node_name="Datasets", run_id = None):
  ...
  if (newest_data_folder == None) or (newest_data_folder.split('/')[-1].split('_')[0] != dataset_name):
      ...
      if (len(train_dataloader) > 0) and (os.path.exists(newest_data_folder + '/train_dataloader.pkl')): #Both exist
          percent_20 = int((20/100)*len(train_labels))
          
          with open(newest_data_folder + '/train_dataloader.pkl', 'rb') as f:
              x = dill.load(f)

              if not np.array_equal(next(iter(train_loader))[label_index], next(iter(x))[label_index]): #Both exist but not equal
                  save_new_data_flag = 1
      elif (~(os.path.exists(newest_data_folder + '/train_dataloader.pkl'))) and (len(train_dataloader) == 0): #Both don't exist
          pass
      else: #One exists, one doesn't
          save_new_data_flag = 1
              
      if ((os.path.exists(newest_data_folder + '/val_dataloader.pkl')) and (save_new_data_flag == 0) and (len(val_dataloader) > 0)):
          percent_20 = int((20/100)*len(val_labels))

          with open(newest_data_folder + '/val_dataloader.pkl', 'rb') as f:
              x = dill.load(f)

              if not np.array_equal(next(iter(val_loader))[label_index], next(iter(x))[label_index]):
                  save_new_data_flag = 1
      elif ((~(os.path.exists(newest_data_folder + '/val_dataloader.pkl'))) and (save_new_data_flag == 0) and (len(val_dataloader) == 0)):
          pass  
      else:
          save_new_data_flag = 1


      if (os.path.exists(newest_data_folder + '/test_dataloader.pkl')) and (save_new_data_flag == 0) and (len(test_dataloader) > 0):
          percent_20 = int((20/100)*len(test_labels))

          with open(newest_data_folder + '/test_dataloader.pkl', 'rb') as f:
              x = dill.load(f)
              
              if not np.array_equal(next(iter(test_loader))[label_index], next(iter(x))[label_index]):
                  save_new_data_flag = 1
      elif ((~(os.path.exists(newest_data_folder + '/test_dataloader.pkl'))) and (save_new_data_flag == 0) and (len(test_dataloader) == 0)):
          pass
      else:
          save_new_data_flag = 1
