...
def torch_data(train_dataloader=None, val_dataloader=None, test_dataloader=None, dataset_name="", label_index=1,
  ...
  newest_data_folder = newest(current_dir+'/data')
  print("Latest:", newest_data_folder)
  print(newest_data_folder.split('/')[-1].split('_')[0])
  ...
  if newest_data_folder == None:
      ...
      if (len(train_dataloader) > 0) and (os.path.exists(newest_data_folder + '/train_dataloader.pkl')): #Both exist
          ...
          with open(newest_data_folder + '/train_dataloader.pkl', 'rb') as f:
              ...
              if not np.array_equal(next(iter(train_dataloader))[label_index], next(iter(x))[label_index]): #Both exist but not equal
                  print(next(iter(train_dataloader))[label_index], next(iter(x))[label_index])
                  save_new_data_flag = 1
      ...
      if ((os.path.exists(newest_data_folder + '/val_dataloader.pkl')) and (save_new_data_flag == 0) and (len(val_dataloader) > 0)):
          ...
          with open(newest_data_folder + '/val_dataloader.pkl', 'rb') as f:
              ...
              if not np.array_equal(next(iter(val_dataloader))[label_index], next(iter(x))[label_index]):
                  print(next(iter(val_dataloader))[label_index], next(iter(x))[label_index])
                  save_new_data_flag = 1
      ...
      if (os.path.exists(newest_data_folder + '/test_dataloader.pkl')) and (save_new_data_flag == 0) and (len(test_dataloader) > 0):
          ...
          with open(newest_data_folder + '/test_dataloader.pkl', 'rb') as f:
              ...
              if not np.array_equal(next(iter(test_dataloader))[label_index], next(iter(x))[label_index]):
                  print(next(iter(test_dataloader))[label_index], next(iter(x))[label_index])
                  save_new_data_flag = 1
