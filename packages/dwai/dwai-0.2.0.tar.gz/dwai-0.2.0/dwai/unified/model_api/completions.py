...
def zhipuai_chatglm_std(**kwargs):
    model = ModelAPI()
    json = model.invoke(model="chatglm_std", prompt=kwargs.get('prompt'),
...
def pangu_completions(**kwargs):
    ...
    json = chat.call(prompt=kwargs.get('prompt'), max_tokens=kwargs.get('max_tokens'),
...
def tione_chat_completion(**kwargs):
    chat = TioneClient()
    content = chat.ChatCompletion(content=kwargs.get('content')).to_json_string()
...
if __name__ == '__main__':
    ...
    # 测试zhipuai
    resp = sdk.call('kingsoft.default', prompt=[{"role": "user", "content": "人工智能"}], top_p=0.7, temperature=0.9)
    print(resp)
