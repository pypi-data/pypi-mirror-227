...
_NUM_PLAYERS = 2
_NUM_MAX_PEACE_STEP = 20
_NUM_ROWS = 8
_NUM_COLS = 3
_NUM_CELLS = _NUM_ROWS * _NUM_COLS
_NUM_CHESS_TYPES = 8
_NUM_CHESS_QUANTITY = 11

_TENSOR_SHAPE_LENGTH = _NUM_CHESS_TYPES + _NUM_CHESS_TYPES + _NUM_CHESS_TYPES + 46
...
_RAILWAY_ROW_IDX = [1, 3, 4, 6]
_RAILWAY_COL_IDX = [0, 2]
_CAMP_POSITIONS = [[2, 1], [5, 1]]

_BLOCK_DOWN_POSITIONS = []
_BLOCK_UP_POSITIONS = []

_BLOCK_MINE_ACTIONS = range(6, 18)
_BLOCK_BOMB_ACTIONS = range(9, 15)
_FLAG_ACTIONS = [1, 22]
...
class JunQiState(pyspiel.State):
    ...
    def __init__(self, game):
        ...
        self.chess_list = [
            [
                ...
                12, 10, 9, 8, 8,
                7, 2, 2,
                2, # This 2 is for avoiding error "Index out of range"
            ],
            [
                1, 11, 11,
                12, 10, 9, 8, 8,
    ...
    def _apply_action(self, action: int) -> None:
        ...
        if self.game_length == _GAME_INFO.max_game_length or self.game_length_peace == _NUM_MAX_PEACE_STEP:
            # End game by considered a draw
        ...
        if action == _NUM_COLS * _NUM_ROWS:
            ...
            #self._is_terminal = True
            #self._player0_score = -0.1 if player == 0 else 0.1
    ...
    def returns(self):
        ...
        return [self._player0_score, -self._player0_score] if not self.draw else [-1, -1]
    ...
    def __init__(self, iig_obs_type, params):
        ...
        self.shape = (  # pointers      |                     |                 |               |
            _NUM_ROWS, _NUM_COLS, (_NUM_CHESS_TYPES + _NUM_CHESS_TYPES + _NUM_CHESS_TYPES + self.num_history_move +
                                   scalar_shape + scalar_shape + scalar_shape + scalar_shape + prev_select_shape + camp_shape))
        #                          Remain Len.    Remain Mov.    Game Phase     Pha.Sele.Mov.  Prev. Selectoin     Camp Posi.
    ...
    def set_from(self, state, player):
        ...
        for row in range(_NUM_ROWS):
            for col in range(_NUM_COLS):
                ...
                def unrevealed(t):
                    if ((state.board[state.selected_pos[player][0]][
                             state.selected_pos[player][1]].type == ChessType.FLAG
                         or state.board[state.selected_pos[player][0]][
                             state.selected_pos[player][1]].type == ChessType.GENERAL)
                            and state.board[state.flags_pos[player][0]][state.flags_pos[player][1]].revealed == True):
                        return 0
                    else:
                        return _NUM_CHESS_QUANTITY_BY_TYPE[t + 1]

                def sum_unrevealedk(butexcept=[]):
                    __sum = 0
                    for t in range(_NUM_CHESS_TYPES):
                        if t not in butexcept:
                            __sum += unrevealed(t)
                    return __sum if __sum != 0 else 25
                    ...
                    if not pass_it:
                        ...
                        for t in range(self.num_history_move):
                            if prev_obs[row][col][t + 3 * _NUM_CHESS_TYPES] == 1:
                                for i in range(_NUM_CHESS_TYPES):
                                    obs[row][col][i + _idx] = unrevealed(i) / sum_unrevealedk(butexcept=[12, 1])
                # ...
                ...
                obs[row][col][_idx] = state.game_length / _GAME_INFO.max_game_length
                ...
                obs[row][col][_idx] = state.game_length_peace / _NUM_MAX_PEACE_STEP
                ...
                # An indication of wether a cell on map is a camp
                # For junqi only
                # Shape: _NUM_COLS * _NUM_ROWS tensor -> _NUM_COLS * _NUM_ROWS * 1 tensor.
                _idx = 3 * _NUM_CHESS_TYPES + self.num_history_move + 5
