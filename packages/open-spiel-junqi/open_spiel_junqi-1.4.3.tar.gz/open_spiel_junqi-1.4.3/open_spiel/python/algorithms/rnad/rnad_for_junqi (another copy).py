  ...
  """
    ...
    def init(self):
        ...
        def network(
            ...
            temp_obs = torso(jnp.pad(jnp.reshape(env_step.obs, (12, 5, 61)), [(0, 0), (0, 1), (0, 0)], mode='constant',
                                     constant_values=0))
...
class ConvResBlock(hk.Module):
    ...
    def __init__(
        ...
        self.skipin_deconv = hk.Conv2DTranspose(
            output_channels=channels // 2 if not stride == 2 else 128,
            kernel_shape=1,
            stride=stride,
            with_bias=False,
            padding="SAME",
    ...
    def __call__(self, inputs):
        ...
        skipout =  self.skipin_deconv(shortcut)
...
class DeconvResBlock(hk.Module):
    ...
    def __call__(self, inputs):
        ...
        print(jnp.shape(self.skipin))

        # if jnp.shape(skipin) != jnp.shape(out):
        # skipin = jnp.pad(skipin, [(0, 0), (0, 1), (0, 0)], mode='constant', constant_values=0)
...
class PyramidModuleIn(hk.Module):
...
class PyramidModuleOut(hk.Module):

    def __init__(self, M, N, skipin):
        super().__init__()
...
class PyramidModule(hk.Module):
    ...
    def __call__(self, input):
        pm0 = PyramidModuleIn(self.M, self.N)
        out, skipout = pm0(input)
        print("=====================")
        for i in skipout:
            print(i.shape)
        print("=====================")
        pm1 = PyramidModuleOut(self.M, self.N, skipout)
        out = pm1(out)
