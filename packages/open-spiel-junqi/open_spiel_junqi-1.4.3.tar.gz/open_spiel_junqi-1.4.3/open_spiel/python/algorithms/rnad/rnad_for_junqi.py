...
from open_spiel.python.games.junqi1 import _TENSOR_SHAPE_LENGTH, _NUM_CELLS, _NUM_ROWS, _NUM_COLS
import pyspiel
...
def _legal_policy(logits: chex.Array, legal_actions: chex.Array) -> chex.Array:
    ...
    try:
        ...
        logits = jnp.reshape(logits, (_NUM_CELLS + 1))
        chex.assert_equal_shape((logits, legal_actions))
  ...
  """
    ...
    def init(self):
        ...
        def network(
            ...
            torso = PyramidModule(N=0, M=0)  # env_step.obs
            pm_value_head = ValueHeadModule()
            pm_policy_head = PolicyHeadModule()

            # print(env_step.obs.shape)
            if env_step.obs.shape == (_NUM_CELLS * _TENSOR_SHAPE_LENGTH,):
                dm0 = 1
            else:
                dm0 = self.config.batch_size
            shape = (dm0, _NUM_ROWS, _NUM_COLS, _TENSOR_SHAPE_LENGTH)
            reshaped_obs = jnp.reshape(env_step.obs, shape)
            temp_obs = torso(
                jnp.pad(reshaped_obs, [(0, 0), (0, 0), (0, 1), (0, 0)], mode='constant', constant_values=0))

            logit = pm_policy_head(temp_obs)
            # Delete uselese extra column generated by deconv.
            # for example: 12 * 5 ->conv-> 12*5 -> deconv -> 12*6
            logit = jnp.reshape(jnp.delete(logit, _NUM_COLS, axis=2), (dm0, _NUM_CELLS))
            v = pm_value_head(temp_obs)
            v = jnp.reshape(jnp.delete(v, _NUM_COLS, axis=2), (dm0, _NUM_CELLS))
    ...
    def step(self):
        ...
        timestep = self.collect_batch_trajectory(self.config.batch_size)
    ...
    def collect_single_timestep(self):
        state = self._play_chance(self._game.new_initial_state())

    #@measure_time
    def collect_batch_trajectory(self, batch_size) -> TimeStep:
        batch_size = self.config.batch_size if batch_size == 0 else batch_size
        states = [
            self._play_chance(self._game.new_initial_state())
...
class DeconvResBlock(hk.Module):
    ...
    def __init__(
        ...
        self.skipin_deconv = hk.Conv2DTranspose(
            output_channels=channels // 2 if stride != 2 else 32,
            kernel_shape=1,
            stride=stride,
            with_bias=False,
            padding="SAME",
...
class PolicyHeadModule(hk.Module):
    ...
    def __init__(self):
        ...
        self.pm_policy_head = PyramidModule(N=0, M=0)
