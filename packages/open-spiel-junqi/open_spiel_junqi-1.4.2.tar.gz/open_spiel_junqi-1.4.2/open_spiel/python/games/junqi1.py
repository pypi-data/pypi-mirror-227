...
_GAME_INFO = pyspiel.GameInfo(
    ...
    max_utility=1.0,
    utility_sum=0.0,
    max_game_length=500  # _NUM_CELLS * _NUM_CELLS
)
    ...
    def set_from(self, state, player):
        ...
        for row in range(_NUM_ROWS):
            for col in range(_NUM_COLS):
                ...
                def unrevealed(t):
                    if ((state.board[state.selected_pos[player][0]][state.selected_pos[player][1]].type == ChessType.FLAG
                         or state.board[state.selected_pos[player][0]][state.selected_pos[player][1]].type == ChessType.GENERAL)
