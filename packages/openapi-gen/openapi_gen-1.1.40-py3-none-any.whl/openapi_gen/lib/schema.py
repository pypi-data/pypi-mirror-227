from openapi_gen.lib.constants import (
    AuthType,
    ComponentType,
    ContentType,
    Meta,
...
import random
import string
from openapi_gen.lib.endpoint import SwaggerEndpoint
from openapi_gen.lib.metadata import EndpointMetadata
from openapi_gen.lib.wrappers import get_endpoint_metadata
from openapi_gen.lib.utils import (
    element_at,
    first,
    validate_constant,
    defined,
    ...
    def _create_endpoint_definition(
        ...
        for method in endpoint.methods:
            ...
            # Add generic metadata if it's not defined on the route
                ...
                metadata = get_endpoint_metadata(
                    view_function_name='get')
                
                # check if parameters are in this endpoint.  get may have multiple endpoints 
                # and not all of them have parameters
                pm_list = parameters.copy()
                for param in pm_list:
                    if endpoint.endpoint_literal.count(param.get(Schema.NAME)) == 0:
                        parameters.remove(param)
                        endpoint.segment_params.remove(param.get(Schema.NAME))
            ...
            method_definition[Schema.OPERATION_ID] = endpoint.tag + "-" + endpoint.component_key +  "-" + ''.join(random.choices(string.ascii_uppercase + string.digits, k=5))
            
            method_definition[Schema.X_GOOGLE_BACKEND] = {
                Schema.ADDRESS: self._app_servers[0]
            }
    ...
    def _add_metadata(
        ...
        if metadata.security:
            ...
            if security_schema.auth_type == AuthType.OAUTH_2:
                ...
                if method_definition[Schema.JWKS_URI] is None:
                    raise Exception("JWKS URI must be provided when using an OAuth flow")
        ...
        return {
            Schema.CONTENT: {
                ContentType.APPLICATION_JSON: {
                    Schema.SCHEMA: {
                        Schema.REF: f'#' + separator + 'components' + separator + 'schemas' + separator + endpoint.component_key
                    }
                }
            }
        }
