...
class PushSingleService:
        ...
        try:
            params = format_params_handle(
                ...
                filter_filed_list=[
                    ...
                    "title|str", "content|str", "send_type|str", "files|json", "is_jump_link|int", "link|str", "snapshot|dict"
                ],
            )
    ...
    def list(params: dict = None, filter_fields: "list|str" = None, only_first: bool = False, need_pagination: bool = True, **kwargs):
        ...
        sort = sort if sort in ["created_time", "-created_time", "id", "-id"] else "-id"
        # -------------------------- section 强制类型 --------------------------------
        ...
        filter_fields_list = filter_fields_handler(
            ...
            all_field_list=[
                ...
                "send_type", "files", "is_read", "is_delete", "created_time", "is_jump_link", "thread_id", "link", "snapshot"
            ]
        )
        # -------------------------- section 参数过滤 --------------------------------
    ...
    def group_list(params: dict = None, filter_fields: "list|str" = None, only_first: bool = False, need_pagination: bool = True, **kwargs):
        ...
        push_record_query = base_db.table("push_single").select_raw("""
                    ...
                    date_format(updated_time, "%%Y-%%m-%%d %%H:%%m:%%s") as updated_time
                """).order_by("updated_time", "desc")
        for i in query_set:
            current_user_newest_record = push_record_query.where("to_user_id", i["to_user_id"]).first()
