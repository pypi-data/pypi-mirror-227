...
class SinglePushPipeline(PipelineBase):
    worker_notice_title = {
        ...
        "check_fail": {
            "title": "派台审核不通过",
            "content": "请您修改标书文件后重新上传",
        },
        "accept_success": {
            "title": "完成订单",
            "content": "恭喜您的，您的订单完成验收",
        },
        "accept_fail": {
            "title": "客户拒绝订单",
            "content": "请修改后再次上传标书文件",
        },
        "cancel_publish": {
            "title": "订单关闭",
            "content": "客户已取消订单",
    ...
    custom_notice_title = {
        ...
        "appoint": {
            "title": "已派单，等待付款",
            "content": "付款后镖师收到通知，开始制作",
        },
        "pay": {
            "title": "等待镖师上传标书",
            "content": "订单完成付款，待镖师提交标书",
        },
        "upload": {
            "title": "平台审核资料",
    ...
    def process(*args, params: dict = None, **kwargs):
        ...
        add_params = {"thread_id": thread_id, "source_code": "订单动态", "snapshot": params, "is_jump_link": True}
        ...
        # 标书初审通过
            ...
            if bx_worker:
                ...
                add_params["snapshot"]["worker_status_code"] = 234
                PushSingleService.add(params=add_params)
