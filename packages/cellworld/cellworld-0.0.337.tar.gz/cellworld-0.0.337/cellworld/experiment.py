...
class Trajectories(JsonList):
    ...
    def get_step_index_by_time_stamp(self, time_stamp: float, exact: bool = False) -> int:
        ...
        if exact and self[i].time_stamp != time_stamp:
    ...
    def get_step_index_by_frame(self, frame: int, exact: bool = True) -> int:
        i = bisect_left(self, frame, key=lambda s: s.time_stamp)
        if exact and self[i].time_stamp != frame:
    ...
    def get_segment(self, start_frame=None, end_frame=None, start_time=None, end_time=None):
        ...
        if start_time is not None:
            s = self.get_step_index_by_time_stamp(start_time)
        if start_frame is not None:
            s = self.get_step_index_by_frame(start_frame)

        if end_time is not None:
            e = self.get_step_index_by_time_stamp(end_time)
        if end_frame is not None:
            e = self.get_step_index_by_frame(end_frame)
