...
class Tactigon_RT_Computing:
    ...
    def __init__(self, config: RealTimeConfig, tactigon="RIGHT"):
        if tactigon == "RIGHT":
            model_name = config.MODEL_NAME_RIGHT
        else:
            model_name = config.MODEL_NAME_LEFT

        with open(path.join(config.model_full_path, model_name, "model.pickle"), "rb", ) as file:
            self.model = pickle.load(file)
        
        with open(path.join(config.model_full_path, model_name, "encoder.pickle"),"rb",) as file:
